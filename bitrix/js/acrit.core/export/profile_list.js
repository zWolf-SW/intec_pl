...
//
function acrit_exp_move_profile_to_group(action){
	let
		id = parseInt($('#acrit_exp_move_to_group_select').val());
	if(isNaN(id)){
		id = 0;
	}
	eval(action.replace(/#GROUP#/, id));
}
...
$(document).ready(function(){
	$('.adm-list-table-top a[onclick*="new_course.php"]').attr('id', 'acrit-exp-button-teacher');
});