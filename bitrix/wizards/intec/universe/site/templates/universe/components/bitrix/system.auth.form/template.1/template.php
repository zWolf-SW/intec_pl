...
<div class="ns-bitrix c-system-auth-form c-system-auth-form-template-1" id="<?= $strMainID ?>">
    <div class="system-auth-form-tabs-panel intec-grid intec-grid-wrap intec-grid-a-v-center">
        <div class="system-auth-form-tabs-panel-item tab-auth intec-grid-item active"
            onclick="template.api.components.get(<?=CUtil::PhpToJSObject($arAuthOptions)?>).then(function (popup) {
                $('#<?= $strMainID ?> .system-auth-form-body').html(popup);
            })">
            <?= Loc::getMessage('SYSTEM_AUTH_FORM_TEMPLATE1_AUTHORIZATION') ?>
        </div>
        <div class="system-auth-form-tabs-panel-item tab-reg intec-grid-item"
            onclick="template.api.components.get(<?=CUtil::PhpToJSObject($arRegOptions)?>).then(function (popup) {
                $('#<?= $strMainID ?> .system-auth-form-body').html(popup);
            })">
