...
foreach ($arResult['SKU_PROPS'] as $arSKUProperty) {
    ...
    if ($arSKUProperty['SHOW_MODE'] !== 'TEXT' ) {
        foreach ($arSKUProperty['VALUES'] as $arValue) {
            foreach ($arResult['FIELDS']['OFFERS'] as $key => $arOffer) {
                if ($arValue["XML_ID"] == $arResult['FIELDS']['OFFERS'][$key][$arSKUProperty['CODE']]["VALUE"]) {
                    $arResult['FIELDS']['OFFERS'][$key][$arSKUProperty['CODE']]["VALUE"] = $arValue["NAME"];
                }
            }
        }
    }
