...
if (Loader::includeModule('intec.regionality')) {
    $arTemplateParameters['REGIONALITY_USE'] = [
        'PARENT' => 'BASE',
        'NAME' => Loc::getMessage('C_CATALOG_CATALOG_1_REGIONALITY_USE'),
        'TYPE' => 'CHECKBOX',
        'DEFAULT' => 'N',
        'REFRESH' => 'Y'
    ];
    ...
    if ($arCurrentValues['REGIONALITY_USE'] === 'Y') {
        if (!empty($arIBlock)) {
            $arTemplateParameters['REGIONALITY_FILTER_USE'] = [
                'PARENT' => 'BASE',
                'NAME' => Loc::getMessage('C_CATALOG_CATALOG_1_REGIONALITY_FILTER_USE'),
                'TYPE' => 'CHECKBOX',
                'DEFAULT' => 'N',
                'REFRESH' => 'Y'
            ];
            ...
            if ($arCurrentValues['REGIONALITY_FILTER_USE'] === 'Y') {
                $arTemplateParameters['REGIONALITY_FILTER_PROPERTY'] = [
                    'PARENT' => 'DATA_SOURCE',
                    'NAME' => Loc::getMessage('C_CATALOG_CATALOG_1_REGIONALITY_FILTER_PROPERTY'),
                    'TYPE' => 'LIST',
                ...
                $arTemplateParameters['REGIONALITY_FILTER_STRICT'] = [
                    'PARENT' => 'BASE',
                    'NAME' => Loc::getMessage('C_CATALOG_CATALOG_1_REGIONALITY_FILTER_STRICT'),
                    'TYPE' => 'CHECKBOX',
                    'DEFAULT' => 'N'
                ];
            }
        ...
        if ($bBase) {
            $arTemplateParameters['REGIONALITY_STORES_USE'] = [
                'PARENT' => 'BASE',
                'NAME' => Loc::getMessage('C_CATALOG_CATALOG_1_REGIONALITY_STORES_USE'),
                'TYPE' => 'CHECKBOX',
                'DEFAULT' => 'N',
                'REFRESH' => 'Y'
            ];

            if ($arCurrentValues['REGIONALITY_STORES_USE'] === 'Y') {
                $arTemplateParameters['STORES'] = ['HIDDEN' => 'Y'];
            }
        }
        ...
        if ($bBase || $bLite) {
            $arTemplateParameters['REGIONALITY_PRICES_TYPES_USE'] = [
                'PARENT' => 'BASE',
                'NAME' => Loc::getMessage('C_CATALOG_CATALOG_1_REGIONALITY_PRICES_TYPES_USE'),
                'TYPE' => 'CHECKBOX',
                'DEFAULT' => 'N',
                'REFRESH' => 'Y'
            ];
