...
<?php if ($arForm['STATUS'] === 'empty') {
    ...
    if (empty($sSubmitText))
        $sSubmitText = Loc::getMessage('C_REVIEWS_TEMPLATE_3_FORM_BUTTON_TEXT')
...
?>
    <div class="reviews-form-toggle">
        <?= Html::beginTag('div', [
            ...
            <?= Html::tag('div', Loc::getMessage('C_REVIEWS_TEMPLATE_3_FORM_TOGGLE_BUTTON_TEXT'), [
                'class' => [
                        ...
                        <?php foreach ($arForm['ERROR'] as $sError) { ?>
                            <div class="reviews-form-error-part">
                                <?php if ($sError === 'required') { ?>
                                    <?= Loc::getMessage('C_REVIEWS_TEMPLATE_3_FORM_ERROR_REQUIRED') ?>
                                <?php } else if ($sError === 'captcha') { ?>
                                    <?= Loc::getMessage('C_REVIEWS_TEMPLATE_3_FORM_ERROR_CAPTCHA') ?>
                                <?php } ?>
                            </div>
                        <?php } ?>
                    <?= Html::endTag('div') ?>
                        ...
                        <label class="intec-ui intec-ui-control-checkbox intec-ui-scheme-current">
                            ...
                            <span class="intec-ui-part-content">
                                <?php if (!empty($arVisual['CONSENT']['URL'])) { ?>
                                    <?= Loc::getMessage('C_REVIEWS_TEMPLATE_3_FORM_CONSENT_URL', [
                                         '#URL#' => $arVisual['CONSENT']['URL']
                                    ]) ?>
                                <?php } else { ?>
                                    <?= Loc::getMessage('C_REVIEWS_TEMPLATE_3_FORM_CONSENT_DEFAULT') ?>
                                <?php } ?>
                            </span>
                        </label>
                    </div>
...
<?php } else if ($arForm['STATUS'] === 'exists') { ?>
    <div class="reviews-form-message reviews-form-success">
        <?= Loc::getMessage('C_REVIEWS_TEMPLATE_3_FORM_REVIEW_EXISTS') ?>
    </div>
<?php } else if ($arForm['STATUS'] === 'added') { ?>
    <div class="reviews-form-message reviews-form-success">
        <?php if ($arVisual['FORM']['ADD_MODE'] === 'disabled') { ?>
            <?= Loc::getMessage('C_REVIEWS_TEMPLATE_3_FORM_REVIEW_ADDED_DISABLED') ?>
        <?php } else if ($arVisual['FORM']['ADD_MODE'] === 'active') { ?>
            <?= Loc::getMessage('C_REVIEWS_TEMPLATE_3_FORM_REVIEW_ADDED_ACTIVE') ?>
        <?php } ?>
    </div>
<?php } ?>