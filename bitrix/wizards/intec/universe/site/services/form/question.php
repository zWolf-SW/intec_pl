...
if (!empty($form)) {
    ...
    $macros['FORMS_'.$code.'_FIELDS_TEXT_ID'] = null;

    $answer = ArrayHelper::getValue($form, ['FIELDS', 'TEXT', 'ANSWERS', 0]);

    if (!empty($answer))
        $macros['FORMS_'.$code.'_FIELDS_TEXT_ID'] = 'form_'.$answer['FIELD_TYPE'].'_'.$answer['ID'];
