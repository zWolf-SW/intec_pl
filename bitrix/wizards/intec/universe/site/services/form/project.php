...
$form = $import($code, [
    ...
    'name' => Loc::getMessage('wizard.services.form.fields.PROJECT.name'),
    'code' => 'PROJECT',
    'type' => 'text',
    'required' => true
], [
    ...
    'name' => Loc::getMessage('wizard.services.form.fields.COMMENT.name'),
    'code' => 'COMMENT',
    'type' => 'textarea',
    'required' => false
]], [[
...
if (!empty($form)) {
    ...
    $macros['FORMS_'.$code.'_FIELDS_PROJECT_ID'] = null;

    $answer = ArrayHelper::getValue($form, ['FIELDS', 'PROJECT', 'ANSWERS', 0]);

    if (!empty($answer))
        $macros['FORMS_'.$code.'_FIELDS_PROJECT_ID'] = 'form_'.$answer['FIELD_TYPE'].'_'.$answer['ID'];
