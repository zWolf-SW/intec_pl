...
class IntecSaleBasketSmallComponent extends IBlockElements
{
    ...
    public function getBasketItems($arCurrency)
    {
        ...
        if ($this->getIsBase()) {
            ...
            $pricesBasketItems = $this->getBasketItemsPriceBase();
            ...
            foreach ($arBasket as $arBasketItem) {
                ...
                $arPrice = [
                    ...
                    'BASE' => [
                        'VALUE' => Type::toFloat($pricesBasketItems[$arBasketItem['ID']]['BASE_PRICE']?$pricesBasketItems[$arBasketItem['ID']]['BASE_PRICE']:$arBasketItem['BASE_PRICE'])
                    ],
                    'DISCOUNT' => [
                        'VALUE' => Type::toFloat($pricesBasketItems[$arBasketItem['ID']]['PRICE']?$pricesBasketItems[$arBasketItem['ID']]['PRICE']:$arBasketItem['PRICE'])
                    ]
                ];
    ...
    public function getBasketItemsPriceBase()
    {
        // Получаем корзину без отложенных и недоступных товаров
