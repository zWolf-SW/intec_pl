...
class ProfileFieldFeature {
	...
	public static function getIBlockFeatures($intProfileID, $intIBlockID){
		...
		static::replaceSomeCustomFields($arFields);
		#
	...
	/**
	 *	Replace some custom fields: CREATED_BY__NAME => CREATED_BY, MODIFIED_BY__NAME => MODIFIED_BY
	 */
	protected static function replaceSomeCustomFields(&$arFields){
		foreach($arFields as $strGroup => $arGroupFields){
			foreach($arGroupFields as $intKey => $strField){
				if($strField == 'CREATED_BY__NAME'){
					$arFields[$strGroup][$intKey] = 'CREATED_BY';
				}
				elseif($strField == 'MODIFIED_BY__NAME'){
					$arFields[$strGroup][$intKey] = 'MODIFIED_BY';
				}
			}
		}
	}
