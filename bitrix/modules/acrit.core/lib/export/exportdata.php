...
abstract class ExportDataTable extends Entity\DataManager {
	...
	/**
	 *	Set exported flag to 'N' for all profile items
	 */
	public static function setAllDataItemsNotExported($intProfileId){
		$strSql = sprintf("UPDATE %s SET EXPORTED = NULL WHERE PROFILE_ID='%s' AND EXPORTED = 'Y'",
			static::getTableName(), $intProfileId);
		\Bitrix\Main\Application::getConnection()->query($strSql);
		return true;
	}
