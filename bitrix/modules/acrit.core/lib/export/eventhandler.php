...
class EventHandlerExport {
	...
	*/
	...
	public static function OnProlog(){
		...
		if ($strCurrentUrl == '/bitrix/tools/conversion/ajax_counter.php'){
			return;
		}
		...
		$arJsonInput = Helper::getInputJson();
		foreach(Exporter::getExportModules() as $strModuleId){
			if(\Bitrix\Main\Loader::includeModule($strModuleId)){
				foreach(['Profile', 'OrdersProfiles'] as $strProfileType){
					$arProfiles = Helper::call($strModuleId, $strProfileType, 'getProfiles', $arArguments);
					if(!empty($arProfiles)){
						foreach($arProfiles as $intProfileId => $arProfile){
							...
							if($arProfile['EXTERNAL_REQUEST_URL'] == $strCurrentUrl){
								...
								$strPattern = sprintf('#%s#', $arProfile['EXTERNAL_REQUEST_URL']);
								$bSuccess = !!preg_match($strPattern, $strCurrentUrl, $arMatch);
							}
							if($bSuccess){
								...
								Helper::call($strModuleId, $strProfileType, 'execPlugin', [$intProfileId, $arParams]);
							}
						}
					}
				}
