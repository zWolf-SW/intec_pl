...
class SettingsReplace extends SettingsBase {
	...
	public static function showSettings($strFieldCode, $obField, $arParams){
		...
		$bRegExp = in_array($obField->getModuleId(), array_slice(\Acrit\Core\Export\Exporter::getInstance($obField->getModuleId())->getExportModules(true), -2));
		...
		foreach($arReplace['from'] as $key => $value){
			$arReplaceTmp[] = array(
				...
				'WHOLE' => is_array($arReplace['whole']) ? $arReplace['whole'][$key] : '',
				...
				<tbody>
					<?foreach($arReplaceTmp as $arReplaceItem):?>
						...
						<tr <?if($bIsHidden):?> class="acrit-exp-field-settings-replace-row-hidden" data-noserialize="Y"<?endif?>>
							...
							<?endif?>
							<td class="cell-whole">
								<label class="checkbox" title="<?=static::getMessage('WHOLE_HINT');?>">
									<input type="hidden" name="<?=static::getCode();?>[whole][]" value="<?=$arReplaceItem['WHOLE'];?>" />
									<input type="checkbox" data-role="replace-whole"<?if($bIsHidden):?> class="no-checkbox-styling"<?endif?><?if($arReplaceItem['WHOLE']=='Y'):?> checked="checked"<?endif?> />
									<?=static::getMessage('WHOLE');?>
								</label>
							</td>
		...
		if(is_array($arParams[static::getCode()]) && !empty($arParams[static::getCode()]['from'])){
			static::processMultipleValue($mValue, $arParams, $obField, function(&$strValue, $arParams, $obField){
				...
				foreach($mParamValue['from'] as $key => $value){
					...
					$bWhole = $mParamValue['whole'][$key]=='Y';
					if($mParamValue['use_regexp'][$key]=='Y'){
						$strModifier = trim($mParamValue['modifier'][$key]);
						if($bCaseSensitive && Helper::stripos($strModifier, 'i') === false){
							$strModifier = 'i'.$strModifier;
						}
						$strPattern = $mParamValue['from'][$key];
						$strPattern = str_replace('#', '\#', $strPattern);
						$strPattern = '#'.$strPattern.'#'.$strModifier;
						$strValue = preg_replace($strPattern, $mParamValue['to'][$key], $strValue);
					}
					elseif($bWhole){
						$strPattern = $mParamValue['from'][$key];
						$strValueTmp = $strValue;
						if(!$bCaseSensitive){
							$strPattern = toLower($strPattern);
							$strValueTmp = toLower($strValueTmp);
						}
						if($strPattern == $strValueTmp){
							$strValue = $mParamValue['to'][$key];
						}
					}
					else {
						if($bCaseSensitive) {
