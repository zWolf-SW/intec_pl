...
class Html2Text {
    ...
    */
    ...
    protected function convertBlockquotes(&$text)
    {
        if (preg_match_all('/<\/*blockquote[^>]*>/i', $text, $matches, PREG_OFFSET_CAPTURE)) {
            ...
            foreach ($matches[0] as $m) {
                $m[1] = Helper::strlen(Helper::substr($originalText, 0, $m[1]));
                if ($m[0][0] == '<' && $m[0][1] == '/') {
                    $level--;
                    if ($level < 0) {
                        ...
                        $body = Helper::substr($text, $start + $taglen - $diff, $len);
                        ...
                        // Replace content
                        $text = Helper::substr($text, 0, $start - $diff)
                            . $body
                            . Helper::substr($text, $end + Helper::strlen($m[0]) - $diff);

                        $diff += $len + $taglen + Helper::strlen($m[0]) - Helper::strlen($body);
                        unset($body);
                    }
                } else {
                    if ($level == 0) {
                        $start = $m[1];
                        $taglen = Helper::strlen($m[0]);
                    }
                    $level++;
                }
            }
    ...
    {
        switch (Helper::strtolower($matches[1])) {
            case 'p':
