...
class DateFormat extends SettingsBase {
	...
	public static function showSettings($strFieldCode, $obField, $arParams){
		...
		#
		if(!Helper::strlen($arParams[$strInputFrom])){
	...
	public static function process(&$mValue, $arParams, $obField=null){
		if($arParams[static::getCode()]=='Y'){
			...
			static::processMultipleValue($mValue, $arParams, $obField, function(&$strValue, $arParams, $obField){
				...
				static::replaceDefaultFormats($arParams[$strInputFrom]);
				if(Helper::strlen($arParams[$strInputFrom]) && Helper::strlen($arParams[$strInputTo])){
	...
	protected static function replaceDefaultFormats(&$strFormat){
		$arReplace = [
			'#DATE#' => \CDatabase::dateFormatToPHP(FORMAT_DATE),
			'#DATETIME#' => \CDatabase::dateFormatToPHP(FORMAT_DATETIME),
		];
		$strFormat = str_replace(array_keys($arReplace), array_values($arReplace), $strFormat);
	}
	...
	protected static function changeDate(&$strValue, $obDateTime, $arParams, $obField){
		...
		if($arParams[$strInputChange] == 'Y'){
			try {
				...
				if(!Helper::strlen($strDateIntervalType)){
