...
class ValueCondition extends ValueBase {
	...
	public function processValuesForElement(array $arElement, array $arProfile){
		...
		$intMainElementId = $this->obField->getMainElementId(); # If offer, this is ID of parent element, else - current element ID
		$bResult = Helper::call($this->obField->getModuleId(), 'Profile', 'isItemSatisfy', [$this->strConditions, $arElement['IBLOCK_ID'], $arElement['ID'], $bIncludeSubsections, $intMainElementId]);
		$arValuesGrouped = $this->groupValues();
		#
