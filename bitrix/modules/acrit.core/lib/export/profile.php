...
abstract class ProfileTable extends Entity\DataManager {
	...
	public static function getMap() {
		return array(
			...
			'GROUP_ID' => new Entity\IntegerField('GROUP_ID', array(
				'title' => Loc::getMessage('ACRIT_EXP_PROFILE_FIELD_GROUP_ID'),
			)),
	...
	public static function updateIBlockSettings($intProfileID, $intIBlockID, $strPluginClass, $arData){
		...
		if(strlen($strPluginClass) && class_exists($strPluginClass) && is_array($arData['FIELDS'])) {
			...
			Helper::call(static::MODULE_ID, 'ProfileField', 'removeOldFields', [$intProfileID, $intIBlockID, $arExistFields]);
			Helper::call(static::MODULE_ID, 'ProfileValue', 'removeOldFields', [$intProfileID, $intIBlockID, $arExistFields]);
			// Clear cache
			static::clearProfilesCache();
			unset($obPlugin);
			//
			...
			foreach($arFieldValues['params'] as $strSuffix => $arSuffixItems){
				foreach($arSuffixItems as $intIndex => $value){
					...
					foreach($arFieldValues as $key => $values){
						if(!is_array($values)){ # additional field, name is string!
							continue;
						}
	 ...
	 *	Get compiled filter
	 */
	public static function getFilter($intProfileID, $intIBlockID, $intMainElementId=null){
		$arResult = &static::$arCacheGetFilter[static::MODULE_ID][$intProfileID.'_'.$intIBlockID.'_'.$intMainElementId];
		if(!is_array($arResult)){
			...
			if($arItem = $resData->fetch()){
				...
				if($intMainElementId){
					$obFilter->setMainElementID($intMainElementId);
				}
				...
				foreach(\Bitrix\Main\EventManager::getInstance()->findEventHandlers(static::MODULE_ID, 'OnGetProfileFilter') as $arHandler) {
					ExecuteModuleEventEx($arHandler, array(&$arResult, $intProfileID, $intIBlockID, $intMainElementId));
				}
			}
		}
		return $arResult;
	 ...
	 */
	public static function isItemSatisfy($strConditions, $intIBlockID, $intElementID, $bIncludeSubsections=false, $intMainElementId=null){
		if($arCatalog = Helper::getCatalogArray($intIBlockID)){
			if(is_array($arCatalog) && is_numeric($arCatalog['PRODUCT_IBLOCK_ID']) && $arCatalog['PRODUCT_IBLOCK_ID'] > 0){
				$arParentFilter = [
					'IBLOCK_ID' => $intIBlockID,
					'ID' => $intElementID,
				];
				if($arThisOffer = \CIBlockElement::getList([], $arParentFilter, false, false, ['PROPERTY_CML2_LINK'])->fetch()){
					if($arThisOffer['PROPERTY_CML2_LINK_VALUE']){
						$intMainElementId = $arThisOffer['PROPERTY_CML2_LINK_VALUE'];
					}
				}
			}
		}
	...
	*/
		...
		if($obCache->InitCache($intCacheLifeTime, $strCacheID, $strCacheDir)) {
			...
			while($arProfile = $resProfiles->fetch()){
				...
				while($arProfileIBlock = $resProfileIBlocks->fetch()){
					// Helper::decodeEmojii($arProfileIBlock);
				...
				if($bGetFields){
					...
					while($arProfileField = $resProfileFields->fetch()){
						// Helper::decodeEmojii($arProfileField);
						// decompile params
					...
					while($arProfileValue = $resProfileValues->fetch()){
						// Helper::decodeEmojii($arProfileValue);
						// decompile params
	...
	public static function getTmpDir($intProfileID, $bAutoCreate=true, $bRelative=false){
		$strTmpDir = Helper::getTmpDir($bAutoCreate, $bRelative, static::MODULE_ID);
