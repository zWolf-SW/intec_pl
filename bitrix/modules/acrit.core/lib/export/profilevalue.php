...
abstract class ProfileValueTable extends Entity\DataManager {
	 ...
	 */
	public static function loadFieldValuesAll($intProfileID, $intIBlockID, $strField=false){
		$arResult = array();
		$arFilter = array(
	 ...
	 */
	public static function removeOldFields($intProfileID, $intIBlockID, $arExistFieldCode){
		\Bitrix\Main\Application::getConnection()->startTransaction();
		$arFilter = ['PROFILE_ID' => $intProfileID, 'IBLOCK_ID' => $intIBlockID, '!FIELD' => $arExistFieldCode];
		$arSelect = ['ID'];
		$resOldItems = static::getList(['filter' => $arFilter, 'select' => $arSelect]);
		while($arOldItem = $resOldItems->fetch()){
			static::delete($arOldItem['ID']);
		}
