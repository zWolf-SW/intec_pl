...
class Cli {
	...
	/**
	 *	Check software on Linux platform
	 */
	public static function checkLinuxSoftwareTool($strTool){
		$arOutput = null;
		exec('which '.$strTool, $arOutput);
		if(is_array($arOutput) && !empty($arOutput)) {
			return $arOutput[0];
		}
		return false;
	}
	...
	public static function isHostingTimeweb(){
		if(static::isExec() && static::isLinux()) {
			@exec('uname -a', $arExec);
			if(stripos($arExec[0], 'timeweb.ru') !== false || stripos($arExec[0], 'tmweb.ru') !== false) {
				return true;
			}
		}
		return false;
	...
	public static function isMultithreadingSupported(){
		if(static::isMultithreadingDisabled()){
			return false;
		}
	...
	/**
	 * Check multithreading is manually disabled
	 */
	public static function isMultithreadingDisabled(){
		return defined('ACRIT_DISABLE_MULTITHREADING') && ACRIT_DISABLE_MULTITHREADING === true;
	}
	 ...
	 */
	public static function getFullCommand($strModuleId, $strScriptName, $intProfileId=null, $strOutputFilename=null, $arArguments=null){
		...
		$strCommand = static::getProfilePhpCommand($strModuleId, $intProfileId, $strScriptName, $arArguments);
		$strCommandClear = static::getProfilePhpCommand($strModuleId, $intProfileId, $strScriptName, $arArguments, true);
