...
class Log {
	...
	protected $strLogSubdir;
	
	/**
	 *	
	 */
	protected function __construct($strModuleId, $strLogSubdir=null){
		$this->strModuleId = $strModuleId;
		$this->strLogSubdir = strVal($strLogSubdir);
	}
	
	/**
	 *	Get instance for selected module
	 */
	public static function getInstance($strModuleId, $strLogSubdir=null){
		$arModuleObjects = &static::$arModuleObjects;
		$strKey = sprintf('%s_%s', $strModuleId, strVal($strLogSubdir));
		if(!array_key_exists($strKey, $arModuleObjects)){
			$arModuleObjects[$strKey] = new static($strModuleId, $strLogSubdir);
		}
		return $arModuleObjects[$strKey];
	}
	...
	public function getLogFilename($intProfileID=false, $strRelative=false){
		...
		$strSubdir = preg_replace('#\W#', '', $this->strLogSubdir);
		if(strlen($strSubdir)){
			$strSubdir = sprintf('%s/', $strSubdir);
		}
		$strResult = \Bitrix\Main\Loader::getDocumentRoot().'/'.$strUploadDir.'/'.$this->strModuleId.'/log/'.$strSubdir.$strBasename;
		if($strRelative){
	 ...
	 *	Get url for log
	 */
	public function getLogUrl($intProfileID=false, $strLogSubdir=false, $bDownload=false){
		$arGet = array(
	...
	public function showLog($intProfileId=null, $bFull=false){
		...
		print Helper::getHtmlObject($this->strModuleId, $intProfileId, 'log', ($bFull ? 'full' : 'lite'), ['strLogSubdir' => $this->strLogSubdir]);
	}
