 ...
 * <li> SECRET_CODE string mandatory
 * </ul>
 *
...
//				'required' => true,
			...
			'SECRET_CODE' => array(
				'data_type' => 'text',
//				'required' => true,
				'title' => Loc::getMessage('CRM_PROFILES_ENTITY_SECRET_CODE_FIELD'),
			),
		);
	}
...
//		} elseif($obCache->StartDataCache()) {
			...
			$resProfiles = Helper::call(static::MODULE_ID, 'OrdersProfiles', 'getList', [$arQuery]);
...
//		Log::getInstance(static::MODULE_ID)->deleteLog($primary);
	...
	public static function execPlugin($intProfileId, $arParams = [])
	{
		...
		if ($arProfile = static::getProfiles($intProfileId))
		{
			//$arProfile['PLUGIN'];
			//\Acrit\Exportproplus\ProfileTable::execPlugin(68, 'get_stocks');
			if ($arPlugins = \Acrit\Core\Orders\Exporter::getInstance(static::MODULE_ID)->findPlugins())
			{
				$obPlugin = new $arPlugins[$arProfile['PLUGIN']]['CLASS'](static::MODULE_ID);
