...
class Update{
	 ...
	 *	Check module state
	 */
	public static function checkModuleState($strModuleId, &$intDateTo, &$strLastVersion, $bCheckCore=true){
		$arResult = static::getAllAvailableUpdates();
		if(is_array($arResult['MODULE'])){
			foreach($arResult['MODULE'] as $arModule){
				if($arModule['@']['ID'] == $strModuleId){
					return $arModule['@'];
				}
			}
		}
	...
	protected static function getAllAvailableUpdates(){
		...
		if(class_exists('\CUpdateClientPartner')) {
			...
			$strError = null;
			$arResult = \CUpdateClientPartner::getUpdatesList($strError, LANGUAGE_ID, 'Y', $arModulesId, ['fullmoduleinfo'=>'Y']);
