	...
	public function execute(){
		...
		foreach($arProfilesId as $intProfileId){
			...
			if(!$bLocked){
				...
				if($mResult == Exporter::RESULT_SUCCESS){
					...
					if(is_array($arProfile) && $arProfile['ONE_TIME'] == 'Y' && Cli::isProfileOnCron($this->strModuleId, $intProfileId, 'export.php')){
						if(Cli::deleteProfileCron($this->strModuleId, $intProfileId, 'export.php')){
							Log::getInstance($this->strModuleId, 'crm')->add(Loc::getMessage('ACRIT_EXP_PROFILE_ONE_TIME_DELETE_SUCCESS'), $intProfileId);
						}
						else{
							Log::getInstance($this->strModuleId, 'crm')->add(Loc::getMessage('ACRIT_EXP_PROFILE_ONE_TIME_DELETE_ERROR'), $intProfileId);
						}
				...
				Log::getInstance($this->strModuleId, 'crm')->add(Loc::getMessage('ACRIT_EXP_PROFILE_LOCKED', array(
