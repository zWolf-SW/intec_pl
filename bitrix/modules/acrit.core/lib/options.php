...
class Options {
	...
	public function __construct($strModuleId, $arTabs, $arParams=null){
				...
				foreach($arTab['OPTIONS'] as $strGroup => $arGroup){
					foreach($arGroup['OPTIONS'] as $strOption => $arOption){
						if(is_callable($arOption['HEAD_DATA'])){
							ob_start();
							call_user_func_array($arOption['HEAD_DATA'], [$this, $arOption, $strOption]);
		...
		# Config
		$strSingleOption = strlen($arParams['SINGLE_OPTION']) ? $arParams['SINGLE_OPTION'] : null;
		$bSingleWithTable = $arParams['SINGLE_WITH_TABLE'] == 'Y';
		# Display
		$this->display($strSingleOption, $bSingleWithTable);
	}
	...
	public function prepareOptions($arOptions){
		...
		foreach($arOptions as $strFile){
			...
			if(is_file($strFile)){
				...
				if(is_array($arNewOptions['OPTIONS'])){
	...
	protected function handleAjax(){
		if($this->arGet['ajax_action'] == 'option_ajax'){
			$strOptionAjax = $this->arPost['option'];
			if(strlen($strOptionAjax)){
				foreach($this->arTabs as $strTab => $arTab){
					if(is_array($arTab['OPTIONS'])){
						foreach($arTab['OPTIONS'] as $arGroup){
							foreach($arGroup['OPTIONS'] as $strOption => $arOption) {
								if($strOption == $strOptionAjax){
									if(is_callable($arOption['CALLBACK_AJAX'])){
										$arJsonResult = Json::prepare();
										call_user_func_array($arOption['CALLBACK_AJAX'], [$this, $arOption, &$arJsonResult]);
					...
					foreach($arTab['OPTIONS'] as $arGroup){
						foreach($arGroup['OPTIONS'] as $strOption => $arOption) {
							...
							if(is_array($strValue)){
								$strSeparator = isset($arOption['VALUE_SEPARATOR']) ? $arOption['VALUE_SEPARATOR'] : ',';
	 ...
	 *	Display all
	 */
	public function display($strSingleOption=null, $bSingleWithTable=true){
		if(!is_null($strSingleOption)){
			foreach($this->arTabs as $strTab => $arTab){
				if(is_array($arTab['OPTIONS'])){
					foreach($arTab['OPTIONS'] as $strGroup => $arGroup){
						if(is_array($arGroup['OPTIONS'][$strSingleOption])){
							$this->displaySingleOption($arGroup['OPTIONS'][$strSingleOption], $bSingleWithTable);
						}
					}
				}
			}
