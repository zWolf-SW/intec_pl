...
class DiscountRecalculation {
	 ...
	 *	Get element prices
	 */
	public static function getElementPrices($intElementId){
		$arResult = [];
		$arPricesId = static::getCurrentPricesID();
		$arFilter = ['PRODUCT_ID' => $intElementId, 'CATALOG_GROUP_ID' => $arPricesId];
		$arSelect = ['ID', 'CATALOG_GROUP_ID', 'PRICE', 'CURRENCY'];
		$resPrices = \CPrice::getList([], $arFilter, false, false, $arSelect);
		while($arPrice = $resPrices->fetch()){
			$arResult[$arPrice['CATALOG_GROUP_ID']] = $arPrice;
		}
		return $arResult;
	}
	...
	public static function processElement($intElementID, $arPrices=null, $strSiteID=null){
		...
		if($arElement = $resElement->getNext(false, false)){
			...
			$arPropValues = array();
			$arCurrentPriceTypes = static::getCurrentPricesID();
			$arCurrentPriceValues = static::getElementPrices($intElementID);
			foreach($arPrices as $arPrice){
				$intPriceID = $arPrice['ID'];
				if(!in_array($intPriceID, $arCurrentPriceTypes)) {
					continue;
				}
				$arPrice = array(
					'ID' => $arCurrentPriceValues[$intPriceID]['ID'],
					'PRICE' => $arCurrentPriceValues[$intPriceID]['PRICE'],
					'CURRENCY' => $arCurrentPriceValues[$intPriceID]['CURRENCY'],
