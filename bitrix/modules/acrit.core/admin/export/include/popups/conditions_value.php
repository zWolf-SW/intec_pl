...
elseif($strIBlockType=='parent'){
	$intFieldIBlockID = $intIBlockParentID;
}
...
elseif($strCustomAction=='load_sections'){
	$resSections = Filter::searchSectionsByText($intFieldIBlockID, $strValueSearch); // $intIBlockID
	while($arSection = $resSections->GetNext()){
		$intIndex++;
		if($intIndex >= $intIndexMin && $intIndex <= $intIndexMax){
			$arJsonItems[] = array(
	...
	<?if(is_array($arCurrentLogic)):?>
		...
		<div class="acrit-exp-field-select-wrapper acrit-exp-select-wrapper" id="acrit-exp-field-select-wrapper" style="padding-top:0;">
			...
			<?elseif($strType=='S' && $strUserType=='_Currency'):?>
				...
				?>
			<?// TYPE = N:_ID_LIST ?>
			<?elseif($strType=='N' && $strUserType=='_ID_LIST' && in_array($strCurrentLogic, ['IN_LIST', 'NOT_IN_LIST'])):?>
			...
			<?else:?>
				<?if($strType=='S' && ($strUserType=='DateTime' || $strUserType=='Date')):?>
					<?if(in_array($strCurrentLogic, ['FOR_THE_LAST', 'NOT_FOR_THE_LAST', 'IN_FUTURE'])):?>
						<?
						$strDatetimeType = 'd';
