...
<table class="adm-list-table">
	<tbody>
		...
		<tr>
			<td width="40%" style="padding-top:11px; vertical-align:top;">
				<?=Helper::showHint(Loc::getMessage('ACRIT_EXP_TAB_OFFERS_GROUP_HINT'));?>
				<?=Loc::getMessage('ACRIT_EXP_TAB_OFFERS_GROUP');?>:
			</td>
			<td width="60%">
				<?
				$arOfferGroup = $arProfile['IBLOCKS'][$intIBlockID]['PARAMS']['OFFERS_GROUP'];
				if(!is_array($arOfferGroup)){
					$arOfferGroup = [];
				}
				#
				$arOfferProps = [];
				$resProps = \CIBlockProperty::getList(['NAME' => 'ASC'], ['IBLOCK_ID' => $intIBlockOffersID]);
				while($arProp = $resProps->fetch()){
					$arOfferProps[$arProp['ID']] = sprintf('%s [%d, %s]', $arProp['NAME'], $arProp['ID'], $arProp['CODE']);
				}
				?>
				<select name="iblockparams[<?=$intIBlockID;?>][OFFERS_GROUP][]" multiple size="8" style="min-width:400px">
					<?foreach($arOfferProps as $intOfferProp => $strOfferProp):?>
						<option value="<?=$intOfferProp;?>"<?if(in_array($intOfferProp, $arOfferGroup)):?> selected<?endif?>><?=$strOfferProp;?></option>
					<?endforeach?>
				</select>
			</td>
		</tr>
