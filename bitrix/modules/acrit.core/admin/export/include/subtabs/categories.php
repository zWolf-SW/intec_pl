	...
	<tbody>
		...
		<tr id="row_CATEGORIES_LIST">
			<td width="40%" valign="top">
				...
				<br/>
				<a href="#" data-role="sections_select2_mode"><?=Loc::getMessage('ACRIT_EXP_TAB_CATEGORIES_LIST_SELECT2');?></a>&nbsp;
			</td>
			<td width="60%">
				<div>
					...
					<select class="acrit-exp-categories" multiple="multiple" size="12" data-role="categories-list">
						<?foreach($arCategoriesAll as $arCategory):?>
							...
							<option value="<?=$arCategory['ID'];?>"<?if($bSelected):?>selected="selected"<?endif?>><?
								print str_repeat('.&nbsp;&nbsp;', max(0, $arCategory['DEPTH_LEVEL']-1));
		...
		<?if(is_object($obPlugin)):?>
			<?if($obPlugin->areCategoriesExport()):?>
				...
				<?if($obPlugin->hasCategoryList() && !$obPlugin->hideCategoriesUpdateButton() && ($obPlugin->areCategoriesExport() || $obPlugin->areCategoriesUpdate())):?>
