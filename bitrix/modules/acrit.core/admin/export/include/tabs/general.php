...
if(!$intProfileID){
	...
	$arProfile['GROUP_ID'] = intVal($arGet['group_id']);
	if(!empty($arSites)){
...
// Group ID
if($bGroupsAvailable){
	$obTabControl->BeginCustomField('PROFILE[GROUP_ID]', Loc::getMessage('ACRIT_EXP_FIELD_GROUP_ID'));
	?>
		<tr id="tr_GROUP_ID">
			<td>
			...
			<td>
				<div class="acrit-exp-select-wrapper" style="position:relative;">
					<select name="PROFILE[GROUP_ID]" id="field_GROUP_ID" class="acrit-exp-group-group-id">
						...
						<?foreach(Helper::call($strModuleId, 'ProfileGroup', 'getTree', []) as $arTreeItem):?>
							<option value="<?=$arTreeItem['ID'];?>"<?if($arTreeItem['ID'] == $arProfile['GROUP_ID']):?> selected<?endif?>><?
	...
	<?
	$obTabControl->EndCustomField('PROFILE[GROUP_ID]');
}
...
?>
	...
	<tr id="tr_PLUGIN_FORMAT"<?if(!is_array($arProfilePlugin['FORMATS'])):?> style="display:none"<?endif?>>
		...
		<td class="acrit-exp-select-wrapper">
			<div class="acrit-exp-select-wrapper" style="display:inline-block;position:relative;">
			...
			<div data-role="format_notice" style="display:inline-block;margin-left:10px;">
				<?if(is_object($obPlugin)):?>
					<?=$obPlugin::showFormatNotice();?>
				<?endif?>
			</div>
		</td>
	</tr>
