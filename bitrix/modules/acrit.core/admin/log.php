	...
	switch($strAjaxAction){
		...
		case 'clear':
			if($strAjaxAction == 'clear'){
				Log::getInstance($strModuleID, $strLogSubdir)->deleteLog($intProfileID);
			}
			$arJsonResult['ID'] = $intProfileID;
			$arJsonResult['Success'] = true;
			$arJsonResult['Log'] = '';
			$arJsonResult['LogFilename'] = Log::getInstance($strModuleID, $strLogSubdir)->getLogFilename($intProfileID, true);
			...
			$arJsonResult['LogSize'] = Log::getInstance($strModuleID, $strLogSubdir)->getLogSize($intProfileID, true);
			$arJsonResult['MaxSize'] = Log::getInstance($strModuleID, $strLogSubdir)->getMaxSize(true, true);
			break;
	}
...
if($arGet[Log::DOWNLOAD_PARAM] == Log::DOWNLOAD_PARAM_Y){
	Log::getInstance($strModuleID, $strLogSubdir)->downloadLog($intProfileID);
	die();
}
...
$APPLICATION->RestartBuffer();
Log::getInstance($strModuleID, $strLogSubdir)->showLog($intProfileID, true);
die();
