...
if($lAdmin->EditAction() && $strRight == 'W') {
	...
	foreach($FIELDS as $ID => $arFields) {
		...
		$DB->StartTransaction();
		$ID = IntVal($ID);
		$resProfile = Helper::call($strModuleId, 'OrdersProfiles', 'getList', [[
			'filter' => array(
				'ID' => $ID,
			),
		]]);
		if($arProfile = $resProfile->fetch()){
			if(!Helper::call($strModuleId, 'OrdersProfiles', 'update', [$ID, $arFields])) {
...
if($lAdmin->GroupAction() && $strRight == 'W') {
	...
	if ($strGroupTarget == 'selected') {
		$resProfiles = Helper::call($strModuleId, 'OrdersProfiles', 'getList', [
			[
				'filter' => $arFilter,
				'select' => array(
					'ID',
	...
	foreach ($arID as $ID) {
		...
		if ($arProfile = $resProfile->fetch()) {
			...
			switch ($strGroupAction) {
				case 'delete':
					$obResult = Helper::call($strModuleId, 'OrdersProfiles', 'delete', [$ID]);
				...
				case 'deactivate':
					...
					$obResult        = Helper::call($strModuleId, 'OrdersProfiles', 'update', [$ID, $arProfileFields]);
					$bSuccess        = $obResult->isSuccess();
					if ( ! $bSuccess) {
					...
					break;
				case 'unlock':
					$obResult = Helper::call($strModuleId, 'OrdersProfiles', 'unlock', [$ID]);
					$bSuccess = $obResult->isSuccess();
					if ( ! $bSuccess) {
...
if (!in_array($by, ['ID', 'ACTIVE', 'SORT', 'NAME', 'DESCRIPTION', 'DATE_CREATED', 'DATE_MODIFIED'])) {
	$by = 'NAME';
}
$arQuery = array(
//	'select' => array(
...
$resData = Helper::call($strModuleId, 'OrdersProfiles', 'getList', [$arQuery]);
...
//	$bIsOnCron = Cli::isProfileOnCron($strModuleId, $f_ID, 'export.php', null, true);
	// ID
	$obRow->AddViewField('ID', '<a href="' . $strModuleUnderscore . '_orders_edit.php?ID=' . $f_ID . '&lang=' . LANGUAGE_ID . '">' . $f_ID . '</a>');
	// ACTIVE
	$obRow->AddCheckField('ACTIVE', $f_ACTIVE);
	// NAME
	$obRow->AddInputField('NAME', array('SIZE' => '20'));
	$obRow->AddViewField('NAME', '<a href="' . $strModuleUnderscore . '_orders_edit.php?ID=' . $f_ID . '&lang=' . LANGUAGE_ID . '">' . $f_NAME . '</a>');
	// DESCRIPTION
	...
	$arActions[] = array(
		...
		'ACTION'  => $lAdmin->ActionRedirect($strModuleUnderscore . '_orders_edit.php?ID=' . $f_ID . '&lang=' . LANGUAGE_ID),
	);
//	$arActions[] = array(
//		'ICON'    => 'copy',
//		'DEFAULT' => false,
//		'TEXT'    => getMessage('ACRIT_EXP_CONTEXT_PROFILE_COPY'),
//		'ACTION'  => $lAdmin->ActionRedirect($strModuleUnderscore . '_orders_edit.php?ID=' . $f_ID . '&copy=Y&lang=' . LANGUAGE_ID),
//	);
	$arActions[] = array(
		'ICON'    => 'delete',
		'DEFAULT' => false,
...
$arContext         = array(
	array(
		...
		'LINK' => $strModuleUnderscore . '_orders_edit.php?lang=' . LANGUAGE_ID,
		'ICON' => 'btn_new',
	),
);
...
$arFilenames = [];
$arProfilesAll = Helper::call($strModuleId, 'OrdersProfiles', 'getProfiles', [[], [], false, false]);
...
if(!empty($arFilenames)){
	...
	foreach($arFilenames as $strFilename => $arProfilesId){
		foreach($arProfilesId as $key => $intProfileId){
			$arProfilesId[$key] = sprintf('<a href="/bitrix/admin/acrit_%s_orders_edit.php?ID=%d&lang=%s" target="_blank">%d</a>',
