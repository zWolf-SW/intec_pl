...
if ($intProfileID > 0) {
	...
	if(Helper::call($strModuleId, 'OrdersProfiles', 'getList', [$arQuery])->getSelectedRowsCount() == 0){
		LocalRedirect('/bitrix/admin/acrit_'.$strModuleCodeLower.'_orders_list.php?lang='.LANGUAGE_ID);
	}
...
if($arGet['delete'] == 'Y'){
	Helper::call($strModuleId, 'OrdersProfiles', 'delete', [$intProfileID]);
	LocalRedirect('/bitrix/admin/acrit_'.$strModuleCodeLower.'_orders_list.php?lang='.LANGUAGE_ID);
}
...
$arMenu[] = array(
	...
	'LINK' => 'acrit_'.$strModuleCodeLower.'_orders_list.php?lang='.LANGUAGE_ID,
	'ICON' => 'btn_list',
);
if($intProfileID) {
	$arActionsMenu = array();
	$arActionsMenu[] = array(
		...
		'LINK' => 'acrit_'.$strModuleCodeLower.'_orders_edit.php?lang='.LANGUAGE_ID,
		'ICON' => 'edit',
	);
	if(!$bCopy){
//		$arActionsMenu[] = array(
//			'TEXT'	=> Loc::getMessage('ACRIT_EXP_MENU_COPY'),
//			'LINK' => 'acrit_'.$strModuleCodeLower.'_orders_edit.php?ID='.$intProfileID.'&copy=Y&lang='.LANGUAGE_ID,
//			'ICON' => 'copy',
//		);
		$strDeleteUrl = 'acrit_'.$strModuleCodeLower.'_orders_edit.php?ID='.$intProfileID.'&delete=Y&lang='.LANGUAGE_ID;
		$arActionsMenu[] = array(
...
//			'SEPARATOR' => true,
//		);
//		$strBackupUrl = 'acrit_'.$strModuleCodeLower.'_orders_edit.php?ID='.$intProfileID.'&backup=Y&lang='.LANGUAGE_ID;
...
//file_put_contents(__DIR__.'/prof.txt', json_encode($arProfile), true);
// Save form on POST
$bSave = !!strlen($arPost['AcritExpProfile_active_tab']);
...
//	}
	...
	if ($intProfileID && $bCopy) {
		...
		if ($intNewProfileID) {
			...
			$obResult = Helper::call($strModuleId, 'OrdersProfiles', 'update', [$intProfileID, $arProfileFields]);
			$bCopySuccess = true;
			$strTabParam = $strAdminFormName.'_active_tab';
		}
	}
	elseif($intProfileID && !$bCopy) {
		...
		$obResult = Helper::call($strModuleId, 'OrdersProfiles', 'add', [$arProfileFields]);
		if ($obResult) {
	...
	if($bCopySuccess || $obResult->isSuccess()) {
	    // Add agent
        if ($intProfileID) {
	        \Acrit\Core\Orders\PeriodSync::setModuleId($strModuleId);
	        \Acrit\Core\Orders\PeriodSync::set($intProfileID);
        }
		// Redirect
		if($bApply) {
			...
			$strUrl = '/bitrix/admin/acrit_'.$strModuleCodeLower.'_orders_list.php?lang='.LANGUAGE_ID;
		}
...
if(strlen($strAjaxAction)){
	...
	switch($strAjaxAction){
		...
		case 'save_last_settings_tab':
			...
			#));
			if($strRight == 'W'){
				Helper::call($strModuleId, 'OrdersProfiles', 'update', [$intProfileID, ['LAST_SETTINGS_TAB' => $arGet['tab']]]);
			}
			break;
*/
...
//			$obResult = Helper::call($strModuleId, 'OrdersProfiles', 'unlock', [$intProfileID]);
//			#Profiles::clearSession($intProfileID);
//			Helper::call($strModuleId, 'OrdersProfiles', 'clearSession', [$intProfileID]);
...
//			break;
		...
		case 'get_profile_name_index':
			...
			$arProfiles = Helper::call($strModuleId, 'OrdersProfiles', 'getProfiles');
			...
			while(true){
				...
				if(!empty($arExistNames) && !in_array($strNameNew, $arExistNames)){
					break;
				}
			}
...
//			if(Helper::call($strModuleId, 'OrdersProfiles', 'isLocked', [$arProfile])){
//				print '<div style="margin-top:15px;"></div>';
...
//		);
        $arTabs[] = array(
            'DIV' => 'special',
            'TAB' => Loc::getMessage('ACRIT_EXP_TAB_SPECIAL_NAME'),
            'TITLE' => Loc::getMessage('ACRIT_EXP_TAB_SPECIAL_DESC'),
            'SORT' => 70,
            'FILE' => __DIR__.$strTabsDir.'/special.php',
        );
        if ($obPlugin->feedBack()) {
            $arTabs[] = array(
                'DIV' => 'orders',
                'TAB' => Loc::getMessage('ACRIT_EXP_TAB_ORDERS_NAME'),
                'TITLE' => Loc::getMessage('ACRIT_EXP_TAB_ORDERS_DESC'),
                'SORT' => 70,
                'FILE' => __DIR__ . $strTabsDir . '/orders.php',
            );
        }
        $arTabs[] = array(
            'DIV' => 'log',
...
foreach($arTabs as $key => $arTab) {
	if(!empty($arSystemTabs) && !in_array($arTab['DIV'], $arSystemTabs)){
...
# XDebug notifier
Helper::call($strModuleId, 'OrdersProfiles', 'checkXDebug');
...
$obTabControl->Buttons(array(
	'disabled' => false,
	'back_url' => 'acrit_'.$strModuleCodeLower.'_orders_list.php?lang='.LANGUAGE_ID,
));
$obTabControl->Show();
