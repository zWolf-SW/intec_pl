...
function acritCoreLogAjax(ajaxAction, moduleId, profileId, subdir, callbackSuccess, callbackError){
	BX.showWait();
	if(window.acritCoreLogAjaxQuery){
		window.acritCoreLogAjaxQuery.abort();
	}
	window.acritCoreLogAjaxQuery = $.ajax({
		url: '/bitrix/admin/acrit_core_log.php?action='+ajaxAction+'&module='+moduleId+'&profile='+profileId+'&subdir='+subdir,
		type: 'POST',
		data: {},
		datatype: 'json',
...
$(document).delegate('[data-role="log-wrapper"] [data-role^="log-"][data-ajax="Y"]', 'click', function(e){
	...
	var action = $(this).attr('data-role').replace(/^log-(.*?)/g, '$1'),
		...
		subdir = wrapper.attr('data-subdir'),
	...
	if(maySend) {
		acritCoreLogAjax(action, moduleId, profileId, subdir, function(jsonResult, textStatus, jqXHR){
