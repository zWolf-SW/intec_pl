...
function AcritExpConditionsPopupCallbackClickEntity(e, sender, options){
	...
	var divFilter = $(sender).closest('[data-role="filter"]'),
		...
		iblockTypeRaw = $(sender).closest('[data-role="item"]').attr('data-iblock-type'),
		iblockType = iblockTypeRaw == 'offers' ? 'offers' : (iblockTypeRaw == 'parent' ? 'parent' : 'main'),
...
AcritExpConditionsPopup.FilterFields = function(){
	...
	if(searchText=='') {
		...
		$('option', fieldList).not('[data-role="entity-select-item-not-found"]').acritExpToggleListItem(true);
		fieldList.children('option').filter('[data-role="entity-select-item-not-found"]').acritExpToggleListItem(false);
	}
	else {
		var found = false;
		fieldList.children('option').not('[data-role="entity-select-item-not-found"]').acritExpToggleListItem(true);
		if(grouped) {
			$('optgroup', fieldList).acritExpToggleListItem(false);
		}
		$('option', fieldList).not('[data-role="entity-select-item-not-found"]').acritExpToggleListItem(false).each(function(){
			var search = $(this).val().toLowerCase() + ' ' + $.trim($(this).text()).toLowerCase();
			var matched = search.indexOf(searchText) > -1;
			if(matched){
				$(this).acritExpToggleListItem(true);
				if(grouped){
					$(this).closest('optgroup').acritExpToggleListItem(true);
				}
				found = true;
			}
		});
...
$(document).delegate('.acrit-exp-field-select-table select[data-role="entity-select-type"]', 'change', function(){
	var table = $(this).closest('.acrit-exp-field-select-table'),
		...
		listParent = $('[data-role="entity-select-item"][data-type="parent"]', table);
	listMain.hide();
	listOffers.hide();
	listParent.hide();
	if($(this).val()=='offers'){
