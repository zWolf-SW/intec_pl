...
namespace Acrit\Core\Orders\Plugins\PromUaHelpers;
...
class Orders extends Request {
	...
	public function checkConnection($token, &$message) {
		...
		if (!isset($res['orders'])) {
			$message = Loc::getMessage('ACRIT_CRM_PLUGIN_PROMUA_CHECK_ERROR') . $res;
		}
		else {
			$message = Loc::getMessage('ACRIT_CRM_PLUGIN_PROMUA_CHECK_SUCCESS');
			$result = true;
		}
		return $result;
	}
