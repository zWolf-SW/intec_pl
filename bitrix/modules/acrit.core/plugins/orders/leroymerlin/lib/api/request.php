...
class Request {
	const URL = 'https://api.leroymerlin.ru/marketplace';
...
//	}
	 ...
	 *	Request wrapper
	 */
	public function request($method, $data, $user_key, $reqmethod, $jwt){
		$apiKey = $this->strApiKey;
		if ($user_key) {
			$apiKey = $user_key;
		}
		$result = $this->execute($method, $data, $apiKey, $reqmethod, $jwt );
//        file_put_contents(__DIR__.'/result1.txt', var_export($result, true) );
	 ...
	 */
    public function execute($strCommand, $data, $token, $reqmethod, $jwt  ){
        $bSkipErrors = false;
        ...
        $httpClient->query($reqmethod, $ext_url , $data);
