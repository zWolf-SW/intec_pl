...
class Avito extends Plugin {
	...
	public function getFields($intProfileID, $intIBlockID, $bAdmin = false) {
			...
			$arResult[] = new Field(array(
					'CODE' => 'AUCTION_PRICE',
					'DISPLAY_CODE' => 'AuctionPrice',
					'NAME' => static::getMessage('FIELD_AUCTION_PRICE_NAME'),
					'SORT' => 152,
					'DESCRIPTION' => static::getMessage('FIELD_AUCTION_PRICE_DESC'),
			));
			$arResult[] = new Field(array(
					'CODE' => 'AUCTION_PRICE_LAST_DATE',
					'DISPLAY_CODE' => 'AuctionPriceLastDate',
					'NAME' => static::getMessage('FIELD_AUCTION_PRICE_LAST_DATE_NAME'),
					'SORT' => 154,
					'DESCRIPTION' => static::getMessage('FIELD_AUCTION_PRICE_LAST_DATE_DESC'),
			));
			...
			$arResult[] = new Field(array(
					'CODE' => 'IMAGE_URLS',
					'DISPLAY_CODE' => 'ImageUrls',
					'NAME' => static::getMessage('FIELD_IMAGE_URLS_NAME'),
					'SORT' => 322,
					'DESCRIPTION' => static::getMessage('FIELD_IMAGE_URLS_DESC'),
					'DEFAULT_VALUE' => array(),
					'PARAMS' => array(
							'MULTIPLE' => 'join',
							'MULTIPLE_separator' => 'vertical_line',
					),
			));
			$arResult[] = new Field(array(
					'CODE' => 'IMAGE_NAMES',
					'DISPLAY_CODE' => 'ImageNames',
					'NAME' => static::getMessage('FIELD_IMAGE_NAMES_NAME'),
					'SORT' => 324,
					'DESCRIPTION' => static::getMessage('FIELD_IMAGE_NAMES_DESC'),
					'DEFAULT_VALUE' => array(),
					'PARAMS' => array(
							'MULTIPLE' => 'join',
							'MULTIPLE_separator' => 'vertical_line',
	...
	protected function stepExport_writeXmlOffers($intProfileID, $arData) {
		...
		$bStepByStep = $arData['PROFILE']['PARAMS']['STEP_BY_STEP'] == 'Y';
		if($bStepByStep){
			$intCountPerStep = IntVal($arData['PROFILE']['PARAMS']['STEP_BY_STEP_COUNT']);
			if($intCountPerStep < 1) {
				$bStepByStep = false;
			}
			$arLastExportedItem = $this->getLastExportedItem($this->arProfile['ID']);
			$intCountExportedAtStep = 0;
		}
		$intCountExported = 0;
		$bBreakedByStepCount = false;
		$bFoundNext = false;
		$strLastExportedSort = null;
		$strLastExportedElementId = null;
		#
		$intOffset = 0;
		$intLimit = 5000;
		...
		while(true) {
			...
			while($arItem = $resItems->fetch()) {
				# Write XML
				$strXml .= rtrim(Xml::addOffset($arItem['DATA'], 1)) . "\n";
				$intCountBulk++;
				$intCountExported++;
				# Step-by-step: Place last exported to var
				if($bStepByStep){
					$strLastExportedSort = $arItem['SORT'];
					$strLastExportedElementId = $arItem['ELEMENT_ID'];
				}
				# Step-by-step: Check resume
				if($bStepByStep && !$bFoundNext){
					if(!$arLastExportedItem){
						$bFoundNext = true;
					}
					elseif(strcmp(toLower($arItem['SORT']), toLower($arLastExportedItem['SORT'])) > 0){
						$bFoundNext = true;
					}
					elseif($arItem['SORT'] == $arLastExportedItem['SORT']){
						if($arItem['ELEMENT_ID'] > $arLastExportedItem['ELEMENT_ID']){
							$bFoundNext = true;
						}
					}
					// break;
				...
				if($bStepByStep && $bFoundNext){
					$intCountExportedAtStep++;
					if($intCountExportedAtStep >= $intCountPerStep){
			...
			$arData['SESSION']['EXPORT']['INDEX'] += $intCountExported;
			file_put_contents($strFile, $strXml, FILE_APPEND);
			# Check finish
			...
			if($bStepByStep){
				...
				$this->setLastExportedItem($this->arProfile['ID'], array_merge($arLastExportedItem, array(
					...
					'FINISHED' => $bFinished ? 'Y' : 'N',
				)), intVal($arLastExportedItem['STEP']) + ($intCountExportedAtStep ? 1 : 0));
				if($bBreakedByStepCount) {
					break;
				}
			}
			# Finishing..
	...
	protected static function outdatedGetNameSuffix(){
		return static::getMessage('OUTDATED_SUFFIX');
	}

	protected static function outdatedGetMessage(){
		return Helper::showError(static::getMessage('OUTDATED_MESSAGE'));
	}
}
?>