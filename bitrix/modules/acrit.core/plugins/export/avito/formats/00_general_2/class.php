...
class AvitoGeneral2 extends UniversalPlugin {
	
	const DATE_UPDATED = '2023-09-05';
	...
	public function getFieldCode($arField){
		...
		if($arField['type'] == 'checkbox' && isset($arField['values']) && is_array($arField['values'])){
	...
	protected function onUpBuildXml(&$arXmlTags, &$arXmlAttr, &$strXmlItem, &$arElement, &$arFields, &$arElementSections, &$arDataMore){
		...
		# Use just attributes from target category
		if(Helper::strlen($strCategoryId = $this->parseCategoryId($strCategoryName))){
			if(is_array($arAttributes[$strCategoryId])){
