      ...
      $arResult[] = new Field(array(
          ...
          'DESCRIPTION' => static::getMessage('FIELD_TYPE_DESC'),
			...
			$arResult[] = new Field(array(
				'CODE' => 'NDS',
				'DISPLAY_CODE' => 'NDS',
				'NAME' => static::getMessage('FIELD_NDS_NAME'),
				'SORT' => 5000,
				'DESCRIPTION' => static::getMessage('FIELD_NDS_DESC'),
			));
      #
      ...
      if (!Helper::isEmpty($arFields['CONDITION']))
         $arXmlTags['Condition'] = Xml::addTag($arFields['CONDITION']);
			...
			if(!Helper::isEmpty($arFields['MATERIAL']))
				$arXmlTags['Material'] = Xml::addTag($arFields['MATERIAL']);
			if(!Helper::isEmpty($arFields['GENDER']))
				$arXmlTags['Gender'] = Xml::addTag($arFields['GENDER']);
			if(!Helper::isEmpty($arFields['NDS']))
				$arXmlTags['NDS'] = Xml::addTag($arFields['NDS']);
      # build XML
      $arXml = array(
          'Ad' => array(
              '#' => $arXmlTags,
