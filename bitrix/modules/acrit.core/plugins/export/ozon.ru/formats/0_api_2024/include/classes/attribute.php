...
class AttributeTable extends Entity\DataManager {
	...
	public static function getTableName(){
		return 'acrit_ozon_2024_attribute';
	}
	...
	public static function getMap() {
		...
		return array(
			...
			'TYPE_ID' => new Entity\IntegerField('TYPE_ID', array(
				'title' => Helper::getMessage($strLang.'TYPE_ID'),
			)),
			...
			foreach($arFilter as $strField => $strValue){
				...
				elseif(is_array($strValue)){
					foreach($strValue as $key => $value){
						$value = \Bitrix\Main\Application::getConnection()->getSqlHelper()->forSql($value);
					}
					$strValue = implode(', ', array_map(function($item){return sprintf("'%s'", $item);}, $strValue));
					$strSql .= " AND (`{$strField}` IN ({$strValue}))";
				}
				else{
					$strValue = \Bitrix\Main\Application::getConnection()->getSqlHelper()->forSql($strValue);
