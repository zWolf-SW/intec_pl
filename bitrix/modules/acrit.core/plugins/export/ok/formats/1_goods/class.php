...
//			$arApiFields['category_id'] = static::getCategoryRedef($arProfile, $arFields, reset($arElementSections));
		...
		$strRedefPhoto = static::getCategoryRedefPhoto($arProfile, (int)$category);
		if ($strRedefPhoto)
			$arApiFields['category_photo'] = $strRedefPhoto;
		elseif(!Helper::isEmpty($arFields['CATEGORY_PHOTO']))
	...
	public function stepExport($intProfileID, $arData) {
		...
		//Log::getInstance($this->strModuleId)->add('$arOKCatalogsIDs: '.print_r($arOKCatalogsIDs, true), $intProfileID);
		...
		Log::getInstance($this->strModuleId)->add('intExportCount: ' . $intExportCount, $intProfileID, true);
		$intOffset = 0;
		$intIndex = 0;
		...
		while ($intIndex < $intExportCount) {
			...
			// Count result
			Log::getInstance($this->strModuleId)->add('intExportedCount: '.$intExportedCount, $intProfileID);
	...
	function addMarketItem($arItem, $product_id){
		...
		if( $product_id ){
			...
			$arResponse = OkSDK::makeRequest( "market.edit", $arPostParams, $requestStr, $responseStr);
			$this->addToLog('(market.edit) request: ' . $requestStr, true);
		}
		else{
			$arResponse = OkSDK::makeRequest( "market.add", $arPostParams, $requestStr, $responseStr);
			$this->addToLog('(market.add) request: ' . $requestStr, true);
		}
		$this->addToLog('(market.add/market.edit) response: ' . $responseStr, true);
		//$this->addToLog('(market.add/market.edit) response: ' . print_r($arResponse, true), true);

		if( isset( $arResponse["error_code"] ) ){
			$this->addToLog('Error item add/edit [' . $arResponse["error_code"] . ']: ' . $arResponse["error_msg"]);
		}
	...
	protected function getCategoryRedef($arProfile, $arFields, $intCategId, $strValueDef='') {
		$intProfileID = $arProfile['ID'];
		$arCategoryRedefinitions = $this->getCategoriesRedefinitions($intProfileID);
		if (intval($intCategId) && isset($arCategoryRedefinitions[$intCategId]) && $arCategoryRedefinitions[$intCategId]['SECTION_NAME']) {
			$strCategName = $arCategoryRedefinitions[$intCategId]['SECTION_NAME'];
		}
		elseif ($strValueDef) {
			$strCategName = $strValueDef;
		}
		else {
			$strCategName = false;
		}
	...
	protected function getCategoryRedefPhoto($arProfile, $intCategId) {
		$strCategPhoto = false;
		$intProfileID = $arProfile['ID'];
		$arCategoryRedefinitions = $this->getCategoriesRedefinitions($intProfileID);
		if (intval($intCategId) && isset($arCategoryRedefinitions[$intCategId]) && $arCategoryRedefinitions[$intCategId]['PICTURE']) {
			$strCategPhoto = $arCategoryRedefinitions[$intCategId]['PICTURE'];
			// Format picture
			if (strpos($strCategPhoto, 'http') === false) {
				$strCategPhoto = 'https://' . $_SERVER['SERVER_NAME'] . $strCategPhoto;
			}
		}
		return $strCategPhoto;
	}
	...
	function uploadPhotos( $arPhotos, $aid = false ){
		...
		$arGetUploadUrlParams = array(
			"gid" => $this->strGroupId,
			"count" => count( (is_array($arPhotos)?$arPhotos:[]) ),
		);
	...
	function getMarketItems($assoc=true){
		...
		if (is_array($arGetMarketItems["short_products"]) && count($arGetMarketItems["short_products"]) > 0) {
	...
	public function checkConnection($arInpParams, &$message) {
		...
		OkSDK::init(
			$arInpParams['APP_ID'],
			$arInpParams['APP_PUB_KEY'],
			$arInpParams['APP_SEC_KEY'],
			$arInpParams['ACCESS_TOKEN']
		);
		$arPostParams = array(
			"gid" => $arInpParams['GROUP_ID'],
			"tab" => 'PRODUCTS',
			"count" => 1,
		);
		$arResponse = OkSDK::makeRequest( "market.getProducts", $arPostParams );
		if (isset($arResponse["error_code"])) {
			$message = static::getMessage('SETTINGS_ACCESS_TOKEN_CHECK_ERROR') . $arResponse["error_msg"] . ' [' . $arResponse["error_code"] . ']';
		}
		else {
...
//		}
	...
	protected function onGetFileOpenLink(&$strFile, &$strTitle, $bSingle=false){
		return $this->getExtFileOpenLink($this->getExportFileName(),
