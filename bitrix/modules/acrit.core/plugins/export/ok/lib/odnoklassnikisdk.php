...
class OdnoklassnikiSDK{
	...
	public static function makeRequest($methodName, $parameters = null, &$requestStr='', &$responseStr=''){
		...
		$responseStr = HttpRequest::post(self::$API_REQUSET_ADDRESS, [
				'CONTENT' => $requestStr,
			]
		);
		try {
			$arRes = Json::decode($responseStr);
		}
		catch(\Exception $e){}
		return $arRes;
	}
