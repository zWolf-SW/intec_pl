...
if (!window.aliLocApiInitialized) {

	$(document).delegate('input[data-role="log-tasks-refresh"]', 'click', function(e, params) {
		if (typeof params === 'undefined') {
			params = {}
		}
		params.status = $('[data-role="log-tasks-filter-status"]').val();
	...
	$(document).delegate('[data-role="log-tasks-filter-status"]', 'change', function(e, params) {
		acritExpAjax(['plugin_ajax_action', 'refresh_tasks_list'], {
			'status': $(this).val()
		}, function(JsonResult, textStatus, jqXHR){
			$('#tr_LOG_CUSTOM > td').html(JsonResult.HTML);
			acritExpHandleAjaxError(jqXHR, false);
	...
	$(document).delegate('a[data-role="log-tasks-item-update-status"]', 'click', function(e) {
		let
			...
			btn_link = $('a[data-role="log-tasks-item-update-status"]', row);
		row.addClass('tasks-task-loading');
		acritExpAjax(['plugin_ajax_action', 'update_task_status'], data, function(JsonResult, textStatus, jqXHR) {
			row.removeClass('tasks-task-loading');
			row.find('[data-role="log-tasks-item-status"]').html(JsonResult.HTML.status);
			row.find('[data-role="log-tasks-item-errors"]').html(JsonResult.HTML.errors);
			acritExpHandleAjaxError(jqXHR, false);
		}, function(jqXHR) {
			acritExpHandleAjaxError(jqXHR, true);
		}, false);
	...
	window.aliLocApiInitialized = true;
}
