if (!window.lamodaNewApiInitialized) {
	
	function acritExpLamodaNewApiCatAttrUpdateEnableControls(enabled){
		let
	...
	function acritExpLamodaNewApiCatAttrUpdateExecute(start, force, justAttr){
		let
		...
		acritExpLamodaNewApiCatAttrUpdateEnableControls(false);
		window.acritExpLamodaNewApiAjaxUpdateAttr = acritExpAjax(action, data, function (arJsonResult, textStatus, jqXHR) {
			if(arJsonResult.Continue){
				acritExpLamodaNewApiCatAttrUpdateExecute(false);
			}
			else{
				acritExpLamodaNewApiCatAttrUpdateEnableControls(true);
			}
	...
	function acritExpLamodaNewApiCatAttrUpdateStop(){
		acritExpLamodaNewApiCatAttrUpdateEnableControls(true);
		if(window.acritExpLamodaNewApiAjaxUpdateAttr){
			window.acritExpLamodaNewApiAjaxUpdateAttr.abort();
		}
	}
	
	$(document).delegate('input[data-role="categories-update-attributes-start"]', 'click', function(e) {
		acritExpLamodaNewApiCatAttrUpdateExecute(true, e.ctrlKey, e.shiftKey);
	});
	
	$(document).delegate('input[data-role="categories-update-attributes-stop"]', 'click', function(e) {
		acritExpLamodaNewApiCatAttrUpdateStop();
	});
	
	$(document).delegate('input[data-role="acrit_exp_lamoda_access_check"]', 'click', function(e){
		e.preventDefault();
		let
			clientId = $('input[data-role="acrit_exp_lamoda_company_id"]').val(),
			apiKey = $('input[data-role="acrit_exp_lamoda_api_key"]').val(),
	...
	$(document).delegate('input[data-role="acrit_exp_lamoda_new_access_check"]', 'click', function(e){
		e.preventDefault();
		let
			clientId = $('input[data-role="acrit_exp_lamoda_new_client_id"]').val(),
			apiKey = $('input[data-role="acrit_exp_lamoda_new_api_key"]').val(),
			data = {client_id: clientId, api_key: apiKey};
		if(clientId.length && apiKey){
			acritExpAjax(['plugin_ajax_action', 'check_access'], data, function(JsonResult, textStatus, jqXHR){
				if(JsonResult.Message){
	...
	// Export stocks
	$(document).delegate('input[data-role="acrit_exp_lamoda_export_stocks"]', 'change', function(e){
		$('div[data-role="acrit_exp_lamoda_stores_wrapper"]').toggle($(this).prop('checked'));
	});

	// Add store
	$(document).delegate('input[data-role="acrit_exp_lamoda_store_add"]', 'click', function(e){
		let
			items = $('div[data-role="acrit_exp_lamoda_stores_list"]'),
	...
	// Add stores (auto)
	$(document).delegate('input[data-role="acrit_exp_lamoda_store_add_auto"]', 'click', function(e){
		acritExpAjax(['plugin_ajax_action', 'load_stores'], {}, function(JsonResult, textStatus, jqXHR){
			if(typeof JsonResult.Stores == 'object' && Object.keys(JsonResult.Stores).length){
				// Remove exist data
				$('input[data-role="acrit_exp_lamoda_store_delete"]:visible').trigger('click', {force:true});
				$('div[data-role="acrit_exp_lamoda_store"] input[type="text"]').val('');
				// Add new
				var first = true;
				for(var id in JsonResult.Stores){
					if(!first){
						$('input[data-role="acrit_exp_lamoda_store_add"]').trigger('click');
					}
					$('div[data-role="acrit_exp_lamoda_store"]').last().each(function(){
						$('input[data-role="acrit_exp_lamoda_store_id"]', this).val(id);
						$('input[data-role="acrit_exp_lamoda_store_name"]', this).val(JsonResult.Stores[id]);
					});
					first = false;
				}
			}
	...
	// Delete store
	$(document).delegate('input[data-role="acrit_exp_lamoda_store_delete"]', 'click', function(e, data){
		data = typeof data == 'object' ? data : {};
		if(data.force || confirm($(this).attr('data-confirm'))){
			$(this).closest('[data-role="acrit_exp_lamoda_store"]').remove();
		}
	});
	
	function acritExpLamodaNewApiTriggers(){
		$('#checkbox_CATEGORIES_ALTERNATIVE').trigger('change');
		$('input[data-role="acrit_exp_lamoda_export_stocks"]').trigger('change');		
	}

	window.lamodaNewApiInitialized = true;
}

// On load
setTimeout(function(){
	acritExpLamodaNewApiTriggers();
}, 500);
