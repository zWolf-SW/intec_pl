...
class BlizkoRu extends Plugin {
	...
	protected function stepExport_writeXmlPromos($intProfileID, $arData) {
		...
		if(\Bitrix\Main\Loader::includeModule('sale')){
			if ($bExportPromoCodes && is_array($arData['PROFILE']['PARAMS']['YANDEX_MARKET_EXPORT_PROMOCODES_FIELDS']) && count($arData['PROFILE']['PARAMS']['YANDEX_MARKET_EXPORT_PROMOCODES_FIELDS'])) {
		...
		if ($bExportSpecialPrice || $bExportNPlusM) {
			...
			if (is_array($arNPlusM) && count($arNPlusM)) {
		...
		if (is_array($arGiftIDsInOffers) && !empty($arGiftIDsInOffers)) {
