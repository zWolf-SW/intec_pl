if (!window.kaspiNewApiInitialized) {
	
	function acritExpKaspiNewApiCatAttrUpdateEnableControls(enabled){
		let
	...
	function acritExpKaspiNewApiCatAttrUpdateExecute(start, force, justAttr){
		let
		...
		acritExpKaspiNewApiCatAttrUpdateEnableControls(false);
		window.acritExpKaspiNewApiAjaxUpdateAttr = acritExpAjax(action, data, function (arJsonResult, textStatus, jqXHR) {
			if(arJsonResult.Continue){
				acritExpKaspiNewApiCatAttrUpdateExecute(false);
			}
			else{
				acritExpKaspiNewApiCatAttrUpdateEnableControls(true);
			}
	...
	function acritExpKaspiNewApiCatAttrUpdateStop(){
		acritExpKaspiNewApiCatAttrUpdateEnableControls(true);
		if(window.acritExpKaspiNewApiAjaxUpdateAttr){
			window.acritExpKaspiNewApiAjaxUpdateAttr.abort();
		}
	}
	
	$(document).delegate('input[data-role="categories-update-attributes-start"]', 'click', function(e) {
		acritExpKaspiNewApiCatAttrUpdateExecute(true, e.ctrlKey, e.shiftKey);
	});
	
	$(document).delegate('input[data-role="categories-update-attributes-stop"]', 'click', function(e) {
		acritExpKaspiNewApiCatAttrUpdateStop();
	});
	...
	$(document).delegate('input[data-role="acrit_exp_kaspi_new_access_check"]', 'click', function(e){
		e.preventDefault();
		let
			clientId = $('input[data-role="acrit_exp_kaspi_new_client_id"]').val(),
			apiKey = $('input[data-role="acrit_exp_kaspi_new_api_key"]').val(),
	...
	// Export stocks
	$(document).delegate('input[data-role="acrit_exp_kaspi_export_stocks"]', 'change', function(e){
		$('div[data-role="acrit_exp_kaspi_stores_wrapper"]').toggle($(this).prop('checked'));
	});

	// Add store
	$(document).delegate('input[data-role="acrit_exp_kaspi_store_add"]', 'click', function(e){
		let
			items = $('div[data-role="acrit_exp_kaspi_stores_list"]'),
	...
	// Add stores (auto)
	$(document).delegate('input[data-role="acrit_exp_kaspi_store_add_auto"]', 'click', function(e){
		acritExpAjax(['plugin_ajax_action', 'load_stores'], {}, function(JsonResult, textStatus, jqXHR){
			if(typeof JsonResult.Stores == 'object' && Object.keys(JsonResult.Stores).length){
				// Remove exist data
				$('input[data-role="acrit_exp_kaspi_store_delete"]:visible').trigger('click', {force:true});
				$('div[data-role="acrit_exp_kaspi_store"] input[type="text"]').val('');
				// Add new
				var first = true;
				for(var id in JsonResult.Stores){
					if(!first){
						$('input[data-role="acrit_exp_kaspi_store_add"]').trigger('click');
					}
					$('div[data-role="acrit_exp_kaspi_store"]').last().each(function(){
						$('input[data-role="acrit_exp_kaspi_store_id"]', this).val(id);
						$('input[data-role="acrit_exp_kaspi_store_name"]', this).val(JsonResult.Stores[id]);
					});
					first = false;
				}
			}
	...
	// Delete store
	$(document).delegate('input[data-role="acrit_exp_kaspi_store_delete"]', 'click', function(e, data){
		data = typeof data == 'object' ? data : {};
		if(data.force || confirm($(this).attr('data-confirm'))){
			$(this).closest('[data-role="acrit_exp_kaspi_store"]').remove();
		}
	});
	
	function acritExpKaspiNewApiTriggers(){
		$('#checkbox_CATEGORIES_ALTERNATIVE').trigger('change');
		$('input[data-role="acrit_exp_kaspi_export_stocks"]').trigger('change');		
	}

	window.kaspiNewApiInitialized = true;
}

// On load
setTimeout(function(){
	acritExpKaspiNewApiTriggers();
}, 500);
