<?
/**
 * Acrit Core: Wildberries history items
 * @documentation https://suppliers.wildberries.ru/remote-wh-site/api-content.html
 */

namespace Acrit\Core\Export\Plugins\WildberriesHelpers;

use
	\Acrit\Core\Helper,
	\Bitrix\Main\Entity;

Helper::loadMessages(__FILE__);

class HistoryTable extends Entity\DataManager {
	
	/**
	 * Returns DB table name for entity.
	 *
	 * @return string
	 */
	public static function getTableName(){
		return 'acrit_wb_history';
	}
	
	/**
	 * Returns entity map definition.
	 *
	 * @return array
	 */
	public static function getMap() {
		\Acrit\Core\Export\Exporter::getLangPrefix(realpath(__DIR__.'/../../../class.php'), $strLang, $strHead, 
			$strName, $strHint);
		return array(
			'ID' => new Entity\IntegerField('ID', array(
				'primary' => true,
				'autocomplete' => true,
				'title' => Helper::getMessage($strLang.'ID'),
			)),
			'MODULE_ID' => new Entity\IntegerField('MODULE_ID', array(
				'title' => Helper::getMessage($strLang.'MODULE_ID'),
			)),
			'PROFILE_ID' => new Entity\IntegerField('PROFILE_ID', array(
				'title' => Helper::getMessage($strLang.'PROFILE_ID'),
			)),
			'TASK_ID' => new Entity\IntegerField('TASK_ID', array(
				'title' => Helper::getMessage($strLang.'TASK_ID'),
			)),
			'REQUEST_ID' => new Entity\StringField('REQUEST_ID', array(
				'title' => Helper::getMessage($strLang.'REQUEST_ID'),
			)),
			'SUPPLIER_VENDOR_CODE' => new Entity\StringField('SUPPLIER_VENDOR_CODE', array(
				'title' => Helper::getMessage($strLang.'SUPPLIER_VENDOR_CODE'),
			)),
			'CARD_ID' => new Entity\StringField('CARD_ID', array(
				'title' => Helper::getMessage($strLang.'CARD_ID'),
			)),
			'IMT_ID' => new Entity\IntegerField('IMT_ID', array(
				'title' => Helper::getMessage($strLang.'IMT_ID'),
			)),
			'NOMENCLATURE_ID' => new Entity\StringField('NOMENCLATURE_ID', array(
				'title' => Helper::getMessage($strLang.'NOMENCLATURE_ID'),
			)),
			'NM_ID' => new Entity\IntegerField('NM_ID', array(
				'title' => Helper::getMessage($strLang.'NM_ID'),
			)),
			'VENDOR_CODE' => new Entity\StringField('VENDOR_CODE', array(
				'title' => Helper::getMessage($strLang.'VENDOR_CODE'),
			)),
			'ELEMENT_ID' => new Entity\IntegerField('ELEMENT_ID', array(
				'title' => Helper::getMessage($strLang.'ELEMENT_ID'),
			)),
			'STOCK_VALUE' => new Entity\IntegerField('STOCK_VALUE', array(
				'title' => Helper::getMessage($strLang.'STOCK_VALUE'),
			)),
			'JSON' => new Entity\TextField('JSON', array(
				'title' => Helper::getMessage($strLang.'JSON'),
			)),
			'RESPONSE' => new Entity\TextField('RESPONSE', array(
				'title' => Helper::getMessage($strLang.'RESPONSE'),
			)),
			'SUCCESS' => new Entity\StringField('SUCCESS', array(
				'title' => Helper::getMessage($strLang.'SUCCESS'),
			)),
			'CHRT_ID' => new Entity\StringField('CHRT_ID', array(
				'title' => Helper::getMessage($strLang.'CHRT_ID'),
			)),
			'BARCODE' => new Entity\StringField('BARCODE', array(
				'title' => Helper::getMessage($strLang.'BARCODE'),
			)),
			'STOCK_UPDATED' => new Entity\StringField('STOCK_UPDATED', array(
				'title' => Helper::getMessage($strLang.'STOCK_UPDATED'),
			)),
			'STOCK_ERRORS' => new Entity\StringField('STOCK_ERRORS', array(
				'title' => Helper::getMessage($strLang.'STOCK_ERRORS'),
			)),
			'STATUS' => new Entity\StringField('STATUS', array(
				'title' => Helper::getMessage($strLang.'STATUS'),
			)),
			'STATUS_DATETIME' => new Entity\DatetimeField('STATUS_DATETIME', array(
				'title' => Helper::getMessage($strLang.'STATUS_DATETIME'),
			)),
			'SESSION_ID' => new Entity\StringField('SESSION_ID', array(
				'title' => Helper::getMessage($strLang.'SESSION_ID'),
			)),
			'TIMESTAMP_X' => new Entity\DatetimeField('TIMESTAMP_X', array(
				'title' => Helper::getMessage($strLang.'TIMESTAMP_X'),
			)),
		);
	}
	
	/**
	 * Delete by filter
	 *
	 * @return array
	 */
	public static function deleteByFilter($arFilter=null) {
		$strTable = static::getTableName();
		$strSql = "DELETE FROM `{$strTable}` WHERE 1=1";
		if(is_array($arFilter)){
			foreach($arFilter as $strField => $strValue){
				$strEqual = '=';
				if(preg_match('#^(.*?)([A-z0-9_]+)(.*?)$#', $strField, $arMatch)){
					$strField = $arMatch[2];
					if($arMatch[1] == '!'){
						$strEqual = '!=';
					}
				}
				$strField = \Bitrix\Main\Application::getConnection()->getSqlHelper()->forSql($strField);
				$strValue = \Bitrix\Main\Application::getConnection()->getSqlHelper()->forSql($strValue);
				if(is_numeric($strField)){
					$strSql .= " AND ({$strValue})";
				}
				else{
					$strSql .= " AND (`{$strField}`{$strEqual}'{$strValue}')";
				}
			}
			$strSql .= ';';
		}
		return \Bitrix\Main\Application::getConnection()->query($strSql);
	}

}
