...
<div id="acrit_wb_json_preview_popup">
	...
	<?
	$obTabControl->beginNextTab();
	$strJson = $arTask['STOCKS_RESPONSE'];
	try{
		$strJson = Json::encode(Json::decode($arTask['STOCKS_RESPONSE']), JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
	}
	catch(\Throwable $obError){}
	?>

		<div><?Helper::P($strJson);?></div>
