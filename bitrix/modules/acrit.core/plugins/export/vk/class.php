...
class Vk extends Plugin {
	 ...
	 *	Http request
	 */
	public function request($method, array $arParams = Array(), $intProfileID, $token=false) {
		#$arProfile = Profile::getProfiles($intProfileID);
		$arProfile = Helper::call($this->strModuleId, 'Profile', 'getProfiles', [$intProfileID]);
		$access_token = $token ? : $arProfile['PARAMS']['ACCESS_TOKEN'];
		$arBaseParams = array(
		...
		Log::getInstance($this->strModuleId)->add('request: ' . static::API_URL . $method . '?' . http_build_query($arParams), $intProfileID, true);
		try {
	 ...
	 *	Send file to vk
	 */
	protected function sendFileRemote($strFile, $strUrl, $intProfileID=false) {
		$result = false;
		if(is_file($strFile)){
			...
			if(is_array($arFile)){
				...
				usleep(350000);
				try {
					$result = Json::decode($strResult);
				}
				catch(\Exception $e){
					if ($intProfileID) {
						Log::getInstance($this->strModuleId)->add('Error json decode. Input file data: ' . print_r($strResult, true), $intProfileID);
					}
				}
			}
		}
