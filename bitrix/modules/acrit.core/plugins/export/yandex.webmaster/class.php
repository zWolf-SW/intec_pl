...
class YandexWebmaster extends Plugin {
	...
	protected function stepExport_writeXmlCategories($intProfileID, $arData){
		...
		# Categories with unique names and own ids
		// $bUseUniqueCategories = true;
		// $arData['SESSION']['CATEGORIES_UNIQUE'] = array();
		...
		switch($arData['PROFILE']['PARAMS']['CATEGORIES_REDEFINITION_MODE']){
			...
			case CategoryRedefinition::MODE_STRICT:
				...
				foreach($arSectionsForExportAll as $intCategoryID => $arCategory){
					...
					// if($bUseUniqueCategories){
					// 	$this->getUniqueCategory($arData['SESSION'], $intCategoryID, $strCategoryName, $intParentID);
					// }
					$arCategory = array(
				...
				foreach($arSectionsForExportAll as $intCategoryID => $arCategory){
					if($arData['PROFILE']['PARAMS']['CATEGORIES_EXPORT_PARENTS']=='Y') {
						...
						while($arSectionsChain = $resSectionsChain->getNext()){
							...
							// if($bUseUniqueCategories){
							// 	if($this->getUniqueCategory($arData['SESSION'], $intCategoryID, $strCategoryName, $intParentID) === true){
							// 		continue;
							// 	}
							// }
							$arCategoryXml = array(
	...
	/**
	 * ToDo: support for parent!!!
	 * $arSession['CATEGORIES_UNIQUE'] is an array of [UNIQUE_ID => ['NAME' => 'UNIQUE_NAME', 'ID' => [123, 124, 125]]]
	 */
	// protected function getUniqueCategory(&$arSession, &$intCategoryId, &$strCategoryName, &$intParentID){
	// 	$bExists = false;
	// 	if(!isset($arSession['CATEGORIES_UNIQUE'])){
	// 		$arSession['CATEGORIES_UNIQUE'] = [];
	// 	}
	// 	$arCategories = &$arSession['CATEGORIES_UNIQUE'];
	// 	#
	// 	$intResultCategoryId = null;
	// 	foreach($arCategories as $intUniqCategoryId => $arUniqudCategory){
	// 		if($arUniqudCategory['NAME'] == $strCategoryName){
	// 			$intResultCategoryId = $intUniqCategoryId;
	// 			break;
	// 		}
	// 	}
	// 	#
	// 	if($intResultCategoryId){
	// 		$strCategoryName = $arCategories[$intResultCategoryId]['NAME'];
	// 		$arUniqueCategory['ID'][] = $intCategoryId;
	// 		$intCategoryId = $intResultCategoryId;
	// 		$bExists = true;
	// 	}
	// 	else{
	// 		$arCategories[$intCategoryId] = [
	// 			'NAME' => $strCategoryName,
	// 			'ID' => [$intCategoryId],
	// 		];
	// 		$intCategoryId = array_key_last($arCategories);
	// 	}
	// 	return $bExists;
	// }
