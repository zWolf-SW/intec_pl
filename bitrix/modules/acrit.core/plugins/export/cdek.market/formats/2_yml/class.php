...
class CdekMarketYml extends CdekMarket {
	...
	public function getFields($intProfileID, $intIBlockID, $bAdmin=false){
		...
		$arResult[] = new Field(array(
			'CODE' => 'AMOUNT',
			'DISPLAY_CODE' => 'amount',
			'NAME' => static::getMessage('FIELD_AMOUNT_NAME'),
			'SORT' => 1300,
			'DESCRIPTION' => static::getMessage('FIELD_AMOUNT_DESC'),
			'REQUIRED' => true,
			'MULTIPLE' => false,
			'DEFAULT_VALUE' => array(
				array(
		...
		$arResult[] = new Field(array(
			...
			'DEFAULT_VALUE' => array(
				array(
					'TYPE' => 'CONST',
					'CONST' => '{=catalog.CATALOG_WEIGHT} / 1000',
				),
			),
		));
		$arResult[] = new Field(array(
			...
			'REQUIRED' => false,
			'MULTIPLE' => false,
			'PARAMS' => ['MULTIPLE_separator' => 'other', 'MULTIPLE_separator_other' => '/'],
			'DEFAULT_VALUE' => array(
				array(
					'TYPE' => 'CONST',
					'CONST' => '{=catalog.CATALOG_LENGTH}/10',
					'PARAMS' => ['MATH' => 'Y'],
				),
				array(
					'TYPE' => 'CONST',
					'CONST' => '{=catalog.CATALOG_WIDTH}/10',
					'PARAMS' => ['MATH' => 'Y'],
				),
				array(
					'TYPE' => 'CONST',
					'CONST' => '{=catalog.CATALOG_HEIGHT}/10',
					'PARAMS' => ['MATH' => 'Y'],
				),
			),
		));
	...
	public function processElement($arProfile, $intIBlockID, $arElement, $arFields){
		...
		if(!Helper::isEmpty($arFields['AMOUNT']))
			$arXmlTags['amount'] = Xml::addTag($arFields['AMOUNT']);
