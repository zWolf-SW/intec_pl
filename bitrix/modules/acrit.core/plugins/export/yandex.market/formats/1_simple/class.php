...
class YandexMarketSimple extends YandexMarket {
	...
	public function getFields($intProfileID, $intIBlockID, $bAdmin=false){
		...
		$arResult[] = new Field(array(
			'CODE' => 'CONDITION_QUALITY',
			'DISPLAY_CODE' => 'condition -> quality',
			'NAME' => static::getMessage('FIELD_CONDITION_QUALITY_NAME'),
			'SORT' => 3422,
			'DESCRIPTION' => static::getMessage('FIELD_CONDITION_QUALITY_DESC'),
			'REQUIRED' => false,
			'MULTIPLE' => false,
		));
		$arResult[] = new Field(array(
			'CODE' => 'MIN_QUANTITY',
			'DISPLAY_CODE' => 'min-quantity',
	...
	public function processElement($arProfile, $intIBlockID, $arElement, $arFields){
		...
		$arXmlTags['categoryId'] = $this->addTagCategoryId($arElementSections);
		...
		if(!Helper::isEmpty($arFields['MIN_QUANTITY']))
			$arXmlTags['min-quantity'] = Xml::addTag($arFields['MIN_QUANTITY']);
		...
		$arXmlTags['condition'] = $this->getXmlTag_Condition($arFields['CONDITION_TYPE'], $arFields['CONDITION_REASON'], $arFields['CONDITION_QUALITY']);
