...
class AliExpress extends Plugin {
	...
	public function getFields() {
		...
		$list[] = [
			'id' => 'buyer_first_name',
			'name' => Loc::getMessage('ACRIT_CRM_PLUGIN_ALI_FIELDS_FIRST_NAME'),
			'direction' => self::SYNC_STOC,
		];
		$list[] = [
			'id' => 'buyer_last_name',
			'name' => Loc::getMessage('ACRIT_CRM_PLUGIN_ALI_FIELDS_LAST_NAME'),
			'direction' => self::SYNC_STOC,
		];
		$list[] = [
			'id' => 'buyer_login_id',
			'name' => Loc::getMessage('ACRIT_CRM_PLUGIN_ALI_FIELDS_LOGIN_ID'),
			'direction' => self::SYNC_STOC,
		];
		$list[] = [
			'id' => 'buyer_country',
			'name' => Loc::getMessage('ACRIT_CRM_PLUGIN_ALI_FIELDS_COUNTRY'),
			'direction' => self::SYNC_STOC,
		];
		$list[] = [
			'id' => 'gmt_create',
	...
	public function getTokenLink() {
		...
		$link = "https://openapi.aliexpress.ru/authorize?response_type=code&client_id=".self::APP_KEY."&state=&view=web&sp=ae";
		return $link;
	}
		...
		<table class="acrit-exp-plugin-settings" style="width:100%;">
			...
			<tr>
				<td width="40%" class="adm-detail-content-cell-l">
                    ...
                    <span class="adm-required-field"><?=Loc::getMessage('ACRIT_CRM_PLUGIN_ALI_SETTINGS_TOKEN');?></span>:
				</td>
	...
	public function getOrder($order_id) {
	    ...
	    if ($ali_order['id']) {
		    ...
		    foreach ($products_list as $item) {
		        ...
		        $attribs = json_decode($item['product_attributes'], true);
		        if (!empty($attribs['sku'])) {
			        foreach ($attribs['sku'] as $sku) {
                        $name .= ' | ' . $sku['pName'] . ':' . $sku['pValue'];
		            }
		        }
                $order['PRODUCTS'][] = [
