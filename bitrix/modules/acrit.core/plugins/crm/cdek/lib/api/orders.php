...
namespace Acrit\Core\Crm\Plugins\CdekHelpers;
...
class Orders extends Request {
	...
	public function checkConnection($api_key, &$message) {
		...
		if (!$res['ordersCount']) {
			$message = Loc::getMessage('ACRIT_CRM_PLUGIN_CDEK_CHECK_ERROR') . implode('. ', $res['message']);
		}
		else {
			$message = Loc::getMessage('ACRIT_CRM_PLUGIN_CDEK_CHECK_SUCCESS');
			$result = true;
		}
		return $result;
	}
