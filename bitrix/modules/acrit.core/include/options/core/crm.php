		...
		if ($app_info) {
			if (!$auth_info) {
				...
				$arTabOptions['OPTIONS']['crm_conn_reset'] = [
					...
					'CALLBACK_MAIN' => function ($obOptions, $arTab) {
						$ajax_link = '/bitrix/admin/'.str_replace('.', '_', $this->strModuleId).'_crm_ajax.php';
						?>
                        <a href="#" class="adm-btn crm-reset-conn"><?=Loc::getMessage('ACRIT_CORE_OPTION_CRM_RESET_BTN');?></a>
                        <script>
                            $(".crm-reset-conn").click(function() {
                                $(this).addClass('adm-btn-disabled');
                                $.post("<?=$ajax_link;?>", {
                                    action: 'options_connect_reset',
                                }, function (data) {
                                    if (data.status == 'ok') {
                                        location.reload();
                                    }
				...
				$arTabOptions['OPTIONS']['crm_conn_check'] = [
					'NAME'          => Loc::getMessage('ACRIT_CORE_OPTION_CRM_CHECK'),
					'HINT'          => Loc::getMessage('ACRIT_CORE_OPTION_CRM_CHECK_HINT'),
					'CALLBACK_MAIN' => function ($obOptions, $arTab) {
						$conn_ok_msg = Utilities::checkConnection() ?
							('<span style="color:green;font-weight:bold">' . Loc::getMessage('ACRIT_CORE_OPTION_CRM_CHECK_Y') . '</span>') :
							('<span style="color:red;font-weight:bold">' . Loc::getMessage('ACRIT_CORE_OPTION_CRM_CHECK_N') . '</span>');
						echo $conn_ok_msg;
					},
				];
			}
