...
$bMultithreadingDisabled = Cli::isMultithreadingDisabled() === true;
$strCoreCount = $GLOBALS['ACRIT_CORE_CPU_CORE_COUNT'] = Cli::getCpuCoresCount();
$bMultithreadingAvailable = in_array($this->strModuleId, array_slice(\Acrit\Core\Export\Exporter::getExportModules(), -3));
$GLOBALS['ACRIT_EXP_MULTITHREADING_SUPPORTED'] = &$bMultithreadingSupported;
$GLOBALS['ACRIT_EXP_MULTITHREADING_DISABLED'] = &$bMultithreadingDisabled;
		...
		'multithreaded' => [
			...
			'CALLBACK_MORE' => function($arOption){
				...
				if($GLOBALS['ACRIT_EXP_MULTITHREADING_AVAILABLE']){
					if($GLOBALS['ACRIT_EXP_MULTITHREADING_DISABLED']){
						print Loc::getMessage('ACRIT_CORE_OPTION_MULTITHREADED_DISABLED');
					}
					elseif(!$GLOBALS['ACRIT_EXP_MULTITHREADING_SUPPORTED']){
						if(Cli::isWindows()){
			...
			'CALLBACK_BOTTOM' => function($obOptions, $arOption){
				?>
					<?if(!$GLOBALS['ACRIT_EXP_MULTITHREADING_SUPPORTED'] && !$GLOBALS['ACRIT_EXP_MULTITHREADING_DISABLED']):?>
						<tr>
							<td style="padding-top:0;"></td>
							<td style="padding-top:0;" data-role="php-paths">
								<div>
