...
require_once($_SERVER["DOCUMENT_ROOT"] . "/bitrix/modules/webizi.csv/classes/general/wi_csv.php");
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"].'/bitrix/modules/iblock/admin/data_import.php');
...
class CAssocData extends CCSVData
{
	...
	function MapEnum($prop_id, $value)
	{
		...
		if ($multi_separator_type != "" && strpos($value, $multi_separator_type) !== false) {
			...
			$arrEnum = WICsv::GetEnumList($this->IBLOCK_ID);
	...
	{
		global $PATH2PROP_FILES, $multi_separator_type;
		$io = CBXVirtualIo::GetInstance();
	...
	function ToTranslite($name, $type)
	{
		...
		if ('Y' == $USE_TRANSLIT)
		{
			...
			{
				$USE_TRANSLIT = 'N';
				$strError .= GetMessage("CATI_USE_CODE_TRANSLIT_OUT")."<br>";
			}
		}
		if ('Y' == $USE_TRANSLIT)
		{
			...
			{
				$USE_TRANSLIT = 'N';
				$strError .= GetMessage("CATI_CODE_TRANSLIT_LANG_ERR")."<br>";
			}
		}
...
?>
	...
	<a href="<?echo $APPLICATION->GetCurPage()?>?lang=<?echo LANG ?>&<?echo $strParams ?>"><?=GetMessage("IBLOCK_ADM_IMP_AUTO_REFRESH_STEP")?></a><br>
...
?>
	...
	if ($fields_type == "R" || strlen($fields_type) <= 0)
		echo "checked"?> onClick="ChangeExtra()"><label for="fields_type_R"><?=GetMessage("IBLOCK_ADM_IMP_RAZDELITEL")?></label><br>
			<input type="radio" name="fields_type" id="fields_type_F" value="F" <?
	if ($fields_type == "F")
		echo "checked"?> onClick="ChangeExtra()"><label for="fields_type_F"><?=GetMessage("IBLOCK_ADM_IMP_FIXED")?></label>
	...
	if ($delimiter_r == "TZP" || strlen($delimiter_r) <= 0)
		echo "checked" ?>><label for="delimiter_r_TZP"><?=GetMessage("IBLOCK_ADM_IMP_TZP")?></label><br>
			<input type="radio" name="delimiter_r" id="delimiter_r_ZPT" value="ZPT" <?
	if ($delimiter_r == "ZPT")
		echo "checked" ?>><label for="delimiter_r_ZPT"><?=GetMessage("IBLOCK_ADM_IMP_ZPT")?></label><br>
			<input type="radio" name="delimiter_r" id="delimiter_r_TAB" value="TAB" <?
	if ($delimiter_r == "TAB")
		echo "checked" ?>><label for="delimiter_r_TAB"><?=GetMessage("IBLOCK_ADM_IMP_TAB")?></label><br>
			<input type="radio" name="delimiter_r" id="delimiter_r_SPS" value="SPS" <?
	if ($delimiter_r == "SPS")
		echo "checked" ?>><label for="delimiter_r_SPS"><?=GetMessage("IBLOCK_ADM_IMP_SPS")?></label><br>
			<input type="radio" name="delimiter_r" id="delimiter_r_OTR" value="OTR" <?
	if ($delimiter_r == "OTR")
		echo "checked" ?>><label for="delimiter_r_OTR"><?=GetMessage("IBLOCK_ADM_IMP_OTR")?></label>
	...
	<tr id="multi_separator" class="heading">
		<td colspan="2"><label><?=GetMessage("WI_MULTI_SEPARATOR")?></label><br></td>
	</tr>

	<tr id="multi_separator1">
		<td class="adm-detail-valign-top">
			<?=GetMessage("WI_SEPARATOR")?><br>
			<small><?=GetMessage("WI_EXAMPLE")?></small>:
		</td>
		<td>
			<input type="text" name="multi_separator_type" size="3" value="<?echo htmlspecialcharsbx($multi_separator_type)?>">
		</td>
	</tr>
...
?>
		<tr>
			<td width="40%">
				<b><?=GetMessage("IBLOCK_ADM_IMP_FIELD")?> <?echo $i + 1 ?></b> (<?echo htmlspecialcharsbx($field)?>):
			</td>
			<td width="60%">
...
?>
	...
	<tr>
		<td width="40%"><label for="USE_TRANSLIT_Y"><?=GetMessage("WI_USE_TRANSLITERATION")?></label>:</td>
		<td width="60%">
	...
	{
		?><tr>
			<td width="40%"><?=GetMessage("WI_LANGUAGE_TRANSLITERATION")?>:</td>
			<td width="60%">
	...
	if ($outFileAction == "H")
		echo "checked"?>><label for="outFileAction_H"><?=GetMessage("IBLOCK_ADM_IMP_OF_DEACT")?></label><br>
			<input type="radio" id="outFileAction_D" name="outFileAction" value="D" <?
	if ($outFileAction == "D")
		echo "checked"?>><label for="outFileAction_D"><?=GetMessage("IBLOCK_ADM_IMP_OF_DEL")?></label><br>
			<input type="radio" id="outFileAction_F" name="outFileAction" value="F" <?
	if (strlen($outFileAction) <= 0 || $outFileAction == "F")
		echo "checked"?>><label for="outFileAction_F"><?=GetMessage("IBLOCK_ADM_IMP_OF_KEEP")?></label>
		</td>
	</tr>
	<tr>
	...
	if (strlen($inFileAction) <= 0 || ($inFileAction == "F"))
		echo "checked"?>><label for="inFileAction_F"><?=GetMessage("IBLOCK_ADM_IMP_KEEP_AS_IS")?></label><br>
			<input type="radio" id="inFileAction_A" name="inFileAction" value="A" <?
	if ($inFileAction == "A")
		echo "checked"?>><label for="inFileAction_A"><?=GetMessage("IBLOCK_ADM_IMP_ACTIVATE_PROD")?></label>
		</td>
	</tr>
	<tr>
...
if ($STEP < 4): ?>
	...
	if ($STEP <> 2): ?>
		...
		<input type="hidden" name="multi_separator_type" value="<?echo htmlspecialcharsbx($multi_separator_type)?>">
...
?>
		...
		<input type="hidden" name="USE_TRANSLIT" value="<?echo htmlspecialcharsbx($USE_TRANSLIT)?>">
