...
/**
 * Возвращает значение дополнительных свойств заказа по названию опции модуля, содержащей его название
 * @param $optionName
 * @return void
 */
...
if (is_null($order)) {
    echo sprintf('Не удалось найти заказ с ID=%s', $orderId);
    die();
}
...
if (empty($shipmentCollection)) {
    echo sprintf('Не удалось найти отгрузки в ПЭК в заказе с ID=%s', $orderId);
    die();
}
...
if (!empty($_POST['action']) && ($_POST['action'] == 'save' || $_POST['action'] == 'update')) {
    ...
    if (!empty($_POST['pec_price']) && $_POST['pec_cost_out'] === '0') {
        ...
        $arFieldsDev = array(
            ...
            "PRICE" => $ordProp["PRICE"] - $_POST['OLD_DELIVERY_PRICE'] + $_POST['pec_price']
        );
    ...
    ?><p>Данные сохранены <br> Страница будет перезагрузена</p>
...
?><form method="POST" action="/bitrix/modules/pecom.ecomm/forms/admin/order.php" id="pec_order_edit_form">
                    ...
                    <tr>
                        <td>
                            ...
                            <select name="DOCUMENT_TYPE" required>
                                <?foreach ($arVariants as $arVariant) {?>
        ...
        <tr>
            ...
            <td class="adm-detail-content-cell-r" width="60%">
                <table cellpadding="0" cellspacing="0" border="0" class="nopadding" width="100%">
                    ...
                    <tr>
                        <td>
                            <input id="PEC_WEIGHT" disabled name="WEIGHT" value="<?=$shipmentData['WEIGHT'] ?? Tools::getOrderWeight()?>" required type="text" onchange="
        ...
        <tr>
            ...
            <td class="adm-detail-content-cell-r" width="60%">
                <table cellpadding="0" cellspacing="0" border="0" class="nopadding" width="100%">
                    ...
                    <tr>
                        <td>
                            <input id="PEC_VOLUME" disabled name="VOLUME" value="<?=$shipmentData['VOLUME'] ?? Tools::getOrderVolume()?>" required type="text" onchange="
                                let container = document.getElementById('pec_widget_container');
                                let frame = container.querySelector('iframe');
                                let src = new window.URL(frame.src);
                    ...
                    <tr>
                        <td>
                            <input id="PEC_MAX_DIMENSION" disabled name="MAX_DIMENSION" value="<?=$shipmentData['MAX_DIMENSION'] ?? max(
                                Tools::getOrderMaxDimension()['WIDTH'] / 1000,
                                Tools::getOrderMaxDimension()['LENGTH'] / 1000,
                                Tools::getOrderMaxDimension()['HEIGHT'] / 1000
