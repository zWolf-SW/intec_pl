...
class CExportAgent{
    private static $moduleId = "acrit.export";
    ...
    public static function StartExport( $profileId, $cronSetupId, $bTmpAgent = false ){
        ...
        AcritExportSession::Init( 0 );
        AcritExportSession::DeleteSession( $profileId );
        $arAgent = CAgent::GetList(
            array(),
            array(
                "NAME" => "CExportAgent::StartExport(".$profileId.",".$cronSetupId.");"
            )
        )->Fetch();
        ...
        if( $arAgent ){
            ...
            if( isset( $arProfile["SETUP"]["CRON"][$cronSetupId]["IS_PERIOD"] ) && ( $arProfile["SETUP"]["CRON"][$cronSetupId]["IS_PERIOD"] == "Y" ) ){
                register_shutdown_function( array( "CExportAgent", "UpdateAgent" ), $profileId, $cronSetupId, $arAgent, $arProfile );
            }
        }

        $export = new CAcritExportExport( intval( $profileId ) );
        ...
        if( $profileId > 0 ){
            ...
            if( $cronSetupIdPreset ){
                ...
                $agentId = CAgent::AddAgent(
                    "CExportAgent::StartExport(".$profileId.",".$cronSetupIdPreset.",true);",
                    self::$moduleId,
                    "N",
                    $agentPeriod,
                    "",
                ...
                if( is_array( $arProfile["SETUP"]["CRON"] ) && !empty( $arProfile["SETUP"]["CRON"] ) ){
                    foreach( $arProfile["SETUP"]["CRON"] as $cronSetupId => $arCronItem ){
                        ...
                        if( !$arAgent ){
                            ...
                            $agentId = CAgent::AddAgent(
                                "CExportAgent::StartExport(".$profileId.",".$cronSetupId.");",
                                self::$moduleId,
                                "N",
                                $agentPeriod,
                                "",
