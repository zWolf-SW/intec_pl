...
class CAcritExportTools{
    ...
    public function GetSimpleData(){
        return GetMessage( "ACRIT_TOOLS_SOMEDATA" );
    }
    ...
    public function GetSections( $arProfile ){
        $arSessionData = AcritExportSession::GetAllSession( $arProfile["ID"] );
        $sessionData = array();
        if( !empty( $arSessionData ) ){
            $sessionData = $arSessionData[0];
            if( !is_array( $sessionData["EXPORT"][$arProfile["ID"]]["CATEGORY"] ) )
                $sessionData["EXPORT"][$arProfile["ID"]]["CATEGORY"] = array();

            unset( $arSessionData[0] );
            foreach( $arSessionData as $sData ){
                if( is_array( $sData["EXPORT"][$arProfile["ID"]]["CATEGORY"] ) ){
                    $sessionData["EXPORT"][$arProfile["ID"]]["CATEGORY"] = array_merge(
                        $sessionData["EXPORT"][$arProfile["ID"]]["CATEGORY"],
                        $sData["EXPORT"][$arProfile["ID"]]["CATEGORY"]
                    );
                }
            }
        }
        return array_unique( $sessionData["EXPORT"][$arProfile["ID"]]["CATEGORY"] );
    }
    ...
    public function GetCurrencies( $arProfile ){
        ...
        if( !empty( $arSessionData ) ){
            $sessionData = $arSessionData[0];
            if( !is_array( $sessionData["EXPORT"][$arProfile["ID"]]["CURRENCY"] ) )
                $sessionData["EXPORT"][$arProfile["ID"]]["CURRENCY"] = array();

            unset( $arSessionData[0] );
            foreach( $arSessionData as $sData ){
                if( is_array( $sData["EXPORT"][$arProfile["ID"]]["CURRENCY"] ) ){
                    $sessionData["EXPORT"][$arProfile["ID"]]["CURRENCY"] = array_merge(
                        $sessionData["EXPORT"][$arProfile["ID"]]["CURRENCY"],
                        $sData["EXPORT"][$arProfile["ID"]]["CURRENCY"]
                    );
                }
            }
        }
        return array_unique( array_filter( $sessionData["EXPORT"][$arProfile["ID"]]["CURRENCY"] ) );
    }

    public function SaveSections( $arProfile, $sections ){
        if( is_array( $sections ) ){
            $sessionData = AcritExportSession::GetSession( $arProfile["ID"] );
            ...
            AcritExportSession::SetSession( $arProfile["ID"], $sessionData );
        }
    }
    ...
    public function HasBadCronExports( $arProfile ){
        ...
        $exportPath = ( $arPath[1] == "acrit.export" ) ? $_SERVER["DOCUMENT_ROOT"]."/upload/".$arPath[1]."/" : $_SERVER["DOCUMENT_ROOT"]."/".$arPath[1]."/";
    ...
    public function ClearExportSession( $profileId ){
        AcritExportSession::DeleteSession( $profileId, $_SERVER["DOCUMENT_ROOT"]."/bitrix/tools/acrit.export/" );
    }
    ...
    public function ArrayToExcel( $fileFullName, $fileTitle, $arData, $arProfile, $page ){
        if( !empty( $fileFullName ) && is_array( $arData ) ) {
            $obLog = new CAcritExportLog( $arProfile["ID"] );
            try{
                $objPHPExcel = false;
                if( $page == 1 ){
                ...
                if( $objPHPExcel ){
                    $objPHPExcel->getProperties()->setCreator( "acrit.export" )
                    ->setLastModifiedBy( "acrit.export" )
                    ->setTitle( $fileTitle )
                            ...
                            foreach( $arDataset as $indexRow => $row ){
                                if( $page == 1 ){
                                    if( count( $row ) == count( $arData["HEADER"] ) ){
                                        ...
                                        $obLog->AddMessage( GetMessage( "ACRIT_EXPORT_PHPEXCEL_BROKEN_DATA" ).$indexRow.PHP_EOL );
                                    ...
                                    if( count( $row ) == $iHeaderColumnsCnt ){
                                        ...
                                        $obLog->AddMessage( GetMessage( "ACRIT_EXPORT_PHPEXCEL_BROKEN_DATA" ).( $indexRow + count( $arRows ) ).PHP_EOL );
                                        $obLog->IncProductError();
                                    }
                                }
                            }
                ...
                $obLog->AddMessage( GetMessage( "ACRIT_EXPORT_PHPEXCEL_BAD_EXPORT" ) );
                $obLog->IncProductError();
            }
        }
    }
...
class CAcritExportStringProcess{
        ...
        if( !empty( $convertField ) ){
            if( is_array( $convertField ) ){
                foreach( $convertField as &$val ){
                    $templateValueCharset = CAcritExportTools::GetStringCharset( $val );
                ...
                $templateValueCharset = CAcritExportTools::GetStringCharset( $convertField );
                if( $templateValueCharset == "cp1251" ){
...
class CAcritExportMarketCategories{
    ...
    private function GetLocalFilePath( $yandexCategoryServerPath ){
        ...
        $yandexCategoryLocalPath = $_SERVER["DOCUMENT_ROOT"]."/upload/acrit.export/".$yandexCategoryFileName;
    ...
    private function GetCorrectCharsetRow( $inputRow ){
        ...
        $itemCharset = CAcritExportTools::GetStringCharset( $inputRow );
