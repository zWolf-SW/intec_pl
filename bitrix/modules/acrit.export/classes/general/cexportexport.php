...
class CAcritExportExport{
    ...
    public function __construct( $profileID ){
        global $exportstep;
        $this->lockDir = $_SERVER["DOCUMENT_ROOT"]."/bitrix/tools/acrit.export/";

        $this->dbMan = new CExportProfileDB();
        $this->profile = $this->dbMan->GetByID( $profileID );

        $siteDocumentRoot = CAcritExportTools::GetSiteDocumentRoot( $this->profile["LID"] );
        $this->documentRoot = ( $siteDocumentRoot ) ? $siteDocumentRoot : $_SERVER["DOCUMENT_ROOT"];

        $this->baseDir = $this->documentRoot."/acrit.export/";
        ...
        if( empty( self::$fileExport ) || self::$fileExport == $this->documentRoot ){
            $sessionData = AcritExportSession::GetSession( $profileID );
            if( !$exportstep || ( $exportstep == 1 ) ){
                self::$fileExport = $this->baseDir."market_".date( "d_m_Y_H_i", time() ).".xml";
                $sessionData["EXPORT"]["TMP_NAME"][$this->profile["ID"]] = self::$fileExport;
            }
            else{
                self::$fileExport = $sessionData["EXPORT"]["TMP_NAME"][$this->profile["ID"]];
            }
        }

        if( strlen( strstr( self::$fileExport, $this->baseDir ) ) > 0 ){
            self::$fileExport = str_replace( $this->baseDir, $this->documentRoot."/upload/acrit.export/", self::$fileExport );
        ...
        if( isset( $sessionData ) && !empty( $sessionData ) ){
            AcritExportSession::SetSession( $profileID, $sessionData );
        }
    }
    ...
    public function Export( $type = "", $cronpage = 0, $processId = 0 ){
        ...
        if( file_exists( $_SERVER["DOCUMENT_ROOT"]."/bitrix/tools/acrit.export/export_{$this->profile["ID"]}_run.unlock" )
            || ( $bBaseCron && file_exists( $_SERVER["DOCUMENT_ROOT"]."/bitrix/tools/acrit.export/export_{$this->profile["ID"]}_run.lock" ) && !$cronpage ) ){
            if( !$bBaseCron ){
                $this->Unlock();
            }
            return false;
        ...
        AcritExportSession::Init( $exportstep );
        ...
        $obLog = new CAcritExportLog( $this->profile["ID"] );
            ...
            if( $_REQUEST["unlock"] == "Y" ){
                ...
                if( $type != "cron" ){
                    ...
                    CAdminMessage::ShowMessage(
                        array(
                            "MESSAGE" => GetMessage( "ACRIT_EXPORT_PROCESS_RUN" ),
                            "TYPE" => "FAIL",
                            "HTML" => "TRUE"
                        )
                    );
                    ...
                    $subject = GetMessage( "ACRIT_EXPORT_PROCESS_RUN_SUBJECT" );
                    $errorMessage = GetMessage( "ACRIT_EXPORT_PROCESS_RUN_ERROR_MESSAGE" );
        ...
        if( CModule::IncludeModule( "catalog" ) ){
            ...
            CAcritExportProps::$arIBlockFilter = $obProfileUtils->PrepareIBlock( $this->profile["IBLOCK_ID"], $this->profile["USE_SKU"] );
        ...
        $elementsObj = new CAcritExportElement( $this->profile );
        ...
        if( !$end ){
            if( !$exportstep || $exportstep == 1 ){
                ...
                AcritExportSession::DeleteSession( $this->profile["ID"] );
            }

            if( $cronrun ){
                if( intval( $cronpage ) > 1 ){
                    $ProcessEnd = false;
                    $procResult = $elementsObj->Process( $exportstep, $cronrun, $this->profile["SETUP"]["FILE_TYPE"], self::$fileExport, $this->profile["SETUP"]["URL_DATA_FILE"], $ProcessEnd, $bStepExport, $iLastSessionExportProductsCnt, $processId );
                    ...
                    if( isset( $sessionData["EXPORT"]["THREAD"][$this->profile["ID"]] )
                        && is_array( $sessionData["EXPORT"]["THREAD"][$this->profile["ID"]] ) ){
                        $sessionData["EXPORT"]["THREAD"][$this->profile["ID"]] = array();
                    ...
                    $ProcessEnd = false;
                    $allPages = $elementsObj->Process( 1, $cronrun, $this->profile["SETUP"]["FILE_TYPE"], self::$fileExport, $this->profile["SETUP"]["URL_DATA_FILE"], $ProcessEnd, $bStepExport, $iLastSessionExportProductsCnt, $processId );

                    $sessionData = AcritExportSession::GetSession( $this->profile["ID"] );
                    $sessionData["EXPORT"]["THREAD"][$this->profile["ID"]] = array();
                    $steps = $sessionData["EXPORT"]["LOG"][$this->profile["ID"]]["STEPS"];
                    ...
                    for( $i = 0; $i < $steps2; $i++ ){
                        for( $j = 0; $j < $tCnt; $j++ ){
                            ...
                            $threadsId = $threads->newThread(
                                $this->documentRoot."/bitrix/modules/acrit.export/tools/cronrun_proc.php",
                                array(
            ...
            if( !$allPages && $allPages != -1 && !$cronrun ){
                ...
                if( ( $this->profile["SETUP"]["FILE_TYPE"] != "csv" ) && ( $this->profile["SETUP"]["FILE_TYPE"] != "xls" ) ){
                    ...
                    $paternCharset = CAcritExportTools::GetStringCharset( $basePatern );

                    if( $paternCharset == "cp1251" ){
                        $basePatern = $APPLICATION->ConvertCharset( $basePatern, "windows-1251", "UTF-8" );
                    }

                    $dateGenerate = ( $this->profile["DATEFORMAT"] == $basePatern ) ? CAcritExportTools::GetYandexDateTime( date( "d.m.Y H:i:s" ) ) : date( str_replace( "_", " ", $this->profile["DATEFORMAT"] ), time() );
                    ...
                    $defaultFields = array(
                        ...
                        "#CATEGORY#" => ( ( $this->profile["USE_MARKET_CATEGORY"] == "Y" ) && ( $this->profile["CHANGE_MARKET_CATEGORY"] == "Y" ) && ( $this->profile["TYPE"] != "google" ) ) ? $this->GetYandexCategoryXML( CAcritExportTools::GetSections( $this->profile ) ) : $this->GetCategoryXML( CAcritExportTools::GetSections( $this->profile ) ),
                        "#CURRENCY#" => ( CModule::IncludeModule( "catalog" ) ) ? $this->GetCurrencyXML( CAcritExportTools::GetCurrencies( $this->profile ) ) : "RUB",
                        "#DATE#" => $dateGenerate,
                ...
                if( ( $this->profile["SETUP"]["FILE_TYPE"] != "csv" ) && ( $this->profile["SETUP"]["FILE_TYPE"] != "xls" ) ){
                    ...
                    CExportInformer::GetInformerData();

                    $this->SetExportFileEncoding();
                }

                if( CAcritExportTools::HasBadCronExports( $this->profile ) ){
                    $arSupportData[] = GetMessage( "ACRIT_EXPORT_PROCESS_CRON_BAD_EXPORTS" );
                }
                ...
                if( $this->profile["USE_COMPRESS"] == "Y" ){
                    ...
                    elseif( file_exists( $this->documentRoot."/upload/acrit.export/".$this->originalNamePath ) ){
                        $zipSavePath = $this->documentRoot."/upload/acrit.export/".$this->originalNamePath;
                    }
    ...
    private function Unlock(){
        ...
        AcritExportSession::DeleteSession( $this->profile["ID"], $_SERVER["DOCUMENT_ROOT"]."/bitrix/tools/acrit.export/" );
    }
    ...
    private function GetUAHRate(){
        ...
        $currencyRates = CExportProfile::LoadCurrencyRates();
        $CURRENCIES = array();
    ...
    private function GetCategoryXML( $sections ){
        ...
        foreach( $sections as $sectionId ){
            $dbSectionList = CAcritExportTools::GetSectionTree(
                false,
                $sectionId
            );
    ...
    private function GetYandexCategoryXML( $sections ){
        ...
        foreach( $arMarketProcessCategory as $marketProcessCategoryItem ){
            if( $this->profile["MARKET_CATEGORY"]["CATEGORY"] == 2 ){
                $arCategoryParts = explode( " > ", $marketProcessCategoryItem );
            }
            else{
                $arCategoryParts = explode( "/", $marketProcessCategoryItem );
            }
            ...
            if( count( $arCategoryParts ) > 1 ){
                ...
                $parentCategoryName = implode( "/", $arCategoryParts );
                $sectionFields["#PARENT_ID#"] = array_search( $parentCategoryName, $arMarketCategory ) + 1;
