...
$arClasses = array(
    ...
    "CExportProfileDB" => "classes/mysql/cexportprofiledb.php",
    "CExportMarketDB" => "classes/mysql/cexportpro_marketdb.php",
    "CExportProfile" => "classes/general/cexportprofile.php",

    "CAcritGlobalCondCtrl" => "classes/general/cexportcond.php",
    "CAcritGlobalCondCtrlComplex" => "classes/general/cexportcond.php",
    "CAcritGlobalCondCtrlAtoms" => "classes/general/cexportcond.php",
    "CAcritGlobalCondCtrlGroup" => "classes/general/cexportcond.php",
    "CAcritGlobalCondTree" => "classes/general/cexportcond.php",
    "CAcritCatalogCondCtrl" => "classes/general/cexportcond.php",
    "CAcritCatalogCondCtrlComplex" => "classes/general/cexportcond.php",
    "CAcritCatalogCondCtrlGroup" => "classes/general/cexportcond.php",
    "CAcritCatalogCondCtrlIBlockFields" => "classes/general/cexportcond.php",
    "CAcritCatalogCondCtrlIBlockProps" => "classes/general/cexportcond.php",
    "CAcritCatalogCondTree" => "classes/general/cexportcond.php",

    "CAcritExportCatalog" => "classes/general/cexportfilter.php",
    "CAcritExportPrices" => "classes/general/cexportfilter.php",
    "CAcritExportProps" => "classes/general/cexportfilter.php",
    "CAcritExportCatalogCond" => "classes/general/cexportfilter.php",
    "CAcritExportLog" => "classes/general/cexportlog.php",
    "AcritExportSession" => "classes/general/cexportsession.php",
    "CAcritExportUrlRewrite" => "classes/general/cexporturlrewrite.php",
    "CAcritExportTools" => "classes/general/cexporttools.php",
    "CAcritExportStringProcess" => "classes/general/cexporttools.php",
    "CAcritExportMarketCategories" => "classes/general/cexporttools.php",

    "CAcritExportExport" => "classes/general/cexportexport.php",
    "CExportAgent" => "classes/general/cexportagent.php",
    "CExportInformer" => "classes/general/cexportinformer.php",
    "Threads" => "classes/general/threads.php",
    "ThreadsSession" => "classes/general/threads.php",
);

CModule::AddAutoloadClasses( "acrit.export", $arClasses );
...
class CAcritExportMenu{
    public static function OnBuildGlobalMenu( &$aGlobalMenu, &$aModuleMenu ){
        global $USER, $APPLICATION, $adminMenu, $adminPage;
				if(\Bitrix\Main\Config\Option::get('acrit.export', 'disable_old_core') == 'Y'){
					return;
				}
        ...
        $acritMenuGroupName = COption::GetOptionString( "acrit.export", "acritmenu_groupname" );
...
class CAcritExportElement{
    public $profile = null;
    public $DEMO = 2;
    public $isDemo = true;
    public $DEMO_CNT;
    public $MODULEID = "acrit.export";
    ...
    public function __construct( $profile ){
        ...
        $this->obProfileUtils = new CExportProfile();
        ...
        $this->log = new CAcritExportLog( $this->profile["ID"] );

        $this->baseDateTimePatern = "Y-m-dTh:i:s±h:i";

        $paternCharset = CAcritExportTools::GetStringCharset( $this->baseDateTimePatern );

        if( $paternCharset == "cp1251" ){
            $this->baseDateTimePatern = $APPLICATION->ConvertCharset( $this->baseDateTimePatern, "windows-1251", "UTF-8" );
        }

        $dateGenerate = ( $this->profile["DATEFORMAT"] == $this->baseDateTimePatern ) ? CAcritExportTools::GetYandexDateTime( date( "d.m.Y H:i:s" ) ) : date( str_replace( "_", " ", $this->profile["DATEFORMAT"] ), time() );
        ...
        $this->basePriceId = CAcritExportTools::GetProcessPriceId( $this->profile );
    ...
    protected function DemoCount(){
        $arSessionData = AcritExportSession::GetAllSession( $this->profile["ID"] );
        $demoCnt = 0;
        if( !empty( $arSessionData ) ){
            foreach( $arSessionData as $arSessionDataItem ){
                $demoCnt += $arSessionDataItem["EXPORT"][$this->profile["ID"]]["DEMO_COUNT"];
            }
        }
    ...
    protected function DemoCountInc(){
        $sessionData = AcritExportSession::GetSession( $this->profile["ID"] );
        if( !isset( $sessionData["EXPORT"][$this->profile["ID"]]["DEMO_COUNT"] ) )
            $sessionData["EXPORT"][$this->profile["ID"]]["DEMO_COUNT"] = 0;

        $sessionData["EXPORT"][$this->profile["ID"]]["DEMO_COUNT"]++;
        AcritExportSession::SetSession( $this->profile["ID"], $sessionData );
    }
    ...
    public function ExportConvertCharset( $field ){
        ...
        $paternCharset = CAcritExportTools::GetStringCharset( $field );
    ...
    public function CalcProcessXMLLoadingByOneProduct(){
        ...
        $sessionData = AcritExportSession::GetSession( $this->profile["ID"] );
        $sessionData["EXPORT"]["LOG"][$this->profile["ID"]]["STEPS"] = $this->isDemo ? 1 : $dbElements->NavPageCount;
        AcritExportSession::SetSession( $this->profile["ID"], $sessionData );
        ...
        CAcritExportTools::SaveCurrencies( $this->profile, $this->currencyList );

        return round( getmicrotime() - $calcTimeStart, 3 );
    }

    public function Process( $page = 1, $cronrun = false, $fileType = "xml", $fileExport = false, $fileExportName = false, &$_ProcessEnd = false, $bStepExport = false, $iLastSessionExportProductsCnt = 0, $processId = 0 ){
        global $fileExportDataSize, $fileExportData, $ProcessEnd;
        $fileThread = false;

        $arIblockData = CAcritExportTools::GetSimpleData();
        $arIblockDataRows = unserialize( base64_decode( $arIblockData ) );

        if( !in_array( $this->profile["TYPE"], $arIblockDataRows ) ){
            return false;
        }
        ...
        elseif( $fileType == "xls" ){
            ...
            $ret = self::ProcessXML( $page, $cronrun, $bStepExport, $iLastSessionExportProductsCnt, $processId );
        }
    ...
    public function PrepareProcess( $page = 1, $bStepExport = false ){
        ...
        $this->currencyRates = CExportProfile::LoadCurrencyRates();
        ...
        $pregMatchExp = GetMessage( "ACRIT_EXPORT_A_AA_A" );
        ...
        $dbEvents = GetModuleEvents( "acrit.export", "OnBeforePropertiesSelect" );
        $eventResult = array();
    ...
    public function ProcessBasicCsv( $dbElements, $fileExport, $page, $navPageCount, $bXls = false ){
        ...
        if( $page == 1 ){
            ...
            if( !$bXls ){
                CAcritExportTools::ExportArrayMultiply( $arResFields, $arTuple );
            }
            else{
        ...
        foreach( $arProcess as $arRow ){
            ...
            if( !$bXls ){
                ...
                CAcritExportTools::ExportArrayMultiply( $arTupleXls["ROWS"], $arTuple );
            }
        }
        ...
        if( !$bXls ){
            ...
            if( is_array( $arTupleXls ) && !empty( $arTupleXls ) ){
                ...
                CAcritExportTools::ArrayToExcel( $fileExportPath, $this->profile["CODE"], $arTupleXls, $this->profile, $page );
            }
        }
    }
    ...
    public function ProcessCSV( $page = 1, $cronrun = false, $fileExport = false, $fileExportName = false, $bXls = false ){
        ...
        $sessionData = AcritExportSession::GetSession( $this->profile["ID"] );
        $sessionData["EXPORT"]["LOG"][$this->profile["ID"]]["STEPS"] = $navPageCount;
        ...
        if( !$cronrun ){
            ...
            GetMessage( "ACRIT_EXPORT_RUN_EXPORT_RUN" ), "<br/>",
            str_replace( array( "#PROFILE_ID#", "#PROFILE_NAME#" ), array( $this->profile["ID"], $this->profile["NAME"] ), GetMessage( "ACRIT_EXPORT_RUN_STEP_PROFILE" ) ), "<br/>",
            str_replace( array( "#STEP#", "#COUNT#" ), array( $page, $navPageCount ), GetMessage( "ACRIT_EXPORT_RUN_STEP_RUN" ) ),
            "</div>";
        }
    ...
    public function ProcessXML( $page = 1, $cronrun = false, $bStepExport = false, $iLastSessionExportProductsCnt = 0, $processId = 0 ){
        ...
        while( $arElement = $dbElements->GetNextElement() ){
            $processSessionData = AcritExportSession::GetSession( $this->profile["ID"] );

            if( $bStepExport ){
                if( ( $processSessionData["EXPORT"]["LOG"][$this->profile["ID"]]["PRODUCTS_EXPORT"] >= ( $iLastSessionExportProductsCnt + $this->profile["SETUP"]["CRON"][$processId]["STEP_EXPORT_CNT"] ) )
                    || ( ( intval( $this->profile["SETUP"]["CRON"][$processId]["MAXIMUM_PRODUCTS"] ) > 0 ) && ( $processSessionData["EXPORT"]["LOG"][$this->profile["ID"]]["PRODUCTS_EXPORT"] >= $this->profile["SETUP"]["CRON"][$processId]["MAXIMUM_PRODUCTS"] ) ) ){
                    break;
                }
            }
            ...
            if( $bSchemeUseOffer || $bSchemeUseOfferSku || $bSchemeUseSkuByOffer ){
                ...
                $this->delay = "";
                $arItem = $this->ProcessElement( $arElement, false, false, $arElementConfig );
            ...
            if( $this->catalogIncluded && ( $this->profile["USE_SKU"] == "Y" ) && ( $bSchemeUseSku || $bSchemeUseSkuByOffer ) ){
                ...
                if( ( $arItem["ACTIVE"] == "Y" ) && ( $this->catalogSKU[$arItem["IBLOCK_ID"]] ) ){
                    ...
                    while( $arOfferElement = $dbOfferElements->GetNextElement() ){
                        ...
                        $arOfferItem = $this->ProcessElement( $arOfferElement, $arItem, false, $arElementConfig, $arOfferElementResult );
                        unset( $arOfferItem );
            ...
            if( isset( $arElementConfig["DELAY"] ) && $arElementConfig["DELAY"] == true ){
                ...
                $this->ProcessElement( $arElement, false, false, $arElementConfig, $arOfferElementResult );
    ...
    private function ProcessElement( $arElement, $arProductSKU = false, $bCsvMode = false, $arItemConfig = array(), &$arOfferElementResult = array() ){
        ...
        if( $this->catalogIncluded ){
            ...
            if( $profileData["SETUP"]["VALIDATE_CONDITIONS"] == "Y" ){
                ...
                if( $sBeforeConditions !== $sAfterConditions ){
                    ...
                    CAcritExportProps::$arIBlockFilter = CExportProfile::PrepareIBlock( $profileData["IBLOCK_ID"], $profileData["USE_SKU"] );
            ...
            if( !CAcritExportTools::CheckCondition( $arItem, $profileData["EVAL_FILTER"] ) ){
        ...
        if( empty( $arSectionCache[$arItem["IBLOCK_ID"]] ) ){
            ...
            while( $ar = $rs->GetNext( false, false ) ){
                if( intval( $ar["PICTURE"] ) ){
                    $ar["PICTURE"] = CAcritExportTools::GetFilePath( $ar["PICTURE"] );
                }
                if( intval( $ar["DETAIL_PICTURE"] ) ){
                    $ar["DETAIL_PICTURE"] = CAcritExportTools::GetFilePath( $ar["DETAIL_PICTURE"] );
                }
        ...
        foreach( $this->profile["XMLDATA"] as $xmlCode => $field ){
            ...
            if( $useCondition ){
                $conditionTrue = ( CAcritExportTools::CheckCondition( $arItem, $field["EVAL_FILTER"] ) == true );
            }
            ...
            if( $useCondition && !$conditionTrue ){
                    ...
                    if( $field["TYPE"] == "composite" ){
                        ...
                        foreach( $field["COMPOSITE_FALSE"] as $compositeFieldIndex => $compositeField ){
                            ...
                            if( $compositeField["COMPOSITE_FALSE_TYPE"] == "const" ){
                                $compositeValue .= CAcritExportTools::RoundNumber( $compositeField["COMPOSITE_FALSE_CONTVALUE"], $field["ROUND"]["PRECISION"], $field["ROUND"]["MODE"] );
                            }
                                    ...
                                    switch( count( $arValue ) ){
                                        case 1:
                                            ...
                                            if( strpos( $compositeField["COMPOSITE_FALSE_VALUE"], "." ) !== false ){
                                                ...
                                                $compositeValueTmp = CAcritExportTools::RoundNumber( $value, $field["ROUND"]["PRECISION"], $field["ROUND"]["MODE"] );
                                            }
                                            else{
                                                $compositeValueTmp = CAcritExportTools::RoundNumber( $arItem[$compositeField["COMPOSITE_FALSE_VALUE"]], $field["ROUND"]["PRECISION"], $field["ROUND"]["MODE"] );
                                            }
                                            $arItem = $arItemMain;
                                            break;
                                        case 2:
                                            ...
                                            if( ( $field["VALUE"] == "CATALOG-PURCHASING_PRICE" ) && isset( $arItem["CATALOG_PURCHASING_PRICE"] ) ){
                                                ...
                                                preg_match( "/PRICE_[\d]+/".$fieldPrePostfix, $arValue[1], $arPriceCode );
                                            }
                                            ...
                                            if( strpos( $arValue[1], "_CURRENCY" ) > 0 ){
                                                ...
                                                $compositeValueTmp = CAcritExportTools::RoundNumber( $compositeValueTmp, $field["ROUND"]["PRECISION"], $field["ROUND"]["MODE"] );
                                                ...
                                                if( $this->profile["CURRENCY"]["CONVERT_CURRENCY"] == "Y" ){
                                                    if( $this->profile["CURRENCY"][$convertFrom]["CHECK"] ){
                                                        ...
                                                        if( $this->profile["CURRENCY"][$convertFrom]["RATE"] == "SITE" ){
                                                            $compositeValueTmp = CAcritExportTools::RoundNumber( CCurrencyRates::ConvertCurrency(
                                                            ...
                                                            $compositeValueTmp = CAcritExportTools::RoundNumber( $compositeValueTmp *
                                            ...
                                            if( $field["BITRIX_ROUND_MODE"] == "Y" ){
                                                $compositeValueTmp = CAcritExportTools::BitrixRoundNumber( $compositeValueTmp, $arValue[1] );
                                            }
                                            else{
                                                $compositeValueTmp = CAcritExportTools::RoundNumber( $compositeValueTmp, $field["ROUND"]["PRECISION"], $field["ROUND"]["MODE"] );
                                            }
                                        ...
                                        case 3:
                                            ...
                                            if( ( $arValue[0] == $arItem["IBLOCK_ID"] ) || ( $arValue[0] == $arProductSKU["IBLOCK_ID"] ) ){
                                                if( $this->catalogSKU[$arValue[0]]["OFFERS_PROPERTY_ID"] == $arValue[2] ){
                                                    $arItem["PROPERTY_{$arValue[2]}_DISPLAY_VALUE"] = CAcritExportTools::RoundNumber( $arItem["PROPERTY_{$arValue[2]}_VALUE"][0], $field["ROUND"]["PRECISION"], $field["ROUND"]["MODE"] );
                                                }
                                                ...
                                                if( is_array( $arProcessValues ) ){
                                                    $arProcessValuesMultiproFormat = CAcritExportTools::ParseMultiproFormat( $arProcessValues, $this->profile, $field["CODE"] );
                                                    ...
                                                    foreach( $arProcessValues as $val ){
                                                        if( intval( $this->profile["XMLDATA"][$field["CODE"]]["MULTIPROP_LIMIT"] ) > 0 ){
                                                            if( count( $compositeValueTmp ) < $this->profile["XMLDATA"][$field["CODE"]]["MULTIPROP_LIMIT"] ){
                                                                $compositeValueTmp[] = CAcritExportTools::RoundNumber( $val, $field["ROUND"]["PRECISION"], $field["ROUND"]["MODE"] );
                                                            }
                                                        }
                                                        else{
                                                            $compositeValueTmp[] = CAcritExportTools::RoundNumber( $val, $field["ROUND"]["PRECISION"], $field["ROUND"]["MODE"] );
                                                    ...
                                                    $compositeValueTmp = CAcritExportTools::RoundNumber( $arProcessValues, $field["ROUND"]["PRECISION"], $field["ROUND"]["MODE"] );
                                                }
                                            }
                                            $arItem = $arItemMain;
                                            break;
                        ...
                        $templateValues[$fieldIndex] =  CAcritExportTools::RoundNumber( $compositeValue, $field["ROUND"]["PRECISION"], $field["ROUND"]["MODE"] );
                    }
                    else{
                                ...
                                switch( count( $arValue ) ){
                                    case 1:
                                        ...
                                        if( strpos( $field["VALUE"], "." ) !== false ){
                                            ...
                                            unset( $arField );
                                            $templateValues[$fieldIndex] = CAcritExportTools::RoundNumber( $value, $field["ROUND"]["PRECISION"], $field["ROUND"]["MODE"] );
                                        }
                                        else{
                                            $templateValues[$fieldIndex] = CAcritExportTools::RoundNumber( $arItem[$field["VALUE"]], $field["ROUND"]["PRECISION"], $field["ROUND"]["MODE"] );
                                        }
                                        $arItem = $arItemMain;
                                        break;
                                    case 2:
                                        ...
                                        if( strpos( $arValue[1], "_CURRENCY" ) > 0 ){
                                            ...
                                            $templateValues[$fieldIndex] = CAcritExportTools::RoundNumber( $templateValues[$fieldIndex], $field["ROUND"]["PRECISION"], $field["ROUND"]["MODE"] );
                                            if( is_array( $arProductSKU ) ){
                                        ...
                                        elseif( !empty( $arPriceCode[0] ) ){
                                            if( $this->profile["CURRENCY"]["CONVERT_CURRENCY"] == "Y" ){
                                                if( $this->profile["CURRENCY"][$convertFrom]["CHECK"] ){
                                                    $convertTo = $this->profile["CURRENCY"][$convertFrom]["CONVERT_TO"];
                                                    if( $this->profile["CURRENCY"][$convertFrom]["RATE"] == "SITE" ){
                                                        $templateValues[$fieldIndex] = CAcritExportTools::RoundNumber( CCurrencyRates::ConvertCurrency(
                                                                $arItem["CATALOG_".$arValue[1]],
                                                                $this->profile["CURRENCY"][$convertFrom]["CONVERT_FROM"],
                                                                $convertTo
                                                            ),
                                                        ...
                                                        $templateValues[$fieldIndex] = CAcritExportTools::RoundNumber( $templateValues[$fieldIndex] *
                                                            $this->currencyRates[$this->profile["CURRENCY"][$convertFrom]["RATE"]][$convertFrom]["RATE"] /
                                                            $this->currencyRates[$this->profile["CURRENCY"][$convertFrom]["RATE"]][$convertTo]["RATE"] /
                                                            $this->currencyRates[$this->profile["CURRENCY"][$convertFrom]["RATE"]][$convertFrom]["RATE_CNT"] *
                                                            $this->currencyRates[$this->profile["CURRENCY"][$convertFrom]["RATE"]][$convertTo]["RATE_CNT"],
                                        ...
                                        if( $field["BITRIX_ROUND_MODE"] == "Y" ){
                                            $templateValues[$fieldIndex] = CAcritExportTools::BitrixRoundNumber( $templateValues[$fieldIndex], $arValue[1] );
                                        }
                                        else{
                                            $templateValues[$fieldIndex] = CAcritExportTools::RoundNumber( $templateValues[$fieldIndex], $field["ROUND"]["PRECISION"], $field["ROUND"]["MODE"] );
                                        }
                                    ...
                                    case 3:
                                        ...
                                        if( $arValue[0] == $arItem["IBLOCK_ID"] || $arValue[0] == $arProductSKU["IBLOCK_ID"] ){
                                            ...
                                            if( is_array( $arProcessValues ) ){
                                                ...
                                                foreach( $arProcessValues as $val ){
                                                    if( intval( $this->profile["XMLDATA"][$field["CODE"]]["MULTIPROP_LIMIT"] ) > 0 ){
                                                        if( count( $templateValues[$fieldIndex] ) < $this->profile["XMLDATA"][$field["CODE"]]["MULTIPROP_LIMIT"] ){
                                                            $templateValues[$fieldIndex][] = CAcritExportTools::RoundNumber( $val, $field["ROUND"]["PRECISION"], $field["ROUND"]["MODE"] );
                                                        }
                                                    }
                                                    else{
                                                        $templateValues[$fieldIndex][] = CAcritExportTools::RoundNumber( $val, $field["ROUND"]["PRECISION"], $field["ROUND"]["MODE"] );
                                                ...
                                                $templateValues[$fieldIndex] = CAcritExportTools::RoundNumber( $arProcessValues, $field["ROUND"]["PRECISION"], $field["ROUND"]["MODE"] );
                                            }
                                        }
                                        $arItem = $arItemMain;
                                        break;
                    ...
                    if( $field["TYPE"] == "composite" ){
                        ...
                        foreach( $field["COMPOSITE_TRUE"] as $compositeFieldIndex => $compositeField ){
                            ...
                            if( $compositeField["COMPOSITE_TRUE_TYPE"] == "const" ){
                                $compositeValue .= CAcritExportTools::RoundNumber( $compositeField["COMPOSITE_TRUE_CONTVALUE"], $field["ROUND"]["PRECISION"], $field["ROUND"]["MODE"] );
                            }
                                    ...
                                    switch( count( $arValue ) ){
                                        case 1:
                                            ...
                                            if( strpos( $compositeField["COMPOSITE_TRUE_VALUE"], "." ) !== false ){
                                                ...
                                                $compositeValueTmp = CAcritExportTools::RoundNumber( $arItem[$compositeField["COMPOSITE_TRUE_VALUE"]], $field["ROUND"]["PRECISION"], $field["ROUND"]["MODE"] );
                                            }
                                            $arItem = $arItemMain;
                                            break;
                                        case 2:
                ...
                elseif( ( $field["TYPE"] == "const" )
                    ...
                    $templateValues[$fieldIndex] =  CAcritExportTools::RoundNumber( $field["CONTVALUE_TRUE"], $field["ROUND"]["PRECISION"], $field["ROUND"]["MODE"] );
                }
                else{
                    $templateValues[$fieldIndex] = "";
                }
            ...
            if( $field["EXPORT_ROWCATEGORY_PARENT_LIST"] == "Y" ){
                ...
                if( $DB->IsDate( $templateValues[$fieldIndex] ) && ( $this->profile["DATEFORMAT"] == $this->baseDateTimePatern ) ){
                    $templateValues[$fieldIndex] = CAcritExportTools::RoundNumber( CAcritExportTools::GetYandexDateTime( $templateValues[$fieldIndex] ), $field["ROUND"]["PRECISION"], $field["ROUND"]["MODE"] );
                    ...
                    if( stripos( $templateValues[$fieldIndex], $dateTimeFormattedValue ) !== false ){
                        $skipElement = true;
                        $this->log->AddMessage( "{$arItem["NAME"]} (ID:{$arItem["ID"]}) : ".str_replace( "#FIELD#", $fieldIndex, GetMessage( "ACRIT_EXPORT_REQUIRED_FIELD_SKIP" ) ) );
                        $this->log->IncProductError();
                    }
                }

                if( $bCsvMode ){
                    $templateValues = CAcritExportStringProcess::ProcessTagOptions(
                        $templateValues,
                        $field,
                        $fieldIndex
                    );
        ...
        if( !$bCsvMode ){
            ...
            if( !$skipElement ){
                ...
                CAcritExportTools::SaveSections( $this->profile, $arItemSections );
                $this->DemoCountInc();

                if( isset( $arItemConfig["DELAY_FLUSH"] ) && ( $arItemConfig["DELAY_FLUSH"] === true ) ){
                    CAcritExportExport::Save( $itemTemplate.$this->delay );
                    $this->delay = "";
                }
                ...
                elseif( isset( $arItemConfig["DELAY"] ) && ( $arItemConfig["DELAY"] === true ) ){
                    ...
                    CAcritExportExport::Save( $itemTemplate );
                    $this->log->IncProductExport();
                }
            }
    ...
    protected function PrepareIBlock(){
        ...
        if(
            ...
            && ( CAcritExportTools::ArrayValidate( $this->profile["IBLOCK_ID"] ) )
        ){
    ...
    private function GetElementProperties( $arElement ){
        ...
        $arSectionUserFields = CAcritExportTools::GetIblockUserFields( $arItem["IBLOCK_ID"] );
        ...
        if( count( $this->useProperties["ID"] ) ){
            $arProperties = CAcritExportTools::GetProperties( $arItem, array( "ID" => $this->useProperties["ID"] ) );
            ...
            foreach( $arProperties as $property ){
                ...
                elseif( $this->GetResolveProperties( $property, $property["ID"], "PROPERTIES" ) ){
                    ...
                    $property = CIBlockFormatProperties::GetDisplayValue( $arItem, $property, "acrit_export_event" );
