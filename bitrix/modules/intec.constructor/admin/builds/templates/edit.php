...
$data['layoutCode'] = $template->layoutCode;
...
<?= Html::beginForm('', 'post', array('id' => 'page')) ?>
    ...
    <?php $tabs->BeginNextTab() ?>
        ...
        <tr>
            <td width="40%"><?= $template->getAttributeLabel('layoutCode') ?>:</td>
            <td width="60%">
                <?= Html::dropDownList($template->formName().'[layoutCode]', $template->layoutCode, ArrayHelper::merge(
                    array('' => GetMessage('template.fields.theme.unset')),
                    ArrayHelper::map($layouts, 'code', 'name')
                ), array(
                    'data-bind' => '{ value: layoutCode }'
                )) ?>
            </td>
        </tr>
        <tr>
    ...
    <script type="text/javascript">
        (function ($, api) {
            ...
            model.layoutCode = ko.observable(data.layoutCode);
