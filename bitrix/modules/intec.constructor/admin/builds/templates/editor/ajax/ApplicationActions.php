...
class ApplicationActions extends Actions
{
    public function actionSave()
    {
        ...
        if (!Type::isArray($data['containers']))
            $data['containers'] = [];

        if (!Type::isArray($data['settings']))
            $data['settings'] = [];
        ...
        $saveContainer = function ($data, $parent = null) use (
            ...
            $zone = ArrayHelper::getValue($data, 'zone');
            ...
            if (!empty($parent)) {
                ...
                if (!empty($zone) || Type::isNumeric($zone)) {
                    if (empty($layoutZoneLink)) {
                        $layoutZoneLink = new ContainerLayoutZoneLink();
                        $layoutZoneLink->containerId = $container->id;
                    }

                    $layoutZoneLink->layoutZoneCode = $zone;
                    $layoutZoneLink->save();
                } else if (!empty($layoutZoneLink)) {
                    $layoutZoneLink->delete();
                }
            }
        ...
        foreach ($data['containers'] as $dataContainer)
            if (Type::isArray($dataContainer))
                $saveContainer($dataContainer);
        ...
        $this->template->settings = $data['settings'];
        $this->template->save();
