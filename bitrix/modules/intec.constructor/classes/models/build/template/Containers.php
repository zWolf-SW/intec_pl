...
class Containers extends ActiveRecords
{
     ...
     * @param string $layoutZoneCode Код зоны разметки.
     * @return Container|null
     */
    public function getRootContainer($storage = null, $layoutZoneCode = null) {
        /** @var Container[] $containers */
        $containers = $this->asArray();
        /** @var Container $container */
        $container = null;

        if ($layoutZoneCode === null)
            $layoutZoneCode = 'default';
        ...
        foreach ($containers as $container) {
            ...
            if ($link === null) {
                ...
                if (empty($layoutZoneLink)) {
                    if ($layoutZoneCode === 'default')
                        break;
                } else if ($layoutZoneLink->layoutZoneCode === $layoutZoneCode) {
                    break;
                }
            }
     ...
     * @return Container|null
     */
    public function getTree($build = null, $storage = null, $layoutZoneCode = null)
    {
        $containers = clone $this;
        $containers->indexBy('id');
        $container = $this->getRootContainer($storage, $layoutZoneCode);
