...
class Container extends ActiveRecord implements Exchangeable, ConditionsInterface
{
    ...
    public function afterDelete()
    {
        ...
        $layoutZoneLink = $this->getLayoutZoneLink(true);
    ...
    /**
     * Реляция. Возвращает привязку контейнера к зоне разметки.
     * @param bool $result
     * @return ContainerLayoutZoneLink|ActiveQuery|null
     */
    public function getLayoutZoneLink($result = false)
    {
        return $this->relation(
            'layoutZoneLink',
            $this->hasOne(ContainerLayoutZoneLink::className(), ['containerId' => 'id']),
            $result
        );
    }
    ...
    {
        $zone = $this->getLayoutZoneLink(true);
        ...
        $structure['zone'] = $zone ? $zone->layoutZoneCode : null;
