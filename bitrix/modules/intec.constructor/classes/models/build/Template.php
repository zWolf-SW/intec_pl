...
use intec\constructor\models\build\template\ContainerLayoutZoneLink;
...
class Template extends ActiveRecord implements Exchangeable, ConditionsInterface
{
    ...
    public function rules()
    {
        return [
            ...
            [['code', 'layoutCode', 'themeCode', 'name'], 'string', 'max' => 255],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'layoutCode' => GetMessage('intec.constructor.models.build.attributes.labels.layoutCode'),
    ...
    /**
     * Возвращает текущую разметку шаблона.
     * @param boolean $reset
     * @return Layout|null
     */
    public function getLayout($reset = false)
    {
        ...
        if (empty($this->layoutCode) && !Type::isNumeric($this->layoutCode))
            return $result;

        $build = $this->getBuild(true);

        if (!empty($build)) {
            $layouts = $build->getLayouts($reset);

            if (isset($layouts[$this->layoutCode]))
                $result = $layouts[$this->layoutCode];
        }
    ...
    {
        $result = $this->toArray();
        $result['layout'] = $result['layoutCode'];
        $result['theme'] = $result['themeCode'];

        unset($result['id']);
        unset($result['buildId']);
        unset($result['layoutCode']);
        ...
        if (!empty($layout))
            $zones = $layout->getZones()->getKeys();

        $containers = $this->getContainers()
            ->with(['link', 'layoutZoneLink', 'component', 'widget', 'block', 'variator', 'variator.variants'])
            ->all();
        /** @var Containers $containers */

        foreach ($zones as $zoneCode) {
            /** @var Container $container */
            $container = $containers->getTree(null, $this, $zoneCode);

            if (!empty($container)) {
                $containerIndexes = $indexes !== null ? [] : null;
                $container = $container->export($containerIndexes);
                $container['zone'] = $zoneCode;
    ...
    public function import($data, &$indexes = null)
    {
        ...
        $data['layoutCode'] = ArrayHelper::getValue($data, 'layout');
        ...
        if (Type::isArray($dataContainer)) {
            $dataContainer['zone'] = 'default';
            $dataContainers = [$dataContainer];
        }

        unset($dataContainer);

        if (Type::isArray($dataContainers)) {
            foreach ($dataContainers as $dataContainer) {
                if (empty($dataContainer['zone']))
                    continue;
                ...
                if ($container->import($dataContainer, $containerIndexes)) {
                    ...
                    $layoutZoneLink = $container->getLayoutZoneLink(true);

                    if (!empty($link))
                        $link->delete();

                    if (empty($layoutZoneLink)) {
                        $layoutZoneLink = new ContainerLayoutZoneLink();
                        $layoutZoneLink->containerId = $container->id;
                    }

                    $layoutZoneLink->layoutZoneCode = $dataContainer['zone'];
                    $layoutZoneLink->save();
                } else {
                    $container = null;
                }
