...
class Build extends ActiveRecord implements Exchangeable
{
    ...
    public function getDirectory($old = false, $relative = false, $separator = DIRECTORY_SEPARATOR)
    {
        ...
        $path = null;
        $local = FileHelper::isDirectory('@root/local/templates/'.($old ?
        ...
        if ($local) {
            if (!$relative) {
                $path = Core::getAlias('@root/local/templates/'.($old ?
                    $this->getOldAttribute('code') :
                    $this->getAttribute('code')
                ));
            } else {
                $path = '/local/templates/'.($old ?
                    $this->getOldAttribute('code') :
                    $this->getAttribute('code')
                );
            }
            ...
            if (!$relative) {
                $path = Core::getAlias('@templates/'.($old ?
                    $this->getOldAttribute('code') :
                    $this->getAttribute('code')
                ));
            } else {
                $path = '/bitrix/templates/'.($old ?
                    $this->getOldAttribute('code') :
                    $this->getAttribute('code')
                );
            }
