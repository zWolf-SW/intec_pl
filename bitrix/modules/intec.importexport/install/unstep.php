...
<form action="<?= $APPLICATION->GetCurPage() ?>" method="POST">
    ...
    <h3><?= Loc::getMessage('intec.importexport.install.uninstall.database.title') ?></h3>
    <div><?= Loc::getMessage('intec.importexport.install.uninstall.database.description') ?></div>
    <p>
        ...
        <label for="remove-database" class="adm-checkbox-label">
            <?= Loc::getMessage('intec.importexport.install.uninstall.database.action') ?>
        </label>
    </p>
    <div>
        <input type="submit" value="<?= Loc::getMessage('intec.importexport.install.uninstall.go')?>">
    </div>
<form>