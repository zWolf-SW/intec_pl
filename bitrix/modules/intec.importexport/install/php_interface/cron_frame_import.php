...
if(!defined("B_PROLOG_INCLUDED")) {
    ...
    require_once(dirname(__FILE__).'/../../../modules/intec.importexport/classes/models/excel/import/Import.php');
...
foreach($templateIds as $templateId) {
    ...
    $template = intec\importexport\models\excel\import\Template::findOne($templateId);
    ...
    echo date('Y-m-d H:i:s') . ": start of template import with id " . $templateId .  "\r\n";
    ...
    $import = new Import();
    $result = $import->importByCron($templateId);
    $result['error'] = $import->getErrors();
    $result['statistic'] = $import->getStatistic();

    if ($result['error']['is']) {
        echo date('Y-m-d H:i:s').": template import error \r\n";
    ...
    echo date('Y-m-d H:i:s').": template import with id " . $templateId . " completed \r\n \r\n";
}
?>