...
$list = 'importexport_import_templates';
...
$cronImportPath = '/bitrix/php_interface/include/intec.importexport/';
    ...
    if ($request->getIsPost()) {
        ...
        if ($action === 'updateCronList') {
            ...
            if (FileHelper::isEntry($_SERVER["DOCUMENT_ROOT"].$crontabPath)) {
                ...
                $matchLine = [];
                $resultCount = preg_match_all("#^\s*(([\*/,\d]+\s+){5}).*?".preg_quote($cronImportPath).$cronFrame." +(\d[\d,]*) *>.*?$#im", $crontabData, $matchLine);
            }
            ...
            if (FileHelper::isEntry($_SERVER["DOCUMENT_ROOT"].$crontabPath) && !empty($template)) {
                $crontabData = FileHelper::getFileData($_SERVER["DOCUMENT_ROOT"].$crontabPath);
                $changedData = preg_replace("#^.*?".preg_quote($cronImportPath).$cronFrame." +".preg_quote($template)." *>.*?$#im", "", $crontabData);
            ...
            if (!empty($selectedData['time']) && !empty($selectedData['template'])) {
                if (FileHelper::isEntry($_SERVER["DOCUMENT_ROOT"].$crontabPath)) {
                    $crontabData = FileHelper::getFileData($_SERVER["DOCUMENT_ROOT"].$crontabPath);
                    $changedData = preg_replace("#^\s*".preg_quote($selectedData['time'])."\s+.*?".preg_quote($cronImportPath).$cronFrame." +".preg_quote($selectedData['template'])." *>.*?$#im", "", $crontabData);
                    $changeResult = FileHelper::setFileData($_SERVER["DOCUMENT_ROOT"].$crontabPath, $changedData);
            ...
            if (!empty($template)) {
                $crontabData = FileHelper::getFileData($_SERVER["DOCUMENT_ROOT"].$crontabPath);
                $matchLine = [];
                $resultCount = preg_match_all("#^\s*".preg_quote($time)."\s+.*?".preg_quote($cronImportPath).$cronFrame." +".preg_quote($template)." *>.*?$#im", $crontabData, $matchLine);
            }
            ...
            if ($resultCount <= 0 && !empty($template)) {
                ...
                $newData = StringHelper::replaceMacros($newData, [
                    ...
                    'CRON_FRAME_PATH' => $_SERVER["DOCUMENT_ROOT"] . $cronImportPath . $cronFrame,
                    'TEMPLATE' => $template,
                    'LOG_PATH' => $_SERVER["DOCUMENT_ROOT"] . $cronImportPath . 'import_logs/' . $templateLogs . '.txt',
                ]);
...
foreach ($templates as $template) {
    ...
    $actions[] = [
        ...
        'ACTION' => $list->ActionRedirect(StringHelper::replaceMacros($arUrlTemplates['excel.import.templates.edit'], [
    ...
    /*$actions[] = [
        ...
        'ACTION' => $list->ActionRedirect(StringHelper::replaceMacros($arUrlTemplates['excel.import.templates.copy'], [
            'template' => $template->id
        ]))
    ];*/
...
<script type="text/javascript">
    ...
    function showCron () {
        ...
        JHelpers.ShowCron({}, title, 'import');
    }
</script>
