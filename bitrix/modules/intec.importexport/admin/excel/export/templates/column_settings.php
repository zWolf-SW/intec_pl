...
$properties = IBlockSelections::getAllProperties($parameters['iblock'], true, $parameters['level']);
$propertiesOptions = [];

$count = 0;
$groupId = ArrayHelper::getFirstValue($properties);

if (StringHelper::startsWith($groupId['code'], 'GROUP_')) {
    $count = $groupId['groupItemsId']['count'];
    $groupId = $groupId['id'];
}

foreach ($properties as $key => $property) {
    if ($property['code'] === 'GROUP_BASE')
        continue;

    if (!empty($groupId) || $groupId === 0) {
        $propertiesOptions[$groupId][$property['code']] = $property['name'];
        $count--;
    ...
    if (StringHelper::startsWith($property['code'], 'GROUP_')) {
        $groupId = $key;
        $count = $property['groupItemsId']['count'];
    }
}
...
<form class="m-intec-importexport p-field-settings-table" action="" method="post" enctype="multipart/form-data" name="field_settings">
                    ...
                    <td colspan="2">
                        <div class="data-wrapper">
                            ...
                            <select name="data[field][]" v-model="row.field">
                                <option value="CURRENT"><?= Loc::getMessage('option.condition.current') ?></option>
                                <?php foreach ($propertiesOptions as $key => $group) { ?>
                                    <optgroup label="<?= $properties[$key]['name'] ?>">
            ...
            var conditions = new Vue({
                ...
                methods: {
                    ...
                    'openList': function (rowIndex) {
                        ...
                        button = button[0];
                        console.log({'button': button, 'rowIndex': rowIndex});
