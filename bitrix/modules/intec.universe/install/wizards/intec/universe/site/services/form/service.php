...
$form = $import($code, [
    ...
    'name' => Loc::getMessage('wizard.services.form.fields.SERVICE.name'),
    'code' => 'SERVICE',
    'type' => 'text',
    'required' => true
], [
...
if (!empty($form)) {
    ...
    $macros['FORMS_'.$code.'_FIELDS_SERVICE_ID'] = null;

    $answer = ArrayHelper::getValue($form, ['FIELDS', 'SERVICE', 'ANSWERS', 0]);

    if (!empty($answer))
        $macros['FORMS_'.$code.'_FIELDS_SERVICE_ID'] = 'form_'.$answer['FIELD_TYPE'].'_'.$answer['ID'];
