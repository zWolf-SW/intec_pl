...
if (!empty($arCurrentValues['IBLOCK_ID'])) {
    ...
    $hPropertyNumber = function ($sKey, $arProperty) {
        if ($arProperty['PROPERTY_TYPE'] === 'N' && $arProperty['MULTIPLE'] === 'N' || $arProperty['PROPERTY_TYPE'] === 'S'&& $arProperty['MULTIPLE'] === 'N')
            return [
                'key' => $arProperty['CODE'],
    ...
    $hPropertyList = function ($sKey, $arProperty) {
        if ($arProperty['PROPERTY_TYPE'] === 'L' && $arProperty['MULTIPLE'] === 'N' )
            return [
                'key' => $arProperty['CODE'],
                'value' => '['.$arProperty['CODE'].'] '.$arProperty['NAME']
            ];
    ...
    $hPropertyList = $arProperties->asArray($hPropertyList);
...
if (!empty($arCurrentValues['PROPERTY_PRICE'])) {
    ...
    $arTemplateParameters['PRICE_OLD_SHOW'] = [
        'PARENT' => 'VISUAL',
        'NAME' => Loc::getMessage('C_MAIN_SERVICES_TEMPLATE_2_PRICE_OLD_SHOW'),
        'TYPE' => 'CHECKBOX',
        'DEFAULT' => 'N',
        'REFRESH' => 'Y'
    ];
    $arTemplateParameters['PROPERTY_CURRENCY'] = [
        'PARENT' => 'DATA_SOURCE',
        'NAME' => Loc::getMessage('C_MAIN_SERVICES_TEMPLATE_2_PROPERTY_CURRENCY'),
        'TYPE' => 'LIST',
        'VALUES' => $hPropertyList,
        'ADDITIONAL_VALUES' => 'Y'
    ];
    $arTemplateParameters['PROPERTY_FORMAT_USE'] = [
        'PARENT' => 'VISUAL',
        'NAME' => Loc::getMessage('C_MAIN_SERVICES_TEMPLATE_2_PROPERTY_FORMAT_USE'),
        'TYPE' => 'CHECKBOX',
        'DEFAULT' => 'N',
        'REFRESH' => 'Y'
    ];
    if($arCurrentValues['PROPERTY_FORMAT_USE'] === 'Y'){
        $arTemplateParameters['PROPERTY_FORMAT'] = [
            'PARENT' => 'DATA_SOURCE',
            'NAME' => Loc::getMessage('C_MAIN_SERVICES_TEMPLATE_2_PROPERTY_FORMAT'),
            'TYPE' => 'LIST',
            'VALUES' => $arPropertyText,
            'ADDITIONAL_VALUES' => 'Y',
            'REFRESH' => 'Y'
...
if($arCurrentValues['PRICE_OLD_SHOW'] === 'Y'){
    $arTemplateParameters['PROPERTY_PRICE_OLD'] = [
        'PARENT' => 'DATA_SOURCE',
        'NAME' => Loc::getMessage('C_MAIN_SERVICES_TEMPLATE_2_PROPERTY_PRICE_OLD'),
        'TYPE' => 'LIST',
        'VALUES' => $arPropertyText,
        'ADDITIONAL_VALUES' => 'Y',
        'REFRESH' => 'Y'
