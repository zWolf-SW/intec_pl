    ...
    $fHandle = function (&$arItem, $bOffer = false) use (&$arResult, &$arVisual) {
        ...
        if (!$bOffer) {
            ...
            if (!empty($arItem['PREVIEW_PICTURE'])) {
                $arData['img'] = $arItem['PREVIEW_PICTURE'];
            } else if (!empty($arItem['DETAIL_PICTURE'])) {
                $arData['img'] = $arItem['DETAIL_PICTURE'];
            }

            if (!empty($arData['img']) && Type::isString($arData['img'])) {
                $arData['img'] = Arrays::fromDBResult(CFile::GetByID($arData['img']))->asArray();
                $arData['img'] = $arData['img']['SRC'];
            } else if (ArrayHelper::keyExists('SRC', $arData['img'])) {
                $arData['img'] = $arData['img']['SRC'];
            }
