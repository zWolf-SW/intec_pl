...
if (
    ...
    if (!defined('EDITOR')) {
        ...
        $arParams['SEARCH_TIPS_VIEW'] = $properties->get('header-search-popup-template');
        $arParams['AUTHORIZATION_FORM_USE_REGISTRATION'] = $properties->get('header-registration-use') ? 'Y' : 'N';

        /** Параметры корзины */
        $arParams['BASKET_POPUP'] = $properties->get('basket-header-popup-show') ? 'Y' : 'N';
        ...
        if ($arParams['DESKTOP'] === 'template.8' && !ArrayHelper::isIn($arParams['MENU_MAIN_SECTION_VIEW'], ['default', 'images']))
        ...
        if (!empty($arParams['BANNER'])) {
            $arParams['BANNER_MOBILE_BLOCK_SEPARATED'] = $properties->get('header-mobile-banner-view') === 'apart' ? 'Y' : 'N';
