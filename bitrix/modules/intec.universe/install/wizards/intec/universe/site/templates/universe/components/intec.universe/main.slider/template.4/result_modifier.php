...
if (!empty($arParams['BANNER_PRODUCTS_PROPERTY_SHOW_START'])
    || !empty($arParams['BANNER_PRODUCTS_PROPERTY_SHOW_END'])
    || !empty($arParams['BANNER_PRODUCTS_PROPERTY_SHOW_END_DAY'])) {
    $sDate = date('Y-m-d');

    $arFilter['IBLOCK_ID'] = $arParams['BANNER_PRODUCTS_IBLOCK_ID'];

    if ($arParams['BANNER_PRODUCTS_MODE'] === 'period') {
        $arFilter['<PROPERTY_' . $arParams['BANNER_PRODUCTS_PROPERTY_SHOW_START']] = $sDate;
        $arFilter['>PROPERTY_' . $arParams['BANNER_PRODUCTS_PROPERTY_SHOW_END']] = $sDate;
    } else {
        $arFilter['=PROPERTY_' . $arParams['BANNER_PRODUCTS_PROPERTY_SHOW_END_DAY']] = $sDate;
    }
    ...
    $arBannerProducts = new ElementsQuery([
        'filter' => $arFilter,
        'select' => $arSelect
    ]);

    $arBannerProducts = $arBannerProducts
        ->setWithProperties(false)
        ->execute()
        ->indexBy('ID')
        ->asArray();

    unset($sDate, $arFilter, $arSelect);
}
...
$arVisual = [
    ...
    'MOBILE' => [
        ...
        'BANNER' => [
            'SEPARATED' => [
                'USE' => $arParams['MOBILE_BANNER_SEPARATED_TEXT_BLOCK_USE'] === 'Y',
                'LEFT' => $arParams['MOBILE_BANNER_SEPARATED_TEXT_BLOCK_USE'] === 'Y' && $arParams['MOBILE_BANNER_SEPARATED_TEXT_BLOCK_ALIGN_LEFT'] === 'Y',
            ]
        ]
    ],
    'VIDEO' => [
    ...
    'SLIDER' => [
        'NAV' => [
            ...
            'VIEW' => ArrayHelper::fromRange([1, 2], $arParams['SLIDER_NAV_VIEW'])
        ],
        'DOTS' => [
            'SHOW' => $arParams['SLIDER_DOTS_SHOW'] === 'Y',
            'VIEW' => ArrayHelper::fromRange([1, 2, 3], $arParams['SLIDER_DOTS_VIEW'])
        ],
    ...
    'BANNER_PRODUCTS' => [
        'SHOW' => $arParams['BANNER_PRODUCTS_USE'] === 'Y' && !empty($arParams['BANNER_PRODUCTS_IBLOCK_ID']) && !empty($arBannerProducts)
    ]
];

unset($arBannerProducts);
...
if (empty($arForm['BUTTON']))
    $arForm['BUTTON'] = Loc::getMessage('C_MAIN_SLIDER_TEMPLATE_4_FORM_BUTTON_TEXT_DEFAULT');
