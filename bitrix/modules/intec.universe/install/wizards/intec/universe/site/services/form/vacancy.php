...
$form = $import($code, [
    ...
    'name' => Loc::getMessage('wizard.services.form.fields.POST.name'),
    'code' => 'POST',
    'type' => 'text',
    'required' => true
], [
    'name' => Loc::getMessage('wizard.services.form.fields.FILE.name'),
    'code' => 'FILE',
    'type' => 'file',
    'required' => false
]], [[
...
if (!empty($form)) {
    ...
    $macros['FORMS_'.$code.'_FIELDS_POST_ID'] = null;

    $answer = ArrayHelper::getValue($form, ['FIELDS', 'POST', 'ANSWERS', 0]);

    if (!empty($answer))
        $macros['FORMS_'.$code.'_FIELDS_POST_ID'] = 'form_'.$answer['FIELD_TYPE'].'_'.$answer['ID'];
