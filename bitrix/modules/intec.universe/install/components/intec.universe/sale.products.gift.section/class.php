...
class SaleProductsGiftComponentSection extends ElementList
{
    ...
    public function __construct($component = null)
    {
        ...
        $this->setExtendedMode(true)->setMultiIblockMode(true)->setPaginationMode(true);
    }
    ...
    public function onPrepareComponentParams($params)
    {
        ...
        $params['POTENTIAL_PRODUCT_TO_BUY']['ELEMENT'] = array(
            'ID' => $params['POTENTIAL_PRODUCT_TO_BUY']['ELEMENTS_ID']
        );
        ...
        {
            $params['POTENTIAL_PRODUCT_TO_BUY']['ELEMENTS_ID'] = $params['POTENTIAL_PRODUCT_TO_BUY']['PRIMARY_OFFER_ID'];
        }
    ...
    protected function getGiftCollections()
    {
        ...
        if (!empty($this->arParams['POTENTIAL_PRODUCT_TO_BUY']))
        {
            ...
            $arElements = [];
            $arIds = $this->arParams['POTENTIAL_PRODUCT_TO_BUY']['ELEMENTS_ID'];

            foreach ($arIds as $id) {
                $arCollectionParams = [
                    'ID' => $id,
                    'MODULE' => $this->arParams['POTENTIAL_PRODUCT_TO_BUY']['MODULE'],
                    'PRODUCT_PROVIDER_CLASS' => $this->arParams['POTENTIAL_PRODUCT_TO_BUY']['PRODUCT_PROVIDER_CLASS'],
                    'QUANTITY' => $this->arParams['POTENTIAL_PRODUCT_TO_BUY']['QUANTITY'],
                ];
                $arElements[$id]= $this->giftManager->getCollectionsByProduct(
                    \Bitrix\Sale\Basket::loadItemsForFUser(\Bitrix\Sale\Fuser::getId(), $this->getSiteId()), $arCollectionParams
                );
            }

            $collections = $arElements;
        }
    ...
    {
        if (empty($this->arParams['POTENTIAL_PRODUCT_TO_BUY']['ELEMENTS_ID']))
        {
            return array();
        }

        \Bitrix\Sale\Compatible\DiscountCompatibility::stopUsageCompatible();
        $arCollections = $this->getGiftCollections();
        \Bitrix\Sale\Compatible\DiscountCompatibility::revertUsageCompatible();

        $productIds = array();

        foreach ($arCollections as $collections) {
