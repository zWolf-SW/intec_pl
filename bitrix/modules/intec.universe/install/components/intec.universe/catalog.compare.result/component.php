			...
			if (isset($_REQUEST[$arParams['PRODUCT_ID_VARIABLE']]))
			{
				...
				if ($productID > 0 && !isset($_SESSION[$arParams["NAME"]][$arParams["IBLOCK_ID"]]["ITEMS"][$productID]))
				{
					...
					if ($bBase) {
                        ...
                        $arOffers = Arrays::fromDBResult(CStartShopCatalog::GetByIBlock($arParams["IBLOCK_ID"]))->getFirst();
                        $arOffers = [
                            'OFFERS_IBLOCK_ID' => $arOffers['OFFERS_IBLOCK'],
                            'OFFERS_PROPERTY_ID' => $arOffers['OFFERS_LINK_PROPERTY']
                        ];
                    }
