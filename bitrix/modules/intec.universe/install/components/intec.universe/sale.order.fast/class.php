    ...
    public function executeComponent()
    {
        ...
        if (!empty($arResult['PRODUCT'])) {
            ...
            if (!empty($arResult['PRICE'])) {
                ...
                if ($this->getIsBase()) {
                    $arResult['PRICE']['BASE']['VALUE'] = $arResult['PRICE']['BASE']['VALUE'] / $arResult['CURRENCY']['RATE'];
                    $arResult['PRICE']['DISCOUNT']['VALUE'] = $arResult['PRICE']['DISCOUNT']['VALUE'] / $arResult['CURRENCY']['RATE'];

                    $arResult['PRICE']['BASE']['DISPLAY'] = CCurrencyLang::CurrencyFormat(
                        $arResult['PRICE']['BASE']['VALUE'],
                        $arResult['CURRENCY']['CODE']
                    );
        ...
        if ($arResult['ACTION'] === 'apply' && $arResult['AVAILABLE'] && empty($arResult['ERROR'])) {
            if (empty($arUser)) {
                ...
                $arUserExists = CUser::GetList($by = 'sort', $order = 'asc', [
                    'EMAIL' => $arUser['email']
                ])->Fetch();

                $bUserExists = !empty($arUserExists);

                if ($bUserExists && COption::GetOptionString('main', 'new_user_email_uniq_check', 'N') == 'Y') {
                    $arUser['ID'] = $arUserExists['ID'];
                } else {
                    ...
                    if (COption::GetOptionString('main', 'new_user_phone_required', 'N') == 'Y') {
                        $arUser = $USER->Register(
                            ...
                            0,
                            false,
                            (!empty($arUser['phone']))? $arUser['phone'] : ''
                        );
                    } else {
                        $arUser = $USER->Register(
                            $arUser['login'],
            ...
            if ($this->getIsBase()) {
                ...
                if (!empty($arProduct)) {
                    ...
                    $fQuantity = $arOrder['ITEMS'][$arProduct['ID']]['QUANTITY'] < 0 ? 1 : Type::toFloat($arOrder['ITEMS'][$arProduct['ID']]['QUANTITY']);
                    $fOrderSum = $arOrder['ITEMS'][$arProduct['ID']]['PRICE'] * $fQuantity;
                } else {
