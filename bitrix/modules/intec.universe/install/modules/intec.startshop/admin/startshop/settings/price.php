...
<?
    ...
    $arLinks = array(
        'ADD' => "startshop_settings_price_edit.php?lang=".LANG."&action=add",
        'EDIT' => "startshop_settings_price_edit.php?lang=".LANG."&action=edit&ID=#ID#",
        'DELETE' => "startshop_settings_price.php?lang=".LANG."&action=delete&ID=#ID#",
        'ACTIVATE' => "startshop_settings_price.php?lang=".LANG."&action=activate&ID=#ID#",
        'DEACTIVATE' => "startshop_settings_price.php?lang=".LANG."&action=deactivate&ID=#ID#",
        'BASE' => "startshop_settings_price.php?lang=".LANG."&action=base&ID=#ID#",
    );
    ...
    if ($bRightsEdit) {
        if ($sAction == 'delete' && is_numeric($_REQUEST['ID']))
            CStartShopPrice::Delete($_REQUEST['ID']);

        if ($sAction == 'activate' && is_numeric($_REQUEST['ID']))
            CStartShopPrice::Update($_REQUEST['ID'], array("ACTIVE" => "Y"));

        if ($sAction == 'deactivate' && is_numeric($_REQUEST['ID']))
            CStartShopPrice::Update($_REQUEST['ID'], array("ACTIVE" => "N"));

        if ($sAction == 'base' && is_numeric($_REQUEST['ID']))
            CStartShopPrice::SetBase($_REQUEST['ID']);

        if (is_array($_REQUEST['ID'])) {
            if ($sAction == 'delete.selected')
                foreach ($_REQUEST['ID'] as $iItemID)
                    CStartShopPrice::Delete($iItemID);

            if ($sAction == 'activate.selected')
                foreach ($_REQUEST['ID'] as $iItemID)
                    CStartShopPrice::Update($iItemID, array("ACTIVE" => "Y"));

            if ($sAction == 'deactivate.selected')
                foreach ($_REQUEST['ID'] as $iItemID)
                    CStartShopPrice::Update($iItemID, array("ACTIVE" => "N"));
        }
    }

    $sListID = "startshop_price_list";
    ...
    $dbResult = CStartShopPrice::GetList(array($sSortBy => $sOrderBy), array());
