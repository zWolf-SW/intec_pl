...
class ActiveRecord extends ActiveRecordBase
{
    ...
    /**
     * Удаляет записи из базы частями.
     * @param ActiveQuery $query Запрос.
     * @param integer $limit Ограничение на 1 итерацию.
     * @throws \Exception
     */
    public static function deletePartial($query, $limit = 100)
    {
    ...
    /**
     * Переменная-флаг для переключения кеширования.
     * @var boolean
     */
    ...
    {
        if (!static::$isCacheEnabled || !Type::isArray(static::$cache))
            return false;
    ...
    /**
     * Возвращает значение, указывающее на статус кеширования.
     * @return boolean
     */
    ...
    /**
     * Переключает кеширование.
     */
    public static function toggleCache()
    {
        static::$isCacheEnabled = !static::$isCacheEnabled;
    }
