...
class Collection extends BaseObject implements \IteratorAggregate, \ArrayAccess, \Countable, \Serializable
{
     ...
     * @return $this
     */
    public function addUnique($item, $strict = true)
    {
        if (!$this->has($item, $strict))
            $this->add($item);
    ...
    /**
     * Добавляет уникальные элементы в коллекцию.
     * @param array|Collection $items
     * @param boolean $strict
     * @return $this
     */
    public function addUniqueRange($items, $strict = true)
    {
        if (Type::isArrayable($items)) {
            foreach ($items as $item)
                $this->addUnique($item, $strict);
        }
    ...
    /**
     * Модифицирует добавляемое в коллекцию.
     * @param mixed $item
     */
    protected function modify(&$item)
    {
