...
/**
 * Класс для работы с списком инфоблоков
 * @package intec\core\platform\component\parameters
 * @author imber228@gmail.com
 */
class IBlocks extends Collection
{
    /**
     * Формирует индексированный объект списка инфоблоков по заданному фильтру
     * @param array $filter Фильтр выборки инфоблоков
     * @param array $sort Сортировка выборки
     * @param string $indexBy Ключ массива инфоблока для индексации списка
     * @return IBlocks
     */
    public static function getList($filter = [], $sort = [], $indexBy = 'ID')
    {
        ...
        $iBlocks = CIBlock::GetList($sort, $filter);
        $result = [];

        while ($iBlock = $iBlocks->GetNext(false, false)) {
            if (ArrayHelper::keyExists($indexBy, $iBlock))
                $result[$iBlock[$indexBy]] = $iBlock;
            else
    ...
    /**
     * Возвращает форматированный массив-список инфоблоков
