...
use intec\Core;
use intec\core\base\ErrorException;
...
class FileHelperBase
{
     ...
     */
    public static $mimeMagicFile = '@intec/core/helpers/mimeTypes.php';
     ...
     * @deprecated Use \intec\core\io\Path::normalize instead.
     */
    ...
    {
        if ($language === null) {
            $language = Core::$app->language;
        }
        if ($sourceLanguage === null) {
            $sourceLanguage = Core::$app->sourceLanguage;
        }
     ...
     * @throws \intec\core\base\InvalidParamException if unable to open directory
     */
     ...
     * @throws \intec\core\base\Exception if the directory could not be created (i.e. php error due to parallel changes)
     */
    public static function createDirectory($path, $mode = 0775, $recursive = true)
    {
        ...
        try {
            ...
            if (!is_dir($path)) {// https://github.com/yiisoft/yii2/issues/9288
                throw new \intec\core\base\Exception("Failed to create directory \"$path\": " . $e->getMessage(), $e->getCode(), $e);
            }
        }
        try {
            return chmod($path, $mode);
        } catch (\Exception $e) {
            throw new \intec\core\base\Exception("Failed to change permissions for directory \"$path\": " . $e->getMessage(), $e->getCode(), $e);
        }
    }
     ...
     * @throws \intec\core\base\InvalidParamException
    ...
    /**
     * Возвращает вхождения файловой системы.
     * @param string $path
     * @param bool $special
     * @param string $ds
     * @return array
     */
    public static function getDirectoryEntries($path, $special = true, $ds = DIRECTORY_SEPARATOR)
    {
        $result = [];
        $path = static::normalizePath($path, $ds);

        if (static::isDirectory($path)) {
            $entries = scandir($path);

            foreach ($entries as $entry) {
                if (!$special && ($entry == '.' || $entry == '..'))
                    continue;
     ...
     * @return bool
     */
    public static function setFileData($path, $data = null, $append = false, $ds = DIRECTORY_SEPARATOR)
    {
        $path = static::normalizePath($path, $ds);

        if (!static::isFile($path))
            if (static::isSymlink($path))
                return false;

        $directory = static::getEntryDirectory($path);

        if (!static::isDirectory($directory))
            if (!static::createDirectory($directory, 0775, true))
                return false;

        return (bool)file_put_contents($path, $data, $append ? FILE_APPEND : null);
    }
     ...
     * @return bool
     */
    public static function isFile($path, $ds = DIRECTORY_SEPARATOR)
    {
        $path = static::normalizePath($path, $ds);
        return is_file($path);
    }
    ...
    /**
     * Проверяет на возможность редактирования.
     * @param string $path
     * @param string $ds
     * @return bool
     */
    public static function isWritable($path, $ds = DIRECTORY_SEPARATOR)
    {
        $path = static::normalizePath($path, $ds);
    ...
    /**
     * Проверяет на возможность чтения.
     * @param string $path
     * @param string $ds
     * @return bool
     */
    public static function isReadable($path, $ds = DIRECTORY_SEPARATOR)
    {
        $path = static::normalizePath($path, $ds);
    ...
    /**
     * Проверяет на возможность выполнения.
     * @param string $path
     * @param string $ds
     * @return bool
     */
    public static function isExecutable($path, $ds = DIRECTORY_SEPARATOR)
    {
        $path = static::normalizePath($path, $ds);
     ...
     * @return bool
     */
    public static function isSymlink($path, $ds = DIRECTORY_SEPARATOR)
    {
        $path = static::normalizePath($path, $ds);
        return is_link($path);
    }
     ...
     * @return bool
     */
    public static function isDirectory($path, $ds = DIRECTORY_SEPARATOR)
    {
        $path = static::normalizePath($path, $ds);
        return is_dir($path);
    }
     ...
     * @return bool
     */
    public static function isEntry($path, $ds = DIRECTORY_SEPARATOR)
    {
        $path = static::normalizePath($path, $ds);
    ...
    /**
     * Возвращает директорию файла, директории или ссылки.
     * @param string $path
     * @param string $ds
     * @return string
     */
    public static function getEntryDirectory($path, $ds = DIRECTORY_SEPARATOR)
    {
        $path = static::normalizePath($path, $ds);
    ...
    /**
     * Возвращает название файла, дериктории или ссылки.
     * @param string $path
     * @param string $ds
     * @return string
     */
    public static function getEntryName($path, $ds = DIRECTORY_SEPARATOR)
    {
        $path = static::normalizePath($path, $ds);
    ...
    /**
     * Возвращает тип вхождения.
     * @param string $path
     * @param string $ds
     * @return bool|int
     */
    public static function getEntryType($path, $ds = DIRECTORY_SEPARATOR)
    {
        $path = static::normalizePath($path, $ds);

        if (!static::isEntry($path))
            return false;

        if (static::isFile($path))
            return static::TypeFile;

        if (static::isDirectory($path))
            return static::TypeDirectory;

        if (static::isSymlink($path))
            return static::TypeSymlink;
     ...
     * @return string
     */
    public static function getFileNameWithoutExtension($path, $ds = DIRECTORY_SEPARATOR)
    {
        $path = static::normalizePath($path, $ds);
     ...
     * @return string
     */
    public static function getFileExtension($path, $ds = DIRECTORY_SEPARATOR)
    {
        $path = static::normalizePath($path, $ds);
        return pathinfo(Type::toString($path), PATHINFO_EXTENSION);
    }

    public static function getPathParts($path, $ds = DIRECTORY_SEPARATOR)
    {
        $path = static::normalizePath($path, $ds);
        $parts = pathinfo(Type::toString($path));
        $result = [];

        $result['directory'] = ArrayHelper::getValue($parts, 'dirname');
        $result['name'] = [
            'base' => ArrayHelper::getValue($parts, 'basename'),
            'less' => ArrayHelper::getValue($parts, 'filename')
        ];
