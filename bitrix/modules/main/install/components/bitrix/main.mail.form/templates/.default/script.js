...
;(function() {
	...
	BXMainMailForm.prototype.insertCalendarSharingLink = function()
	{
		...
		BX.ajax.runComponentAction(
				...
				&& Object.hasOwn(response.data, 'isSharingFeatureEnabled'))
			{
				...
				{
					this.insertCalendarSharingMessage(response.data.sharingUrl);
				}
				else
				{
	...
	BXMainMailForm.prototype.insertCalendarSharingMessage = function(sharingLink)
	{
		const sharingText = this.getCalendarSharingText(sharingLink);
		...
		if (this.userSelection === this.editor.GetIframeDoc().body)
		{
			...
			{
				range.endContainer.parentElement.after(sharingText);
			}
			else
			{
				this.editor.selection.InsertNode(sharingText, range);
			}
		}
		else
		{
			...
			if (signatureNode)
			{
				signatureNode.before(sharingText);
			}
			else
			{
	...
	BXMainMailForm.prototype.getCalendarSharingText = function(sharingLink)
	{
		return BX.Tag.render`
			<div>
				<span>${BX.Loc.getMessage('MAIN_MAIL_FORM_EDITOR_CALENDAR_SHARING_TEXT')} </span>
				<a class="${this.sharingLinkNodeClass}" href="${sharingLink}">${sharingLink}</a>
			</div>
		`;
	};
