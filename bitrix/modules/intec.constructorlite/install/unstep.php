...
<form action="<?= $APPLICATION->GetCurPage() ?>" method="POST">
    ...
    <h3><?= Loc::getMessage('intec.constructorlite.install.uninstall.database.title') ?></h3>
    <div><?= Loc::getMessage('intec.constructorlite.install.uninstall.database.description') ?></div>
    <p>
        ...
        <label for="remove-database" class="adm-checkbox-label">
            <?= Loc::getMessage('intec.constructorlite.install.uninstall.database.remove') ?>
        </label>
    </p>
    <div>
        <input type="submit" value="<?= Loc::getMessage('intec.constructorlite.install.uninstall.go')?>">
    </div>
<form>