...
<?php $vQuantity = function (&$arItem) use (&$arVisual) { ?>
    <?php $fRender = function (&$arItem, $bOffer = false) use (&$arVisual) { ?>
        ...
        <?= Html::beginTag('div', [
            'class' => 'catalog-products-viewed-item-quantity',
            'data' => [
            ...
            <?php if ($arItem['CAN_BUY']) { ?>
                <span class="catalog-products-viewed-item-quantity-icon catalog-products-viewed-item-quantity-check">
                    <i class="far fa-check"></i>
                </span>
                <?php if ($arVisual['QUANTITY']['MODE'] === 'number') { ?>
                    <?php if ($arItem['CATALOG_QUANTITY'] > 0) { ?>
                        <?= Loc::getMessage('C_CATALOG_PRODUCTS_VIEWED_TILE_2_TEMPLATE_QUANTITY_AVAILABLE').': ' ?>
                ...
                <?php } else if ($arVisual['QUANTITY']['MODE'] === 'text') {?>
                    ...
                    <?php if ($arItem['CATALOG_QUANTITY'] >= $arVisual['QUANTITY']['BOUNDS']['MANY'] || $arItem['CATALOG_QUANTITY'] <= 0) { ?>
                        <?= Loc::getMessage('C_CATALOG_PRODUCTS_VIEWED_TILE_2_TEMPLATE_QUANTITY_BOUNDS_MANY') ?>
                    <?php } else if ($arItem['CATALOG_QUANTITY'] <= $arVisual['QUANTITY']['BOUNDS']['FEW']) { ?>
                        <?= Loc::getMessage('C_CATALOG_PRODUCTS_VIEWED_TILE_2_TEMPLATE_QUANTITY_BOUNDS_FEW') ?>
                    <?php } else if ($arItem['CATALOG_QUANTITY'] >= $arVisual['QUANTITY']['BOUNDS']['FEW'] && $arItem['CATALOG_QUANTITY'] <= $arVisual['QUANTITY']['BOUNDS']['MANY']) { ?>
                        <?= Loc::getMessage('C_CATALOG_PRODUCTS_VIEWED_TILE_2_TEMPLATE_QUANTITY_BOUNDS_ENOUGH') ?>
                ...
                <?php } ?>
            <?php } else { ?>
                <span class="catalog-products-viewed-item-quantity-icon catalog-products-viewed-item-quantity-times">
                    <i class="far fa-times"></i>
                </span>
                <?= Loc::getMessage('C_CATALOG_PRODUCTS_VIEWED_TILE_2_TEMPLATE_QUANTITY_UNAVAILABLE') ?>
            <?php } ?>
        <?= Html::endTag('div') ?>
    <?php } ?>
