...
// seo
$arSeoItems = CNextCache::CIBLockElement_GetList(
	array(
		'CACHE' => array(
			"MULTI" =>"Y",
			"TAG" => CNextCache::GetIBlockCacheTag(CNextCache::$arIBlocks[SITE_ID]["aspro_next_catalog"]["aspro_next_catalog_info"][0])
		)
	),
	array(
	...
	false,
	false,
	array("ID", "IBLOCK_ID", "NAME", "PREVIEW_TEXT", "DETAIL_PICTURE", "PROPERTY_FILTER_URL", "PROPERTY_LINK_REGION", "PROPERTY_FORM_QUESTION", "PROPERTY_TIZERS", "PROPERTY_SECTION", "DETAIL_TEXT", "ElementValues")
);
...
if ($_REQUEST['FROM_POPUP'] === 'Y') {
	$GLOBALS[$arParams["FILTER_NAME"]]['!NAME'] = '%Беговел%'
	?>
	<img src="/upload/popup_result.jpg?1" alt="" style="width: 100%; height: auto"/>
	<br /><br />
	<a href="javascript:showPitbikePopup();" class="btn btn-default">Подобрать еще раз</a>
	<?php
}


if ($arResult['VARIABLES']['SECTION_ID'] && !$arResult['VARIABLES']['SMART_FILTER_PATH']) {
	$APPLICATION->IncludeComponent('ownedmuhaha:section.seo.links', '', Array(
		'SECTION_ID' => $arResult['VARIABLES']['SECTION_ID']
	));
}
elseif ($arResult['VARIABLES']['SECTION_ID'] && $arResult['VARIABLES']['SMART_FILTER_PATH']) {
	$arParams['SHOW_SECTION_DESC'] = 'N';
}

if ($checkCity === 'default') {
	echo $APPLICATION->ShowViewContent('sotbit_seometa_top_desc');
}
...
$seoData = Section::getSeo($arParams['IBLOCK_ID'], ['id' => $arResult['VARIABLES']['SECTION_ID']]);
