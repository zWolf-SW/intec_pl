...
return function (&$arItem) use (&$arResult, &$arVisual, &$sTemplateId) {
    ...
    if ($sType === 'A' || $sType === 'B' || $sType === 'K' || $sType === 'P' || $sType === 'R')
        $bSearchShow = false;
            ...
            <?= Html::beginTag('div', [
                ...
                <?php if (!empty($arItem['FILTER_HINT'])) { ?>
                    <div class="intec-grid-item-auto">
                        <div class="catalog-smart-filter-item-hint" data-role="hint">
                            <?= Html::tag('i', '', [
                                'class' => 'fal fa-question-circle',
                                'data-role' => 'hint.icon'
                            ]) ?>
                            <?= Html::tag('div', $arItem['FILTER_HINT'], [
                                'class' => 'catalog-smart-filter-item-hint-content',
                                'data' => [
                                    'role' => 'hint.content',
                                    'state' => 'hidden'
                                ]
        ...
        <div class="catalog-smart-filter-item-content" data-role="bx_filter_block">
            <?php if ($bSearchShow) { ?>
                <div class="smart-filter-search-wrapper intec-grid intec-grid-a-v-center" data-role="search.holder">
                    <input type="text" class="intec-grid-item" data-role="smart.filter.search" placeholder="<?= Loc::getMessage('C_CATALOG_SMART_FILTER_VERTICAL_2_SEARCH') ?>">
                ...
                <?php if ($sType === 'F') { ?>
                    <?= Html::beginTag('div', [
                        'class' => Html::cssClassFromArray([
                            ...
                            'scrollbar-inner' => $iItemCount > 6
                        ], true),
                        'data' => [
                            'role' => $iItemCount > 6 ? 'scrollbar' : 'none'
                        ]
                    ]) ?>
                        ...
                        <div class="catalog-smart-filter-search-nothing-found" data-role="nothing.found" data-status="disabled">
                            <?= Loc::getMessage('C_CATALOG_SMART_FILTER_VERTICAL_2_SEARCH_NOTHING_FOUND') ?>
                        </div>
                    <?= Html::endTag('div') ?>
