...
<div itemscope itemtype="http://schema.org/<?= $arVisual['MICRODATA']['TYPE'] ?>">
    ...
    <?php if (!empty($arResult['PREVIEW_TEXT'])) { ?>
        <meta itemprop="description" content="<?= htmlspecialchars(strip_tags($arResult['PREVIEW_TEXT'])) ?>" />
    <?php } ?>
    <?php if (!empty($arResult['DETAIL_TEXT'])) { ?>
        <meta itemprop="articleBody" content="<?= htmlspecialchars(strip_tags($arResult['DETAIL_TEXT'])) ?>" />
    <?php } ?>
</div>