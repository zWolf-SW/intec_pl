...
if (!empty($arParams['DETAIL_PRODUCTS_PROPERTY_IMAGES']))
    $arProductsFilter['PROPERTY_'.$arParams['DETAIL_PRODUCTS_PROPERTY_IMAGES']] = $arResult['VARIABLES']['ELEMENT_ID'];

if ($oCache->initCache(36000, 'ELEMENTS'.serialize($arProductsFilter), '/iblock/images')) {
...
if ($arDetail['SHOW']) {
    $sPrefix = 'DETAIL_';
    $arDetail['TEMPLATE'] = 'images.'.$arDetail['TEMPLATE'];
    ...
    $arDetail['PARAMETERS'] = ArrayHelper::merge($arDetail['PARAMETERS'], [
        ...
        'REGIONALITY_USE' => $arParams['REGIONALITY_USE'],
        'REGIONALITY_FILTER_USE' => $arParams['REGIONALITY_FILTER_USE'],
        'REGIONALITY_FILTER_PROPERTY' => $arParams['REGIONALITY_FILTER_PROPERTY'],
        'REGIONALITY_FILTER_STRICT' => $arParams['REGIONALITY_FILTER_STRICT'],
