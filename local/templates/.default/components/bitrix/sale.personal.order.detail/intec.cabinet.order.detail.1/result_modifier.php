...
$arChangeHistory = Arrays::fromDBResult(CSaleOrderChange::GetList(['ID' => 'DESC'], ['ORDER_ID' => $arResult['ID']]))->indexBy('ID');
...
if (!empty($arParams['PROPERTY_CLAIMS'])) {
    $arResult['PATH_TO_NEW_CLAIM'] .= '&'.$arParams['PROPERTY_CLAIMS'].'='.$arResult['ID'];
}

if (Loader::includeModule('support') && $USER->IsAuthorized() && !empty(array_intersect($arRolesUser, $arAdmissibleRoles)) && !empty($arParams['PROPERTY_CLAIMS'])) {
    $sSortBy = 's_id';
    $sSortOrder = 'desc';
