    ...
    <?php $fRender = function (&$arItem, $bOffer = false) use (&$arVisual) { ?>
        ...
        <?= Html::beginTag('div', [
            ...
            <?php if ($arItem['CAN_BUY']) { ?>
                <?php if ($arVisual['QUANTITY']['MODE'] === 'number') { ?>
                    <?php if ($arItem['CATALOG_QUANTITY'] > 0) { ?>
                        ...
                        <?= Loc::getMessage('C_CATALOG_SECTION_PRODUCTS_SMALL_8_QUANTITY_AVAILABLE').': ' ?>
                ...
                <?php } else if ($arVisual['QUANTITY']['MODE'] === 'text') {?>
                    ...
                    <?php if ($arItem['CATALOG_QUANTITY'] >= $arVisual['QUANTITY']['BOUNDS']['MANY'] || $arItem['CATALOG_QUANTITY'] <= 0) { ?>
                        <?= Loc::getMessage('C_CATALOG_SECTION_PRODUCTS_SMALL_8_QUANTITY_BOUNDS_MANY') ?>
                    <?php } else if ($arItem['CATALOG_QUANTITY'] <= $arVisual['QUANTITY']['BOUNDS']['FEW']) { ?>
                        <?= Loc::getMessage('C_CATALOG_SECTION_PRODUCTS_SMALL_8_QUANTITY_BOUNDS_FEW') ?>
                    <?php } else { ?>
                        <?= Loc::getMessage('C_CATALOG_SECTION_PRODUCTS_SMALL_8_QUANTITY_BOUNDS_ENOUGH') ?>
                    <?php } ?>
            ...
            <?php } else { ?>
                ...
                <?= Loc::getMessage('C_CATALOG_SECTION_PRODUCTS_SMALL_8_QUANTITY_UNAVAILABLE'); ?>
            <?php } ?>
        <?= Html::endTag('div') ?>
    <?php } ?>
