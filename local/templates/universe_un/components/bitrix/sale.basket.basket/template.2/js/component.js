	...
	BX.Sale.BasketComponent = {
		...
		bindActionEvents: function(node, data)
		{
			...
			if (BX.util.in_array('DELETE', this.params.COLUMNS_LIST))
			{
				...
				{
                    if (this.params['CONFIRM_REMOVE_PRODUCT_USE'] === 'Y') {
                        BX.bind(entity[i], 'click', BX.proxy(function () {
                            this.showConfirmRemoveProductContainer(this.confirmRemoveProductAgree);
                        }, this));
                    } else {
            ...
            entity = this.getEntity(node, 'quick-view');
            BX.bind(entity, 'click', BX.proxy(this.quickViewAction, this));

            entity = this.getEntities(node, 'basket-item-show-form');
            for (var i = 0, l = entity.length; i < l; i++)
            {
                BX.bind(entity[i], 'click', BX.proxy(this.showAlertFormAction, this));
            }
		},
        ...
        quickViewAction: function () {
            ...
            if (itemData) {
                var quickViewData = JSON.parse(itemData.QUICK_VIEW_DATA).quickView;

                if (quickViewData.show) {
                    this.quickViewShow(quickViewData, this.quickItemsId);
                }
            }
        },

        showConfirmRemoveProductContainer: function (functionName)
        {
            var object = this;
            var itemData = object.getItemDataByTarget(BX.proxy_context);
            var confirmRemoveProductBlock = object.nodes['basket-root'].querySelector('[data-role="confirm.remove.product"]');
            var cancels = object.getEntities(confirmRemoveProductBlock, 'confirm.remove.product.cancel');
            var agree = object.getEntities(confirmRemoveProductBlock, 'confirm.remove.product.agree');
            var delay = object.getEntities(confirmRemoveProductBlock, 'confirm.remove.product.delay');

            agree[0].onclick = function () {
                functionName(itemData.ID, object);
            };

            cancels.forEach(function (item) {
                item.onclick = function () {
                    confirmRemoveProductBlock.dataset.state = 'hidden';
                };
            });

            if (itemData.DELAYED) {
                delay[0].dataset.state = 'hidden';
            } else {
                delay[0].onclick = function () {
                    object.confirmRemoveProductAddDelay(itemData.ID, object);
                };
            }

            confirmRemoveProductBlock.dataset.state = 'visible';
        },

        confirmRemoveProductAgree: function (basketItemId, basketBlock)
        {
            if (basketItemId && basketBlock) {
                var confirmRemoveProductBlock = basketBlock.nodes['basket-root'].querySelector('[data-role="confirm.remove.product"]');

                basketBlock.actionPool.deleteItem(basketItemId);
                basketBlock.items[basketItemId].SHOW_LOADING = true;

                if (basketBlock.params.SHOW_RESTORE === 'Y' && basketBlock.isItemAvailable(basketItemId)) {
                    basketBlock.items[basketItemId].SHOW_RESTORE = true;
                }

                basketBlock.redrawBasketItemNode(basketItemId);
                confirmRemoveProductBlock.dataset.state = 'hidden';
            }
        },

        confirmRemoveProductAddDelay: function(basketItemId, basketBlock)
        {
            if (basketItemId && basketBlock) {
                var confirmRemoveProductBlock = basketBlock.nodes['basket-root'].querySelector('[data-role="confirm.remove.product"]');

                basketBlock.actionPool.addDelayed(basketItemId);
                basketBlock.items[basketItemId].SHOW_LOADING = true;
                basketBlock.redrawBasketItemNode(basketItemId);
                basketBlock.app.api.basket.update();
