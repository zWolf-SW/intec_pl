...
foreach ($arResult['ITEMS'] as &$arItem) {
    ...
    if (!empty($arParams['VIDEO_IBLOCK_ID']) && !empty($arParams['PROPERTY_VIDEO']) && !empty($arParams['VIDEO_IBLOCK_PROPERTY_LINK'])) {
        $arProperty = ArrayHelper::getValue($arItem['PROPERTIES'], [
...
if (!empty($arVideos)) {
    ...
    if (!empty($arVideosData)) {
        ...
        foreach ($arResult['ITEMS'] as &$arItem) {
            if (!empty($arItem['DATA']['VIDEO']['VALUE']) && $arVideosData->exists($arItem['DATA']['VIDEO']['VALUE'])) {
