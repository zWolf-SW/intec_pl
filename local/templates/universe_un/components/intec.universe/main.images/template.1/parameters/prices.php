...
if ($bBase) {
    $arTemplateParameters['PRODUCTS_PRICE_CODE'] = [
        'PARENT' => 'PRODUCTS',
        'NAME' => Loc::getMessage('C_MAIN_IMAGES_TEMPLATE_1_PRODUCTS_PRICE_CODE'),
        'TYPE' => 'LIST',
    ...
    if (Type::isArray($arCurrentValues['PRODUCTS_PRICE_CODE']) && !empty(array_filter($arCurrentValues['PRODUCTS_PRICE_CODE']))) {
        $arTemplateParameters['PRODUCTS_CONVERT_CURRENCY'] = [
            'PARENT' => 'PRODUCTS',
            'NAME' => Loc::getMessage('C_MAIN_IMAGES_TEMPLATE_1_PRODUCTS_CONVERT_CURRENCY'),
            'TYPE' => 'CHECKBOX',
            'DEFAULT' => 'N',
            'REFRESH' => 'Y'
        ];
        ...
        if ($arCurrentValues['PRODUCTS_CONVERT_CURRENCY'] === 'Y') {
            $arTemplateParameters['PRODUCTS_CURRENCY_ID'] = [
                'PARENT' => 'PRODUCTS',
                'NAME' => Loc::getMessage('C_MAIN_IMAGES_TEMPLATE_1_PRODUCTS_CURRENCY_ID'),
                'TYPE' => 'LIST',
        ...
        $arTemplateParameters['PRODUCTS_PRICE_VAT_INCLUDE'] = [
            'PARENT' => 'PRODUCTS',
            'NAME' => Loc::getMessage('C_MAIN_IMAGES_TEMPLATE_1_PRODUCTS_PRICE_VAT_INCLUDE'),
            'TYPE' => 'CHECKBOX',
            'DEFAULT' => 'N'
        ];
        $arTemplateParameters['PRODUCTS_SHOW_PRICE_COUNT'] = [
            'PARENT' => 'PRODUCTS',
            'NAME' => Loc::getMessage('C_MAIN_IMAGES_TEMPLATE_1_PRODUCTS_SHOW_PRICE_COUNT'),
            'TYPE' => 'STRING',
            'DEFAULT' => 1
        ];
    }
        ...
        foreach ($arCurrentValues['PRODUCTS_PRICE_CODE'] as $sCode) {
            if (!empty($sCode)) {
                ...
                $arTemplateParameters['PRODUCTS_PROPERTY_OLD_PRICE_'.$sCode] = [
                    'PARENT' => 'PRODUCTS',
                    'NAME' => Loc::getMessage('C_MAIN_IMAGES_TEMPLATE_1_PRODUCTS_PROPERTY_OLD_PRICE', [
